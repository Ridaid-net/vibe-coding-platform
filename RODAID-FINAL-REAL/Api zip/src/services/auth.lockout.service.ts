...
export async function verificarBloqueoCuenta(
  email: string
): Promise<{ bloqueado: boolean; segundosRestantes: number; intentosFallidos: number }> {
  const row = await queryOne<{
    lockout_hasta: Date | null
    login_intentos_fallidos: number
  }>(
    `SELECT lockout_hasta, login_intentos_fallidos
     FROM usuarios WHERE email = $1`,
    [email]
  )

  if (!row) return { bloqueado: false, segundosRestantes: 0, intentosFallidos: 0 }
...
export async function registrarFalloLogin(opts: {
  email:     string
  ip:        string
  userAgent: string
}): Promise<{ bloqueado: boolean; lockoutHasta: Date | null; intentos: number }> {
  ...
  const row = await queryOne<{
    ...
    `UPDATE usuarios
     SET login_intentos_fallidos = login_intentos_fallidos + 1,
         actualizado_en = NOW()
     WHERE email = $1
     RETURNING id, login_intentos_fallidos`,
    [opts.email]
  )

  if (!row) return { bloqueado: false, lockoutHasta: null, intentos: 0 }
  ...
  let lockoutHasta: Date | null = null

  if (minutos > 0) {
    lockoutHasta = new Date(Date.now() + minutos * 60_000)
    await query(
      `UPDATE usuarios SET lockout_hasta = $2 WHERE id = $1::uuid`,
      [row.id, lockoutHasta]
    )
    log.auth.warn({
  ...
  await query(
    `INSERT INTO auth_audit_log (usuario_id, evento, ip_address, user_agent, datos, ok)
     VALUES ($1::uuid, 'LOGIN_FALLIDO', $2::inet, $3, $4::jsonb, FALSE)`,
    [row.id, opts.ip, opts.userAgent, JSON.stringify({ intentos, bloqueado: minutos > 0 })]
  ).catch(() => {})
...
export async function resetearContadorLogin(userId: string, ip: string): Promise<void> {
  await query(
    `UPDATE usuarios
...
async function registrarIpFallo(ip: string, email: string): Promise<void> {
  try {
    ...
    if (totalCuentas >= IP_MAX_CUENTAS) {
      ...
      log.auth.warn({ ip, cuentas: totalCuentas }, `IP bloqueada por ataque masivo (${totalCuentas} cuentas)`)
    }
  } catch { /* Redis puede no estar disponible; no bloquear el flujo */ }
}

export async function verificarBloqueoIP(ip: string): Promise<boolean> {
  try {
    const redis  = getRedis()
...
export async function logAuthEvento(opts: {
  usuarioId?:  string
  evento:      string
  ip:          string
  userAgent:   string
  ...
  ).catch(err => log.auth.warn({ err: err.message }, 'logAuthEvento failed'))
}
...
export async function getAuditLog(opts: {
  ...
  if (opts.usuarioId) { conditions.push(`usuario_id = $${i++}::uuid`); params.push(opts.usuarioId) }
  if (opts.evento)    { conditions.push(`evento = $${i++}`)           ; params.push(opts.evento) }
  if (opts.ip)        { conditions.push(`ip_address = $${i++}::inet`); params.push(opts.ip) }

  return query<{
    id: string; evento: string; ip_address: string
    ok: boolean; datos: Record<string, unknown>; creado_en: Date
  }>(
    `SELECT id::text, evento, ip_address::text, ok, datos, creado_en
     FROM auth_audit_log
...
export async function getAuthStats() {
  const [stats, bloqueados, ultimaHora] = await Promise.all([
    queryOne<{
      total: number; activos: number; con2fa: number; admins: number
    }>(`
    ...
    queryOne<{ count: number }>(
      `SELECT COUNT(*)::int AS count FROM usuarios WHERE lockout_hasta > NOW()`
    ),
    queryOne<{ intentos: number; bloqueados: number; exitos: number }>(`
      SELECT
