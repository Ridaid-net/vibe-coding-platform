...
exports.acuñarCITEnBFA = acuñarCITEnBFA;
exports.verificarMintEnBFA = verificarMintEnBFA;
exports.getMintStatus = getMintStatus;
exports.getCITsMintFallido = getCITsMintFallido;
const database_1 = require("../config/database");
const logger_1 = require("../middleware/logger");
const bfa_service_1 = require("./bfa.service");
const bfa_indexer_1 = require("./bfa.indexer");
const env_1 = require("../config/env");
const RODAID_CUSTODIAL_WALLET = env_1.env.RODAID_CUSTODIAL_WALLET
...
// ══════════════════════════════════════════════════════════
async function acuñarCITEnBFA(citId, propietarioWallet) {
    const timer = (0, logger_1.startTimer)('mint.bfa', { citId });
    // ── 1. Cargar datos completos del CIT ─────────────────────
    const cit = await (0, database_1.queryOne)(`SELECT c.id, c.numero_cit, c.hash_sha256, c.estado,
     ...
     WHERE c.id = $1`, [citId]);
    if (!cit)
        throw new Error(`CIT ${citId} no encontrado`);
    // ── 2. Pre-checks ─────────────────────────────────────────
    // Idempotencia: si ya está acuñado, retornar datos existentes
    if (cit.nft_token_id && cit.bfa_tx_hash) {
        logger_1.log.bfa.info({ citId, tokenId: cit.nft_token_id }, 'NFT ya acuñado — idempotente');
    ...
    if (cit.hash_sha256) {
        try {
            const bfaCheck = await bfa_service_1.bfaService.verificarIntegridad(cit.hash_sha256);
            ...
            if (err.message.includes('bloqueado en BFA'))
                throw err;
            // Otros errores de BFA (nodo no disponible) → continuar igual
            logger_1.log.bfa.warn({ citId, err: err.message }, 'verificarIntegridad pre-mint falló — continuando');
        }
    }
    // ── 3. Determinar wallet destino ──────────────────────────
    const propietario = await (0, database_1.queryOne)(`SELECT wallet_address FROM usuarios WHERE id = $1`, [cit.propietario_id]);
    ...
    let mintResult;
    try {
        mintResult = await bfa_service_1.bfaService.mint(walletDestino, cit.hash_sha256, cit.numero_cit, cit.numero_serie);
    }
    catch (err) {
       ...
       SET mint_estado       = $2,
           mint_ultimo_error = $3,
           actualizado_en    = NOW()
       WHERE id = $1`, [citId, reintentable ? 'REINTENTANDO' : 'FALLIDO', errMsg]);
        logger_1.log.bfa.error({ citId, errMsg, reintentable }, `✗ Mint BFA falló`);
        throw Object.assign(err, { reintentable });
    }
    ...
    await (0, database_1.transaction)(async (client) => {
        await client.query(`UPDATE cits
       SET estado           = 'ACTIVO',
           nft_token_id     = $2,
           bfa_tx_hash      = $3,
           fecha_emision    = $4,
           ...
           actualizado_en   = NOW()
       WHERE id = $1`, [citId, mintResult.tokenId, mintResult.txHash, ahora, vence]);
        // Notificar al propietario (fire-and-forget, no bloquea la TX)
        const bfaExplorerUrl = (env_1.env.BFA_CHAIN_ID === 4337)
        ...
        void client.query(`INSERT INTO notificaciones (usuario_id, tipo, titulo, cuerpo, datos)
    ...
    let indexado = false;
    try {
        const inspectorWallet = await (0, database_1.queryOne)(`SELECT wallet_address FROM inspectores WHERE id = $1`, [cit.inspector_id]);
        await (0, bfa_indexer_1.indexStubEvent)('CITMinted', mintResult.tokenId, mintResult.txHash, mintResult.blockNumber, {
        ...
        logger_1.log.bfa.warn({ citId, err: err.message }, 'Indexación CITMinted falló — indexer recogerá en próximo poll');
    }
    // ── 8. IPFS en background ─────────────────────────────────
    _subirIPFSBackground(citId, cit.numero_cit).catch(err => {
        logger_1.log.bfa.warn({ citId, err: err.message }, 'IPFS post-mint falló');
    });
    logger_1.log.bfa.info({
        citId, numeroCIT: cit.numero_cit,
...
async function _subirIPFSBackground(citId, numeroCIT) {
    const { subirPDFCIT, subirMetadataCIT, buildTokenURI } = await import('./ipfs.service');
    const { generarPDFCIT } = await import('./pdf.service');
    const citCompleto = await (0, database_1.queryOne)(`SELECT c.puntos, c.punto_detalle, c.hash_sha256, c.nft_token_id, c.bfa_tx_hash,
    ...
    await (0, database_1.query)(`UPDATE cits SET ipfs_pdf_cid=$2, ipfs_metadata_cid=$3, token_uri=$4, ipfs_subido_en=NOW() WHERE id=$1`, [citId, pdfResult.cid, metaResult.cid, buildTokenURI(metaResult.cid)]);
}
async function verificarMintEnBFA(citId) {
    const row = await (0, database_1.queryOne)(`SELECT nft_token_id, hash_sha256, propietario_id FROM cits WHERE id=$1`, [citId]);
    if (!row?.nft_token_id) {
    ...
    try {
        const [integridad, datos] = await Promise.all([
            bfa_service_1.bfaService.verificarIntegridad(row.hash_sha256),
            bfa_service_1.bfaService.datosCIT(row.nft_token_id),
        ]);
        ...
        const propietarioWallet = await (0, database_1.queryOne)(`SELECT wallet_address FROM usuarios WHERE id=$1`, [row.propietario_id]);
...
// ══════════════════════════════════════════════════════════
async function getMintStatus(citId) {
    return (0, database_1.queryOne)(`SELECT id AS "citId", numero_cit AS "numeroCIT", mint_estado AS "mintEstado",
     ...
     FROM cits WHERE id=$1`, [citId]);
}
async function getCITsMintFallido() {
    return (0, database_1.query)(`SELECT id AS "citId", numero_cit AS "numeroCIT", mint_estado AS "mintEstado",
            mint_intentos AS "mintIntentos", nft_token_id AS "tokenId",
            bfa_tx_hash AS "txHash", mint_ultimo_error AS "ultimoError",
