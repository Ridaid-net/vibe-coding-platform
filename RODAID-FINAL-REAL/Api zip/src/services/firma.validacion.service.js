...
exports.validarFirmaPreBFA = validarFirmaPreBFA;
exports.finalizarCITConValidacion = finalizarCITConValidacion;
exports.getHistorialValidaciones = getHistorialValidaciones;
exports.getEstadisticasValidaciones = getEstadisticasValidaciones;
...
// VALIDADOR PRINCIPAL
// ══════════════════════════════════════════════════════════
async function validarFirmaPreBFA(citId) {
    ...
    // ── CHK-1: FIRMA_EXISTE ────────────────────────────────
    const t1 = Date.now();
    const firmaRow = await (0, database_1.queryOne)(`SELECT id, payload_json, firma_base64url, cert_pem, cert_serial, cert_subject,
            firmado_en, valida_hasta, revocada, inspector_id, payload_hash
     FROM firmas_payload_cit WHERE cit_id=$1 ORDER BY firmado_en DESC LIMIT 1`, [citId]);
    addCheck({
        id: 'FIRMA_EXISTE',
        ok: !!firmaRow,
    ...
    if (!firmaRow) {
        ...
        return await persistirYRetornar(citId, checks, aprobado, motivoRechazo, undefined, undefined, undefined, inicio);
    }
    firmaId = firmaRow.id;
    ...
    let certVigenteDetalle;
    try {
        const cert = node_forge_1.default.pki.certificateFromPem(firmaRow.cert_pem);
        const ahora = new Date();
        certVigente = ahora >= cert.validity.notBefore && ahora <= cert.validity.notAfter;
        const diasRestantes = Math.floor((cert.validity.notAfter.getTime() - ahora.getTime()) / 86_400_000);
        ...
        certVigenteDetalle = `Error parseando certificado: ${err.message}`;
    }
    addCheck({
        id: 'CERT_VIGENTE',
        ok: certVigente,
    ...
    if (firmaRow.inspector_id) {
        const insp = await (0, database_1.queryOne)(`SELECT i.activo, (ta.habilitado AND ta.activo) AS taller_habilitado
       FROM inspectores i
       JOIN talleres_aliados ta ON ta.id = i.taller_aliado_id
       WHERE i.id=$1`, [firmaRow.inspector_id]);
    ...
    const t6 = Date.now();
    const ptRow = await (0, database_1.queryOne)(`SELECT puntos_total FROM cit_puntos WHERE cit_id=$1`, [citId]);
    ...
    const t7 = Date.now();
    const fotosRow = await (0, database_1.queryOne)(`SELECT COUNT(*)::text AS count FROM cit_fotos WHERE cit_id=$1`, [citId]);
    ...
    const t8 = Date.now();
    const citRow = await (0, database_1.queryOne)(`SELECT estado, puntos_total FROM cits WHERE id=$1`, [citId]);
    ...
    return await persistirYRetornar(citId, checks, aprobado, motivoRechazo, firmaId, payloadHash, firmadoEn, inicio);
}
// ══════════════════════════════════════════════════════════
// PERSISTIR RESULTADO Y RETORNAR
// ══════════════════════════════════════════════════════════
async function persistirYRetornar(citId, checks, aprobado, motivoRechazo, firmaId, payloadHash, firmadoEn, inicio) {
    const duracionMs = Date.now() - inicio;
    const validadoEn = new Date();
    const get = (id) => checks.find(c => c.id === id)?.ok ?? false;
    const row = await (0, database_1.queryOne)(`INSERT INTO cit_firma_validaciones
 ...
 */
async function finalizarCITConValidacion(citId, propietarioWallet) {
    // 1. Validar firma ANTES del mint
    const validacion = await validarFirmaPreBFA(citId);
    if (!validacion.aprobado) {
        throw new errorHandler_1.AppError(`Firma digital inválida — BFA bloqueado: ${validacion.motivoRechazo}`, 422, 'FIRMA_INVALIDA', {
...
// ══════════════════════════════════════════════════════════
async function getHistorialValidaciones(citId) {
    return (0, database_1.query)(`SELECT id, aprobado, motivo_rechazo,
            chk_firma_existe, chk_rsa_pss_ok, chk_cert_vigente, chk_cert_no_revocado,
            chk_inspector_activo, chk_puntos_suficientes, chk_fotos_minimas, chk_dj_firmada,
            firmado_en, validado_en
     FROM cit_firma_validaciones WHERE cit_id=$1 ORDER BY validado_en DESC`, [citId]);
}
async function getEstadisticasValidaciones(dias = 30) {
    const [resumen, motivos] = await Promise.all([
        (0, database_1.queryOne)(`SELECT COUNT(*)::text AS total, COUNT(*) FILTER(WHERE aprobado)::text AS aprob
       FROM cit_firma_validaciones WHERE validado_en > NOW()-($1||' days')::interval`, [dias]),
        (0, database_1.query)(`SELECT motivo_rechazo AS motivo, COUNT(*)::text AS count
