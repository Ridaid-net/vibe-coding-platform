...
async function validateTOTP(code: string, secret: string, userId: string): Promise<void> {
  ...
  const result = await verify({ token: clean, secret })
  if (!result.valid) throw new AppError('Código 2FA incorrecto. Verificá que la hora de tu dispositivo esté sincronizada.', 401, 'TOTP_INVALID')
