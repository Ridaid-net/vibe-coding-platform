...
function getEncryptionKey(version = VERSION_CLAVE): Buffer {
  ...
  if (!keyHex || keyHex.length < 64) {
    ...
    if (process.env.NODE_ENV !== 'production') {
      const stub = crypto.scryptSync('rodaid-dev-key-DO-NOT-USE-IN-PROD', 'rodaid-salt', 32)
      return stub
    }
    throw new Error(
 ...
 */
export function cifrar(texto: string, keyVersion = VERSION_CLAVE): string {
  if (!texto) return ''
 ...
 */
export function descifrar(valorEnc: string, keyVersion = VERSION_CLAVE): string {
  if (!valorEnc) return ''
  try {
    const clave  = getEncryptionKey(keyVersion)
    const buf    = Buffer.from(valorEnc, 'base64')
    if (buf.length < IV_BYTES + TAG_BYTES + 1) return ''  // datos corruptos
    ...
    log.firma.warn({ valorEnc: valorEnc.slice(0, 20) }, '⚠ Error descifrando campo — datos corruptos o clave incorrecta')
    return ''
  }
}
 ...
 */
export function esCifrado(valor: string): boolean {
 ...
 */
export async function cifrarCampo(opts: {
  tabla:       string
  registroId:  string
  campo:       string
  valor:       string
  ...
  await query(
    `INSERT INTO encrypted_fields (tabla, registro_id, campo, valor_enc, version_clave)
     VALUES ($1,$2,$3,$4,$5)
     ON CONFLICT (tabla, registro_id, campo) DO UPDATE SET
       valor_enc    = EXCLUDED.valor_enc,
       version_clave= EXCLUDED.version_clave,
       rotado_en    = NOW()`,
    [opts.tabla, opts.registroId, opts.campo, enc, VERSION_CLAVE]
  )
}
 ...
 */
export async function descifrarCampo(opts: {
  tabla:       string
  registroId:  string
  campo:       string
}): Promise<string | null> {
  const row = await queryOne<{ valor_enc: string; version_clave: number }>(
    `SELECT valor_enc, version_clave FROM encrypted_fields
     WHERE tabla=$1 AND registro_id=$2 AND campo=$3`,
    [opts.tabla, opts.registroId, opts.campo]
  )
  if (!row) return null
 ...
 */
export async function cifrarCampos(
  tabla: string,
  registroId: string,
...
export async function descifrarCampos(
  ...
  const rows = await query<{ campo: string; valor_enc: string; version_clave: number }>(
    `SELECT campo, valor_enc, version_clave FROM encrypted_fields
 ...
 */
export async function redisSetCifrado(
  key: string,
  valor: unknown,
  ttlSegundos?: number
): Promise<void> {
 ...
 */
export async function redisGetCifrado<T = unknown>(key: string): Promise<T | null> {
  const redis   = getRedis()
  const fullKey = `${REDIS_ENC_PREFIX}${key}`
...
export async function redisDelCifrado(key: string): Promise<void> {
  await getRedis().del(`${REDIS_ENC_PREFIX}${key}`)
}
...
export function enmascararDNI(dni: string | null | undefined): string {
  if (!dni) return ''
...
export function enmascararEmail(email: string | null | undefined): string {
  ...
  const mask = (s: string) => s.length <= 2 ? '***' : s[0] + '*'.repeat(s.length - 2) + s.slice(-1)
  return `${mask(user)}@${mask(domName)}.${ext.join('.')}`
}

export function enmascararTelefono(tel: string | null | undefined): string {
  if (!tel) return ''
 ...
 */
export function enmascararPII<T extends Record<string, unknown>>(obj: T): T {
  const campos: Record<string, (v: any) => string> = {
    dni:            enmascararDNI,
    cuil:           enmascararDNI,
  ...
  for (const [campo, fn] of Object.entries(campos)) {
    if (campo in result && result[campo]) {
      (result as any)[campo] = fn(result[campo])
    }
  }
  return result
}
 ...
 */
export async function rotarClaves(opts: {
  versionAntigua: number
  versionNueva:   number
  dryRun?:        boolean
}): Promise<{ total: number; rotados: number; errores: number }> {
  const rows = await query<{
    id: string; tabla: string; registro_id: string; campo: string
    valor_enc: string; version_clave: number
  }>(
    `SELECT id, tabla, registro_id, campo, valor_enc, version_clave
     FROM encrypted_fields WHERE version_clave=$1`,
    [opts.versionAntigua]
  )

  let rotados = 0; let errores = 0
  for (const row of rows) {
    try {
      const plain = descifrar(row.valor_enc, opts.versionAntigua)
      if (!plain) { errores++; continue }
      const newEnc = cifrar(plain, opts.versionNueva)
      if (!opts.dryRun) {
        await query(
          `UPDATE encrypted_fields SET valor_enc=$2, version_clave=$3, rotado_en=NOW() WHERE id=$1`,
          [row.id, newEnc, opts.versionNueva]
        )
      }
      rotados++
    } catch { errores++ }
...
export async function getEstadisticasCifrado() {
  const [campos, porVersion] = await Promise.all([
    ...
    query<any>(
      `SELECT version_clave, COUNT(*)::int AS count
 ...
 */
export function generarClave(): { keyHex: string; keyBase64: string; bits: number } {
