...
exports.getTendenciaDiaria = getTendenciaDiaria;
exports.getSerialPopular = getSerialPopular;
exports.getMetricasRealtime = getMetricasRealtime;
exports.invalidarCacheAnalytics = invalidarCacheAnalytics;
...
// RESUMEN DEL PERÍODO
// ══════════════════════════════════════════════════════════
async function getResumenPeriodo(dias = 7) {
    ...
    const [totales, porOrigen, porEstado, perf] = await Promise.all([
       ...
       WHERE creado_en >= $1 AND creado_en < $2`, [desde, hasta]),
        (0, database_1.query)(`SELECT origen, COUNT(*)::text AS count
       FROM verificaciones_log
       WHERE creado_en >= $1 AND NOT es_bot
       GROUP BY origen ORDER BY count DESC`, [desde]),
        (0, database_1.query)(`SELECT COALESCE(estado_cit,'DESCONOCIDO') AS estado_cit, COUNT(*)::text AS count
       FROM verificaciones_log
       WHERE creado_en >= $1 AND encontrado AND NOT es_bot
       GROUP BY estado_cit ORDER BY count DESC`, [desde]),
        (0, database_1.queryOne)(`SELECT
       ...
       FROM verificaciones_log
       WHERE creado_en >= $1 AND NOT es_bot AND ms IS NOT NULL`, [desde]),
    ]);
    // ── Top seriales ──────────────────────────────────────────
    const topSeriales = await (0, database_1.query)(`SELECT v.serial, COUNT(*)::text AS consultas, MAX(v.creado_en) AS ultima,
    ...
    const distribHoraria = await (0, database_1.query)(`SELECT EXTRACT(HOUR FROM creado_en)::text AS hora,
            ROUND(COUNT(*)::numeric / $2)::text AS consultas
     FROM verificaciones_log
     WHERE creado_en >= $1 AND NOT es_bot
     GROUP BY hora ORDER BY hora`, [desde, dias]);
    ...
    logger_1.log.analytics.debug({ periodo: `${dias}d`, total }, '✓ Resumen analytics calculado');
    return result;
}
// ══════════════════════════════════════════════════════════
// TENDENCIA DIARIA
// ══════════════════════════════════════════════════════════
async function getTendenciaDiaria(dias = 30) {
...
// ══════════════════════════════════════════════════════════
async function getSerialPopular(limit = 20) {
    const cacheKey = `seriales:top${limit}`;
    const cached = await getCached(cacheKey);
    if (cached)
        return cached;
    // Consultas esta semana vs semana anterior
    const rows = await (0, database_1.query)(`WITH esta_semana AS (
...
// ══════════════════════════════════════════════════════════
async function getMetricasRealtime() {
...
// ══════════════════════════════════════════════════════════
async function invalidarCacheAnalytics() {
    try {
        const redis = (0, redis_1.getRedis)();
        const keys = await redis.keys('analytics:*');
        if (keys.length > 0)
            await redis.del(...keys);
        logger_1.log.analytics.info({ keys: keys.length }, '✓ Caché analytics invalidado');
    }
    catch { /* best-effort */ }
}
