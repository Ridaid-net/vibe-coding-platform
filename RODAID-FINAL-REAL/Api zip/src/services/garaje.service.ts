...
// ══════════════════════════════════════════════════════════
// TIPOS (compartidos con el frontend via rodaid-api-client.ts)
// ══════════════════════════════════════════════════════════
...
export interface BicicletaGaraje {
  ...
  scoreSalud:  number          // 0-100 calculado
}
...
function calcularEstadoCIT(row: any): EstadoCIT {
  if (!row.cit_id)              return 'SIN_CIT'
  if (row.cit_estado === 'BORRADOR') return 'BORRADOR'
  if (row.cit_estado === 'ACTIVO') {
    if (!row.tasa_pagada)       return 'PENDIENTE_PAGO'
    const venc = row.fecha_vencimiento ? new Date(row.fecha_vencimiento) : null
    if (venc && venc < new Date()) return 'EXPIRADO'
    return 'ACTIVO'
  }
  return 'SIN_CIT'
}

function calcularScoreSalud(cit: CITResumen | null, certAseg: CertAsegResumen | null): number {
  if (!cit) return 0
...
export async function getGarajeResumen(usuarioId: string): Promise<GarajeResumen> {
  ...
  const bikesMap = new Map<string, BicicletaGaraje>()

  for (const row of rows) {
    if (bikesMap.has(row.id)) continue

    const cit: CITResumen | null = row.cit_id ? {
      id:              row.cit_id,
    ...
    const certAseg: CertAsegResumen | null = row.cert_num ? {
      numero:    row.cert_num,
    ...
    const poliza: PolizaResumen | null = row.numero_poliza ? {
  ...
  const result: GarajeResumen = { bicicletas, resumen }
...
export async function getCITById(citId: string, usuarioId: string) {
