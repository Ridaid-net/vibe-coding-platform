...
exports.simularDeposito = simularDeposito;
exports.confirmarEnvio = confirmarEnvio;
exports.confirmarEntrega = confirmarEntrega;
exports.procesarAutoReleases = procesarAutoReleases;
exports.cancelarTransaccion = cancelarTransaccion;
exports.abrirDisputa = abrirDisputa;
exports.resolverDisputa = resolverDisputa;
exports.getTransaccion = getTransaccion;
exports.getEventos = getEventos;
exports.getMisTransacciones = getMisTransacciones;
const crypto_1 = __importDefault(require("crypto"));
const database_1 = require("../config/database");
const mercadopago_service_1 = require("./mercadopago.service");
const comision_service_1 = require("./comision.service");
const nft_transfer_service_1 = require("./nft.transfer.service");
const escrow_eventos_1 = require("./escrow.eventos");
...
async function _registrarEventoLOCAL(opts) {
    ...
    await (0, database_1.query)(eventoSQL, eventoParams).catch((err) => logger_1.log.escrow.error({ err: err.message }, '✗ Error escrow evento local'))
        .catch(err => logger_1.log.escrow.warn({ err: err.message }, 'Error registrando evento escrow'));
}
async function transicionarEstado(id, nuevoEstado, camposFecha, // col → 'NOW()' | 'NOW() + INTERVAL ...'
camposExtra) {
    ...
    await (0, database_1.query)(`UPDATE transacciones SET ${sets.join(',')} WHERE id=$1::uuid`, params);
}
function calcularComision(precioARS, plan = 'LIBRE') {
...
async function iniciarCompra(input) {
    ...
    const pub = await (0, database_1.queryOne)(`SELECT id, estado::text AS estado, precio_ars, vendedor_id, NULL::uuid AS comprador_id,
            titulo, slug
    ...
    if (pub.vendedor_id === input.compradorId)
        throw Object.assign(new Error('El vendedor no puede comprar su propia publicación'), { code: 'SELF_PURCHASE', status: 422 });
    // Verificar que no hay transacción activa (también antes del estado check)
    const txActiva = await (0, database_1.queryOne)(`SELECT id FROM transacciones
     WHERE publicacion_id=$1 AND estado_pago NOT IN ('CANCELADA','FALLIDO','LIBERADO','DEVUELTO','COMPLETADA')`, [input.publicacionId]);
    if (txActiva)
        throw Object.assign(new Error('Ya existe una transacción activa para esta publicación'), { code: 'TX_DUPLICATE', status: 409 });
    if (pub.estado !== 'ACTIVA' && pub.estado !== 'PAUSADA')
        throw Object.assign(new Error(`La publicación no está disponible (estado: ${pub.estado})`), { code: 'NOT_AVAILABLE', status: 409 });
    // Calcular importes
    ...
    const comprador = await (0, database_1.queryOne)(`SELECT email FROM usuarios WHERE id=$1`, [input.compradorId]);
    // Crear preference MercadoPago (o STUB)
    const mpResult = await (0, mercadopago_service_1.crearPreferencia)({
        transaccionId: crypto_1.default.randomUUID(), // placeholder — se reemplaza al insertar tx
        monto: precioARS,
    ...
    // Crear transacción
    const tx = await (0, database_1.queryOne)(`INSERT INTO transacciones
    ...
    await (0, database_1.query)(`UPDATE marketplace_publicaciones SET estado='PAUSADA' WHERE id=$1`, [input.publicacionId]);
    // Evento
    await (0, escrow_eventos_1.registrarEvento)({
        transaccionId: tx.id,
...
// ══════════════════════════════════════════════════════════
async function webhookPago(opts) {
    const txId = opts.transaccionId ?? opts.externalReference;
    if (!txId)
        return { ok: false, estado: 'PENDIENTE' };
    const tx = await (0, database_1.queryOne)(`SELECT id, estado_pago::text, monto_ars FROM transacciones WHERE id=$1`, [txId]);
    if (!tx)
        return { ok: false, estado: 'PENDIENTE' };
    if (!['PENDIENTE', 'DEPOSITO_PENDIENTE'].includes(tx.estado_pago)) {
        return { ok: true, estado: tx.estado_pago };
    }
    ...
    if (opts.status === 'rejected') {
        await transicionarEstado(txId, 'CANCELADA', { cancelada_en: 'NOW()' }, { cancelada_motivo: `Pago rechazado por ${opts.gateway}: ${opts.paymentId}` });
        // Re-activar publicación
        await (0, database_1.query)(`UPDATE marketplace_publicaciones mp
       SET estado='ACTIVA'
       FROM transacciones tx
       WHERE tx.id=$1::uuid AND mp.id=tx.publicacion_id`, [txId]);
        await (0, escrow_eventos_1.registrarEvento)({ transaccionId: txId, evento: 'PAGO_RECHAZADO',
...
// ══════════════════════════════════════════════════════════
async function simularDeposito(transaccionId) {
    const stubPaymentId = `STUB_PAY_${crypto_1.default.randomBytes(4).toString('hex').toUpperCase()}`;
    ...
    logger_1.log.escrow.info({ transaccionId, stubPaymentId }, '🔄 STUB: depósito simulado');
    return { estado: result.estado };
}
// ══════════════════════════════════════════════════════════
// 3. CONFIRMAR ENVÍO (vendedor)
// ══════════════════════════════════════════════════════════
async function confirmarEnvio(opts) {
    const tx = await (0, database_1.queryOne)(`SELECT id, estado_pago::text, vendedor_id FROM transacciones WHERE id=$1`, [opts.transaccionId]);
    if (!tx)
        throw Object.assign(new Error('Transacción no encontrada'), { code: 'NOT_FOUND', status: 404 });
    if (tx.vendedor_id !== opts.vendedorId)
        throw Object.assign(new Error('Solo el vendedor puede confirmar el envío'), { code: 'FORBIDDEN', status: 403 });
    if (tx.estado_pago !== 'FONDOS_RETENIDOS')
        throw Object.assign(new Error(`Estado inválido para confirmar envío: ${tx.estado_pago}`), { code: 'INVALID_STATE', status: 422 });
    ...
    logger_1.log.escrow.info({ txId: opts.transaccionId, vendedorId: opts.vendedorId }, '✓ Envío confirmado por vendedor — estado: EN_CAMINO');
    // Fetch updated values from DB
    const txUpdated = await (0, database_1.queryOne)(`SELECT auto_release_en, TRUE AS envio_confirmado FROM transacciones WHERE id=$1`, [opts.transaccionId]);
    return {
        estado: 'EN_CAMINO',
...
// ══════════════════════════════════════════════════════════
async function confirmarEntrega(opts) {
    const tx = await (0, database_1.queryOne)(`SELECT t.id, t.estado_pago::text, t.comprador_id, t.monto_ars, t.comision_ars,
     ...
     WHERE t.id=$1`, [opts.transaccionId]);
    if (!tx)
        throw Object.assign(new Error('Transacción no encontrada'), { code: 'NOT_FOUND', status: 404 });
    if (tx.comprador_id !== opts.compradorId)
        throw Object.assign(new Error('Solo el comprador puede confirmar la entrega'), { code: 'FORBIDDEN', status: 403 });
    if (!['EN_CAMINO', 'FONDOS_RETENIDOS'].includes(tx.estado_pago))
        throw Object.assign(new Error(`Estado inválido: ${tx.estado_pago}`), { code: 'INVALID_STATE', status: 422 });
    ...
    logger_1.log.escrow.info({ comprobanteId: liberacion.comprobanteId }, '✅ Liberación completada');
    ...
    await (0, database_1.query)(`UPDATE marketplace_publicaciones SET estado='VENDIDA',
       vendido_en=NOW(), comprador_id=$1, precio_final_ars=$2, comision_rodaid=$3
     WHERE id=$4`, [opts.compradorId, parseFloat(tx.monto_ars), comisionRodaid, tx.publicacion_id]);
    await (0, escrow_eventos_1.registrarEvento)({
        transaccionId: opts.transaccionId,
    ...
    (0, comision_service_1.registrarComision)({
        transaccionId: opts.transaccionId,
        vendedorId: tx.vendedor_id,
        compradorId: opts.compradorId,
        ...
        gateway: tx.gateway ?? 'STUB',
        mpPaymentId: tx.mp_payment_id ?? undefined,
    }).catch((e) => logger_1.log.escrow.error({ err: e.message }, 'Error registrando comisión'));
    logger_1.log.escrow.info({
    ...
    const citId = await (0, database_1.queryOne)(`SELECT c.id AS cit_id FROM cits c
     JOIN marketplace_publicaciones mp ON mp.bicicleta_id=c.id
     WHERE mp.id=$1`, [tx.publicacion_id]);
    if (citId?.cit_id) {
        (0, nft_transfer_service_1.transferirNFTAlComprador)({
            citId: citId.cit_id,
            transaccionId: opts.transaccionId,
            compradorId: opts.compradorId,
            vendedorId: tx.vendedor_id,
            ...
            logger_1.log.escrow.error({ err: err.message }, '⚠ NFT transfer encolado para reintento');
        });
    }
    // ── Transferencia de Dominio (después de la transferencia NFT) ──
    if (tx.cit_id_from_pub) {
        import('./dominio.transfer.service').then(svc => svc.iniciarTransferenciaDominio({
            transaccionId: opts.transaccionId,
            citId: tx.cit_id_from_pub,
            vendedorId: tx.vendedor_id,
            compradorId: opts.compradorId,
            precioArs: parseFloat(tx.monto_ars),
            comisionArs: parseFloat(tx.comision_ars),
            ip: opts.ip,
        })).catch(err => logger_1.log.escrow.error({ err: err.message }, '⚠ Transferencia de dominio falló — registrar manualmente'));
    }
...
// ══════════════════════════════════════════════════════════
async function procesarAutoReleases() {
    const vencidas = await (0, database_1.query)(`SELECT id, monto_vendedor, vendedor_id, comprador_id, precio_ars, comision_ars, gateway FROM transacciones
    ...
    for (const tx of vencidas) {
        try {
            ...
            procesadas++;
            logger_1.log.escrow.info({ txId: tx.id }, `✓ Auto-release procesado`);
        }
        catch (err) {
            logger_1.log.escrow.error({ txId: tx.id, err: err.message }, 'Error en auto-release');
        }
    }
    if (procesadas > 0)
        logger_1.log.escrow.info({ procesadas }, `✓ ${procesadas} auto-releases procesados`);
    return { procesadas };
}
// ══════════════════════════════════════════════════════════
// 5. CANCELAR (reembolso al comprador)
// ══════════════════════════════════════════════════════════
async function cancelarTransaccion(opts) {
    const tx = await (0, database_1.queryOne)(`SELECT id, estado_pago::text, monto_ars, comprador_id, vendedor_id, publicacion_id
     FROM transacciones WHERE id=$1`, [opts.transaccionId]);
    if (!tx)
        throw Object.assign(new Error('Transacción no encontrada'), { code: 'NOT_FOUND', status: 404 });
    const estadosPermitidos = ['DEPOSITO_PENDIENTE', 'FONDOS_RETENIDOS', 'EN_CAMINO'];
    if (!estadosPermitidos.includes(tx.estado_pago))
        throw Object.assign(new Error(`No se puede cancelar en estado: ${tx.estado_pago}`), { code: 'INVALID_STATE', status: 422 });
    ...
    await transicionarEstado(opts.transaccionId, 'CANCELADA', { cancelada_en: 'NOW()' }, { cancelada_motivo: opts.motivo });
    // Re-activar publicación
    await (0, database_1.query)(`UPDATE marketplace_publicaciones SET estado='ACTIVA' WHERE id=$1`, [tx.publicacion_id]);
    await (0, escrow_eventos_1.registrarEvento)({
        transaccionId: opts.transaccionId,
    ...
    if (reembolsado) {
        (0, comision_service_1.devolverComision)(opts.transaccionId).catch(() => { });
    }
    logger_1.log.escrow.info({ txId: opts.transaccionId, reembolsado, motivo: opts.motivo }, '✓ Transacción cancelada');
    return { estado: 'CANCELADA', reembolsado };
}
// ══════════════════════════════════════════════════════════
// 6. ABRIR DISPUTA
// ══════════════════════════════════════════════════════════
async function abrirDisputa(opts) {
    const tx = await (0, database_1.queryOne)(`SELECT estado_pago::text, comprador_id, vendedor_id FROM transacciones WHERE id=$1`, [opts.transaccionId]);
    if (!tx)
        throw Object.assign(new Error('Transacción no encontrada'), { code: 'NOT_FOUND', status: 404 });
    if (!['FONDOS_RETENIDOS', 'EN_CAMINO'].includes(tx.estado_pago))
        throw Object.assign(new Error(`No se puede disputar en estado: ${tx.estado_pago}`), { code: 'INVALID_STATE', status: 422 });
    ...
    if (!esComprador && !esVendedor)
        throw Object.assign(new Error('Solo compradores o vendedores de la transacción pueden abrir disputas'), { code: 'FORBIDDEN', status: 403 });
    const disputa = await (0, database_1.queryOne)(`INSERT INTO disputas
       (transaccion_id, abierta_por_id, motivo, descripcion)
     VALUES ($1,$2,$3,$4) RETURNING id`, [opts.transaccionId, actorId, opts.motivo, opts.descripcion ?? null]);
    await transicionarEstado(opts.transaccionId, 'DISPUTADA', undefined, { disputa_id: disputa.id });
    await (0, escrow_eventos_1.registrarEvento)({
        transaccionId: opts.transaccionId,
    ...
    logger_1.log.escrow.warn({ txId: opts.transaccionId, disputaId: disputa.id, motivo: opts.motivo }, '⚠ Disputa abierta — equipo RODAID debe revisar');
    return { disputaId: disputa.id };
}
// ══════════════════════════════════════════════════════════
// 7. RESOLVER DISPUTA (admin)
// ══════════════════════════════════════════════════════════
async function resolverDisputa(opts) {
    const disputa = await (0, database_1.queryOne)(`SELECT transaccion_id, estado FROM disputas WHERE id=$1`, [opts.disputaId]);
    if (!disputa)
        throw Object.assign(new Error('Disputa no encontrada'), { code: 'NOT_FOUND', status: 404 });
    const tx = await (0, database_1.queryOne)(`SELECT monto_vendedor, monto_ars, vendedor_id, comprador_id
    ...
    await (0, database_1.query)(`UPDATE escrow_disputas SET estado=$2, resolucion=$3, resuelta_por=$4, resuelta_en=NOW()
     WHERE id=$1`, [opts.disputaId, estadoDisputa, opts.descripcion, opts.adminId]);
    let estadoFinal;
...
async function liberarFondos(opts) {
    ...
    logger_1.log.escrow.info({ ...opts }, '💸 FONDOS LIBERADOS al vendedor');
    await (0, escrow_eventos_1.registrarEvento)({
        transaccionId: opts.transaccionId,
...
async function procesarReembolso(opts) {
    logger_1.log.escrow.info({ ...opts }, '💸 REEMBOLSO al comprador');
    await (0, escrow_eventos_1.registrarEvento)({
        transaccionId: opts.transaccionId,
...
async function getTransaccion(id) {
    const row = await (0, database_1.queryOne)(`SELECT id, publicacion_id, comprador_id, vendedor_id,
     ...
     FROM transacciones WHERE id=$1`, [id]);
    if (!row)
        return null;
    return {
        id: row.id,
...
async function getEventos(transaccionId) {
    return (0, database_1.query)(`SELECT evento, estado_previo, estado_nuevo, actor_tipo, datos, creado_en
     FROM escrow_eventos WHERE transaccion_id=$1 ORDER BY creado_en`, [transaccionId]);
}
// ══════════════════════════════════════════════════════════
// HISTORIAL DE TRANSACCIONES DEL USUARIO
// ══════════════════════════════════════════════════════════
async function getMisTransacciones(opts) {
    ...
    const [rows, countRow] = await Promise.all([
        (0, database_1.query)(`SELECT id, publicacion_id, comprador_id, vendedor_id,
       ...
       ORDER BY creada_en DESC LIMIT $2 OFFSET $3`, [opts.usuarioId, porPagina, offset]),
        (0, database_1.queryOne)(`SELECT COUNT(*)::text AS count FROM transacciones WHERE ${cond}`, [opts.usuarioId]),
    ]);
    return {
        items: rows,
