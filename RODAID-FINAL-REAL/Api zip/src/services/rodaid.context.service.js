...
exports.buildContextoRico = buildContextoRico;
...
// ══════════════════════════════════════════════════════════
async function buildContextoRico(usuarioId, tipoConsulta) {
...
function renderContexto(d) {
    ...
    for (const b of d.bicicletas) {
        ...
        if (b.numero_cit) {
            ...
            if (!b.tasa_pagada)
                L.push(`         ⚠ Tasa MxM pendiente ($3.000 ARS) → acción: POST /cit/pago`);
            if (!b.nft_token_id)
                L.push(`         ⚠ NFT BFA pendiente de mint → acción: POST /bfa/mint/:id`);
        }
        else {
    ...
    if ((mp?.activas ?? 0) > 0 || (mp?.vendidas ?? 0) > 0) {
        ...
        L.push(`MARKETPLACE:`);
        if (mp.activas > 0)
            L.push(`  · ${mp.activas} publicaciones activas`);
        if (mp.pausadas > 0)
            L.push(`  · ${mp.pausadas} pausadas`);
        if (mp.vendidas > 0)
            L.push(`  · ${mp.vendidas} bicicletas vendidas ($${mp.cobrado_ars.toLocaleString('es-AR')} ARS cobrados)`);
        if (mp.total_vistas > 0)
            L.push(`  · ${mp.total_vistas} vistas acumuladas`);
    }
    // ── Aliado ────────────────────────────────────────────
    if (d.aliado) {
        const al = d.aliado;
    ...
    if (d.tipoConsulta && FOCOS[d.tipoConsulta])
        L.push(FOCOS[d.tipoConsulta]);
    return L.join('\n');
}
