...
exports.calcularScoreAsegurabilidad = calcularScoreAsegurabilidad;
exports.emitirCertificadoAsegurabilidad = emitirCertificadoAsegurabilidad;
const database_1 = require("../config/database");
const crypto_1 = __importDefault(require("crypto"));
// ══════════════════════════════════════════════════════════
// CALCULAR SCORE
// ══════════════════════════════════════════════════════════
function calcularScoreAsegurabilidad(puntos) {
    // Pesos por categoría
    ...
    const total = Math.round(identidad * 20 +
        estructura * 25 +
        seguridad * 25 +
        transmision * 15 +
        rodadura * 15);
    const nivel = total >= 90 ? 'EXCELENTE' :
        total >= 75 ? 'BUENO' :
            total >= 60 ? 'REGULAR' : 'INSUFICIENTE';
    const asegurable = total >= 60;
    const recomendaciones = [];
    if (!puntos.serial_cuadro)
        recomendaciones.push('Verificar y gravar el número de serie del cuadro');
    if (!puntos.freno_delant || !puntos.freno_trasero)
        recomendaciones.push('Revisión urgente del sistema de frenos');
    if (!puntos.cadena)
        recomendaciones.push('Lubricar o reemplazar la cadena');
    if (!puntos.rueda_delantera || !puntos.rueda_trasera)
        recomendaciones.push('Ajuste de ruedas y radios');
    if (!puntos.luces)
        recomendaciones.push('Instalar luces delanteras y traseras (obligatorio Ley 9556)');
    if (recomendaciones.length === 0)
        recomendaciones.push('Bicicleta en excelentes condiciones técnicas');
...
// EMITIR CERTIFICADO
// ══════════════════════════════════════════════════════════
async function emitirCertificadoAsegurabilidad(opts) {
    // Traer datos del CIT
    const cit = await (0, database_1.queryOne)(`SELECT c.*, b.numero_serie, b.marca, b.modelo,
            f.payload_json
     FROM cits c
     JOIN bicicletas b ON b.id = c.bicicleta_id
     LEFT JOIN firmas_payload_cit f ON f.cit_id = c.id::text
     WHERE c.id = $1::uuid`, [opts.citId]);
    if (!cit)
        return null;
    ...
    await (0, database_1.query)(`INSERT INTO certificados_asegurabilidad
