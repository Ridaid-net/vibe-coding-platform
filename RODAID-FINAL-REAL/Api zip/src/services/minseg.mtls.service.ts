...
export interface ConvenioEstado {
  ...
  fase:            FaseConvenio
  version:         string
  contactoMinSeg:  string | null
  emailMinSeg:     string | null
  expedienteNro:   string | null
  iniciadoEn:      string
  csrEnviadoEn:    string | null
  certEmitidoEn:   string | null
  sandboxDesde:    string | null
  produccionDesde: string | null
  venceEn:         string | null
  notas:           string | null
  // mTLS status
  modoActual:      'STUB' | 'SANDBOX' | 'LIVE'
  mtlsActivo:      boolean
  certRodaidCN:    string | null
  certVenceEn:     string | null
}

export interface CSRParams {
  cn:       string   // Common Name: "rodaid.net"
  org:      string   // Organization: "RODAID SAS"
  ou:       string   // Org Unit: "Certificacion Bicicletas"
  country:  string   // "AR"
  state:    string   // "Mendoza"
  locality: string   // "San Martin"
  email:    string   // "infra@rodaid.net"
  keyBits:  2048 | 4096
  validDays:number   // 365 | 730
}

export interface CSRResult {
  csrPEM:         string   // el CSR a enviar a MinSeg
  fingerprint:    string   // SHA-256 del CSR para verificación
  subjectDN:      string   // /C=AR/ST=Mendoza/.../CN=rodaid.net
  keyBits:        number
  generadoEn:     string
  instrucciones:  string   // pasos para entregar a MinSeg
}
...
function getModo(): 'STUB' | 'SANDBOX' | 'LIVE' {
  if (env.MINSEG_CERT_PEM && env.MINSEG_KEY_PEM && env.MINSEG_CA_PEM) {
    return env.MINSEG_SANDBOX === 'true' ? 'SANDBOX' : 'LIVE'
  }
  return 'STUB'
}
...
export async function getConvenioEstado(): Promise<ConvenioEstado | null> {
...
export async function generarCSR(params: CSRParams): Promise<CSRResult> {
  const modo = getModo()
  const subjectDN = [
    `/C=${params.country}`,
    `/ST=${params.state}`,
  ...
  if (modo === 'STUB') {
    ...
    try {
      ...
      fingerprint = crypto.createHash('sha256').update(csrPEM).digest('hex')
        .match(/.{2}/g)!.join(':').toUpperCase()

      log.bfa.info({ cn: params.cn, keyBits: params.keyBits }, '✓ CSR generado con clave real RSA')
    } catch {
  ...
  const conv = await queryOne<{ id: string }>(`SELECT id FROM minseg_convenio ORDER BY creado_en DESC LIMIT 1`, [])
...
export async function registrarCertificadoRecibido(opts: {
  certPEM:     string
  tipo:        'CERT_RODAID' | 'CERT_MINSEG_CA' | 'CERT_MINSEG_SERVER'
  notas?:      string
}): Promise<{ ok: boolean; fingerprint: string; venceEn: string | null }> {
...
export async function activarSandbox(): Promise<{ ok: boolean; mensaje: string }> {
  const conv = await queryOne<{ id: string; fase: string }>(`
    SELECT id, fase FROM minseg_convenio ORDER BY creado_en DESC LIMIT 1
  `, [])

  if (!conv) return { ok: false, mensaje: 'Sin convenio activo' }
  ...
  log.bfa.info({ convenioId: conv.id }, '✓ MinSeg Sandbox activado')
  return {
    ok: true,
...
export async function healthCheck(): Promise<HealthCheckResult> {
  ...
  try {
    ...
    const agent = new https.Agent({
      cert: env.MINSEG_CERT_PEM,
      key:  env.MINSEG_KEY_PEM,
      ca:   env.MINSEG_CA_PEM,
      rejectUnauthorized: modo === 'LIVE',
    })

    const baseUrl = modo === 'SANDBOX'
      ? 'https://sandbox.seguridadmendoza.gob.ar'
      : env.MINSEG_API_URL ?? 'https://api.seguridadmendoza.gob.ar'
    ...
    const res = await fetch(`${baseUrl}/api/v1/rodaid/ping`, {
      ...
      headers: {
        'X-RODAID-KEY': env.MINSEG_KEY_ID ?? 'rodaid-prod-001',
        'User-Agent':   'RODAID-MinSeg-Client/1.0',
      },
    })

    const latencia = Date.now() - t0
    const tlsSocket = (res as any).socket
    ...
    `, ['ping', latencia, (err as Error).message, modo])
...
export async function avanzarFase(
  fase: FaseConvenio,
  opts: { expedienteNro?: string; notas?: string; emailMinSeg?: string } = {}
): Promise<{ ok: boolean; faseAnterior: FaseConvenio; faseNueva: FaseConvenio }> {
  const conv = await queryOne<{ id: string; fase: FaseConvenio }>(
    `SELECT id, fase FROM minseg_convenio ORDER BY creado_en DESC LIMIT 1`, []
  )
  if (!conv) return { ok: false, faseAnterior: 'INICIADO', faseNueva: fase }

  const updates: Record<string, unknown> = {
    fase,
    actualizado_en: new Date(),
  }
  if (opts.expedienteNro) updates.expediente_nro = opts.expedienteNro
  if (opts.notas)         updates.notas         = opts.notas
  if (opts.emailMinSeg)   updates.email_minseg  = opts.emailMinSeg

  // Timestamps automáticos según la fase
  if (fase === 'SANDBOX_ACTIVO')  updates.sandbox_desde    = new Date()
  if (fase === 'PRODUCCION')      updates.produccion_desde = new Date()
  ...
  await query(
    `UPDATE minseg_convenio SET ${setClauses} WHERE id=$1::uuid`,
    [conv.id, ...Object.values(updates)]
  )
...
export async function getResumenOperacional() {
  const [conv, intercambios, denuncias, health] = await Promise.all([
    getConvenioEstado(),
    queryOne<{ total: number; ok: number; error: number }>(`
    ...
    `, []),
    queryOne<{ total: number; notificadas: number }>(`
      SELECT COUNT(*)::int AS total,
             COUNT(*) FILTER (WHERE minseg_notificado=TRUE)::int AS notificadas
      FROM denuncias
    `, []),
    queryOne<{ avg_ms: number; ok_pct: number }>(`
