...
export type EstadoItem = 'PENDIENTE' | 'EN_CURSO' | 'COMPLETADO' | 'BLOQUEADO'

export interface ChecklistItem {
  id:          string
  titulo:      string
  descripcion: string
  responsable: 'RODAID' | 'MINSEG' | 'AMBOS'
  estado:      EstadoItem
  completadoEn?: string | null
  notas?:      string
  requerido:   boolean     // si false = deseable pero no bloqueante
}
...
export interface FaseChecklist {
  ...
  titulo:         string
  descripcion:    string
  faseRequerida:  FaseConvenio | null    // fase que debe completarse antes
  items:          ChecklistItem[]
  completada:     boolean
  bloqueante:     boolean                // si esta fase bloquea la siguiente
}

export interface ConvenioChecklist {
  convenioId:    string
  faseActual:    FaseConvenio
  expedienteNro: string | null
  fases:         FaseChecklist[]
  siguientePaso: string
  porcentaje:    number   // % global de avance
}
...
function buildChecklist(
  ...
  const completada = (fase: FaseConvenio) => {
    const orden: FaseConvenio[] = [
  ...
  const enCurso = (fase: FaseConvenio) => f === fase
...
function calcularAvance(fases: FaseChecklist[]): number {
...
function siguientePasoStr(fases: FaseChecklist[], faseActual: FaseConvenio): string {
  const faseActualChecklist = fases.find(f => f.fase === faseActual)
  if (!faseActualChecklist) return 'Ver estado del convenio'

  const pendientePropio = faseActualChecklist.items.find(
    i => i.requerido && (i.estado === 'PENDIENTE' || i.estado === 'EN_CURSO') && i.responsable !== 'MINSEG'
  )
  if (pendientePropio) return `RODAID: ${pendientePropio.titulo}`

  const pendienteMinSeg = faseActualChecklist.items.find(
    i => i.requerido && (i.estado === 'PENDIENTE' || i.estado === 'EN_CURSO')
  )
  if (pendienteMinSeg) return `Esperando a MinSeg: ${pendienteMinSeg.titulo}`

  const siguienteFase = fases.find(f => f.faseRequerida === faseActual)
  if (siguienteFase) return `Iniciar fase: ${siguienteFase.titulo}`
...
export async function getConvenioChecklist(): Promise<ConvenioChecklist | null> {
...
const KEY_CACHE = new Map<string, { permisos: string[]; expira_en: Date }>()

export async function validarApiKeyMinSeg(
  rawKey: string,
  endpoint: string
): Promise<{ ok: boolean; keyId?: string; permisos?: string[]; motivo?: string }> {
  ...
  if (cached) {
    if (cached.expira_en < new Date()) KEY_CACHE.delete(keyHash)
    else {
  ...
  const key = await queryOne<{
    key_id: string; activa: boolean; permisos: string[]; expira_en: Date
  }>(
    `SELECT key_id, activa, permisos, expira_en
  ...
  if (!key.activa)          return { ok: false, motivo: 'API Key inactiva'  }
  if (key.expira_en < new Date()) return { ok: false, motivo: 'API Key vencida' }
  ...
  await query(
    `UPDATE minseg_api_keys SET ultimo_uso_en=NOW() WHERE key_id=$1`,
    [key.key_id]
  ).catch(() => {})
...
export async function registrarApiKeyMinSeg(opts: {
  descripcion: string
  permisos:    string[]
  expirarEn?:  Date
}): Promise<{ keyId: string; rawKey: string; keyHash: string }> {
  ...
  log.bfa.info({ keyId, permisos: opts.permisos }, '✓ API Key MinSeg registrada')
...
export async function simularClienteMinSeg(opts: {
  endpoint:    'consulta-serial' | 'alerta-robo' | 'recuperacion' | 'health'
  serial?:     string
  apiKey?:     string
  baseUrl?:    string
}): Promise<{ ok: boolean; statusCode: number; data: unknown; latenciaMs: number }> {
  const t0     = Date.now()
  ...
  const serialHash = opts.serial
    ? crypto.createHash('sha256').update(opts.serial.toUpperCase()).digest('hex')
    : crypto.createHash('sha256').update('SN-GIANT-002-2021').digest('hex')
  ...
  try {
    ...
    return { ok: false, statusCode: 0, data: { error: (err as Error).message }, latenciaMs: Date.now() - t0 }
  }
}
