...
export const FONT_FAMILY = 'BiancoSport'
...
const RAJDHANI_BASE = path.resolve(
  process.cwd(),
  'node_modules/@fontsource/rajdhani/files'
)

function rajdhaniFile(weight: number, ext: 'woff2' | 'woff'): string {
  return path.join(RAJDHANI_BASE, `rajdhani-latin-${weight}-normal.${ext}`)
}
...
interface FontSet {
  ...
  medium?:   Buffer   // 500
  semibold:  Buffer   // 600
  source:    'bianco-sport' | 'rajdhani' | 'system'
}

let _fontSetCache: FontSet | null = null
...
async function cargarFuentes(): Promise<FontSet> {
  if (_fontSetCache) return _fontSetCache
  ...
  if (env.RODAID_FONT_BS_REGULAR_B64 && env.RODAID_FONT_BS_SEMIBOLD_B64) {
    try {
      const regular  = Buffer.from(env.RODAID_FONT_BS_REGULAR_B64,  'base64')
      const semibold = Buffer.from(env.RODAID_FONT_BS_SEMIBOLD_B64, 'base64')
      if (regular.length > 1000 && semibold.length > 1000) {
        log.font.info({ source: 'env', bytes: regular.length + semibold.length },
          '🔤 Bianco Sport cargada desde env')
        _fontSetCache = { regular, semibold, source: 'bianco-sport' }
        return _fontSetCache
      }
    } catch (err) {
      log.font.warn({ err: (err as Error).message }, 'Error cargando Bianco Sport desde env')
    }
  }

  // 2. Bianco Sport desde directorio configurado (ej: ./fonts/BiancoSport-Regular.ttf)
  const fontDir = env.RODAID_FONT_PATH
  if (fontDir) {
    const candidates = [
    ...
    for (const [reg, bold] of candidates) {
      const regPath  = path.join(fontDir, reg)
      const boldPath = path.join(fontDir, bold)
      if (fs.existsSync(regPath) && fs.existsSync(boldPath)) {
        const regular  = fs.readFileSync(regPath)
        const semibold = fs.readFileSync(boldPath)
        log.font.info({ source: fontDir, regular: reg }, '🔤 Bianco Sport cargada desde disco')
        _fontSetCache = { regular, semibold, source: 'bianco-sport' }
        return _fontSetCache
      }
    }
  ...
  log.font.info('🔤 Bianco Sport no disponible → usando Rajdhani (fallback fiel)')
  return await cargarRajdhani()
}

async function cargarRajdhani(): Promise<FontSet> {
  const pesos = [
    { peso: 300, clave: 'light'    as const },
    { peso: 400, clave: 'regular'  as const },
    { peso: 500, clave: 'medium'   as const },
    { peso: 600, clave: 'semibold' as const },
  ]

  const buffers: Partial<Record<keyof Omit<FontSet, 'source'>, Buffer>> = {}
  let source: FontSet['source'] = 'rajdhani'
  ...
  for (const { peso, clave } of pesos) {
    ...
    try {
      if (fs.existsSync(woff2)) {
        buffers[clave] = fs.readFileSync(woff2)
      } else if (fs.existsSync(woff)) {
        buffers[clave] = fs.readFileSync(woff)
      }
  ...
  if (!buffers.regular) {
    ...
    log.font.warn('Rajdhani no disponible → usando DejaVu Sans (sistema)')
    buffers.regular  = fs.readFileSync(dejaVuPath)
    buffers.semibold = fs.readFileSync(dejaVuBold)
    source = 'system'
  }

  const result: FontSet = {
    light:    buffers.light,
...
export function invalidarCacheFuentes(): void {
  _fontSetCache = null
  log.font.info('🔤 Caché de fuentes invalidado')
}

/** Buffers de fuente para PDFKit */
export async function getFontBuffers(): Promise<{
  regular: Buffer; semibold: Buffer; light?: Buffer; medium?: Buffer; source: string
}> {
...
export async function getFontFaceCSS(): Promise<string> {
  ...
  const toB64 = (buf: Buffer, isWOFF2 = true) =>
    `data:font/${isWOFF2 ? 'woff2' : 'ttf'};base64,${buf.toString('base64')}`

  const weights: Array<[number, Buffer | undefined, string]> = [
...
/* ${FONT_FAMILY} ${label} (${weight}) — embedded */
@font-face {
...
export async function getFontInfo(): Promise<{
  family:  string
  source:  string
  ...
  if (fonts.light)    weights['300-light']    = fonts.light.length
  if (fonts.regular)  weights['400-regular']  = fonts.regular.length
  if (fonts.medium)   weights['500-medium']   = fonts.medium.length
  if (fonts.semibold) weights['600-semibold'] = fonts.semibold.length
