...
exports.cotizarSeguro = cotizarSeguro;
exports.contratarSeguro = contratarSeguro;
exports.getMisPolizas = getMisPolizas;
...
// MOTOR DE COTIZACIÓN
// ══════════════════════════════════════════════════════════
async function cotizarSeguro(req) {
  ...
  `, [req.citId]);
    if (!cit)
        throw new Error('CIT no encontrado o no activo');
    if (!(cit.asegurable ?? true))
        throw new Error('Bicicleta no asegurable — CIT con score insuficiente');
    // ── 2. Verificar elegibilidad por descuentos ─────────────
    const nftOk = !!cit.nft_token_id && cit.hash_sha256?.length === 64;
    const mxmLvl = await (0, database_1.queryOne)(`
  ...
  `, [req.usuarioId]).then(r => r?.nivel ?? 0).catch(() => 0);
    ...
    const aseguradoras = await (0, database_1.query)(`
  ...
  `, [req.tipoCobVert]);
    ...
    for (const row of aseguradoras) {
        ...
        const primaBase = row[tipoKey] ?? row.prima_base_urbana;
        ...
        const coberturas = [];
        if (row.cobertura_robo)
            coberturas.push('Robo total');
        if (row.cobertura_daños)
            coberturas.push('Daños accidentales');
        if (row.cobertura_resp_civil)
            coberturas.push('Responsabilidad civil');
        if (row.cobertura_asistencia)
            coberturas.push('Asistencia mecánica 24h');
        cotizaciones.push({
    ...
    if (cotizaciones.length > 0)
        cotizaciones[0].recomendado = true;
    // ── 5. Persistir cotización ───────────────────────────────
    const numero = `COT-${new Date().getFullYear()}-${String(Math.floor(Math.random() * 90000) + 10000)}`;
    const certId = await (0, database_1.queryOne)(`SELECT id::text FROM certificados_asegurabilidad WHERE cit_id=$1::uuid ORDER BY creado_en DESC LIMIT 1`, [req.citId]).catch(() => null);
...
// ══════════════════════════════════════════════════════════
async function contratarSeguro(req) {
    // Validar cotización
    const cot = await (0, database_1.queryOne)(`
  ...
  `, [req.cotizacionId]);
    if (!cot)
        throw new Error('Cotización no encontrada, ya utilizada o expirada');
    // Encontrar la cotización específica en el JSON
    const cotizaciones = typeof cot.cotizaciones_json === 'string' ? JSON.parse(cot.cotizaciones_json) : (cot.cotizaciones_json ?? []);
    const linea = cotizaciones.find(c => c.aseguradoraCodigo === req.aseguradoraCodigo);
    if (!linea)
        throw new Error(`Cotización para ${req.aseguradoraCodigo} no encontrada`);
    // IDs de aseguradora y producto
    const aseg = await (0, database_1.queryOne)(`SELECT id::text FROM seguros_aseguradoras WHERE codigo=$1`, [req.aseguradoraCodigo]);
    const prod = await (0, database_1.queryOne)(`SELECT id::text FROM seguros_productos WHERE codigo=$1`, [linea.productoCodigo]);
    if (!aseg || !prod)
        throw new Error('Aseguradora o producto no encontrado');
    ...
    const hashPoliza = crypto_1.default.createHash('sha256')
        .update(JSON.stringify({ numeroPoliza, linea, ts: Date.now() }))
        .digest('hex');
    const pol = await (0, database_1.queryOne)(`
  ...
  `, [
    ...
    await (0, database_1.query)(`UPDATE seguros_cotizaciones SET estado='CONTRATADA' WHERE numero=$1`, [req.cotizacionId]);
    return {
        polizaId: pol.id,
        numeroPoliza,
...
// ══════════════════════════════════════════════════════════
async function getMisPolizas(usuarioId) {
