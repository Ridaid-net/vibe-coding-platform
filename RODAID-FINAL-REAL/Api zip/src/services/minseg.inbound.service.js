...
exports.verificarAutenticidadMinSeg = verificarAutenticidadMinSeg;
exports.consultarSerialInbound = consultarSerialInbound;
exports.recibirAlertaRobo = recibirAlertaRobo;
exports.getProtocoloEspecificacion = getProtocoloEspecificacion;
exports.getHealthResponse = getHealthResponse;
exports.getResumenInbound = getResumenInbound;
...
// ══════════════════════════════════════════════════════════
const NONCES_USADOS = new Set(); // En prod → Redis con TTL 5min
function verificarAutenticidadMinSeg(opts) {
    ...
    if (opts.modo !== 'STUB' && opts.firma) {
        ...
        const expected = crypto_1.default
            ...
            .digest('hex');
        if (!crypto_1.default.timingSafeEqual(Buffer.from(opts.firma), Buffer.from(expected))) {
...
// ══════════════════════════════════════════════════════════
async function consultarSerialInbound(input, meta) {
    const t0 = Date.now();
  ...
  `, [
    ...
    ]).catch(err => logger_1.log.minseg?.warn({ err: err.message }, 'No se pudo registrar consulta entrante'));
...
// ══════════════════════════════════════════════════════════
async function recibirAlertaRobo(input, meta) {
    // Resolver el serial desde el hash
    const bici = await (0, database_1.queryOne)(`SELECT b.id::text, b.numero_serie, b.propietario_id::text
     FROM bicicletas b
     WHERE encode(digest(UPPER(b.numero_serie), 'sha256'), 'hex') = $1
        OR encode(digest(b.numero_serie, 'sha256'), 'hex') = $1`, [input.serialHash]);
    const alertaId = crypto_1.default.randomUUID();
  ...
  `, [
    ...
    if (bici?.propietario_id) {
        ...
        const cit = await (0, database_1.queryOne)(`SELECT c.id::text, c.numero_cit, b.marca, b.modelo
       FROM cits c JOIN bicicletas b ON b.id=c.bicicleta_id
       WHERE c.bicicleta_id=$1::uuid AND c.estado='ACTIVO' LIMIT 1`, [bici.id]);
        if (triggerAlertaRobo && cit) {
            ...
            }).catch(err => logger_1.log.minseg?.warn({ err: err.message }, 'triggerAlertaRobo falló'));
            notifEnviada = true;
        }
        // Marcar alerta como procesada
        await (0, database_1.query)(`UPDATE minseg_alertas_entrantes SET procesado=TRUE, procesado_en=NOW(),
       notif_enviada=$2 WHERE id=$1::uuid`, [alertaId, notifEnviada]);
    }
    logger_1.log.minseg?.info({
...
// ══════════════════════════════════════════════════════════
function getProtocoloEspecificacion() {
...
// ══════════════════════════════════════════════════════════
async function getResumenInbound() {
