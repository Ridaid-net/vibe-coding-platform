...
exports.resolverLabel = resolverLabel;
exports.resolverEstado = resolverEstado;
exports.resolverEstadoPorSerial = resolverEstadoPorSerial;
exports.estadoBadge = estadoBadge;
...
function resolverEstado(input) {
    ...
    if (estadoBase === 'EN_VALIDACION' && input.pipelineInicio) {
        diasEnEstado = Math.floor((ahora.getTime() - input.pipelineInicio.getTime()) / (1000 * 60 * 60 * 24));
        pipelineVenceEn = new Date(input.pipelineInicio.getTime() + 72 * 60 * 60 * 1000);
        const horasRestantes = Math.max(0, Math.floor((pipelineVenceEn.getTime() - ahora.getTime()) / (1000 * 60 * 60)));
        fuentesPrincipales.push(`${horasRestantes} horas restantes de validación`);
    }
    if (input.fechaEmision) {
        diasEnEstado = Math.floor((ahora.getTime() - input.fechaEmision.getTime()) / (1000 * 60 * 60 * 24));
    }
...
// ══════════════════════════════════════════════════════════
async function resolverEstadoPorSerial(serial) {
    const row = await (0, database_1.queryOne)(`SELECT c.id AS "citId", c.estado::text AS "dbEstado",
    ...
    const [denuncias, bfaIndex] = await Promise.all([
        (0, database_1.query)(`SELECT estado, creado_en FROM denuncias_robo WHERE numero_serie=$1 AND estado='ACTIVA' LIMIT 1`, [serial]),
        (async () => {
            try {
