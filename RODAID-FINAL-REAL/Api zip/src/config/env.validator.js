...
exports.runEnvValidation = runEnvValidation;
...
// HELPERS
// ══════════════════════════════════════════════════════════
function check(nombre, severidad, ok, ok_msg, fail_msg, detalle) {
...
async function validateProductionEnvironment() {
    ...
    for (const { var: v, desc } of VARS_CRITICAS) {
        const ok = hasVar(v);
        checks.push(check(`VAR:${v}`, 'CRITICAL', ok, `${v} presente`, `Falta variable crítica: ${v} — ${desc}`));
    }
    // JWT_SECRET mínimo 32 chars
    const jwtSecret = process.env.JWT_SECRET ?? '';
    checks.push(check('JWT_SECRET:min32', 'CRITICAL', jwtSecret.length >= 32, 'JWT_SECRET tiene longitud adecuada (≥32 chars)', `JWT_SECRET demasiado corto: ${jwtSecret.length} chars (mínimo 32)`));
    ...
    for (const { var: v, desc, modulo } of VARS_PRODUCCION) {
        ...
        const sev = isProd ? 'CRITICAL' : 'WARN';
        checks.push(check(`VAR:${v}`, sev, ok, `${v} presente [${modulo}]`, `${isProd ? '❌ Falta' : '⚠ Falta (dev-ok)'}: ${v} — ${desc} [módulo: ${modulo}]`));
    }
    // ── 3. Variables avanzadas (warnings) ────────────────────
    for (const { var: v, desc } of VARS_AVANZADAS) {
        checks.push(check(`VAR:${v}`, 'WARN', hasVar(v), `${v} configurada`, `⚠ Opcional no configurada: ${v} — ${desc}`));
    }
    ...
    if (hasVar('DATABASE_URL')) {
        try {
            ...
            const tablas = await pgClient.query(`SELECT COUNT(*)::text AS count FROM information_schema.tables WHERE table_schema = 'public'`);
            ...
            const tablasList = await pgClient.query(`SELECT tablename FROM pg_tables WHERE schemaname='public'`);
            const existentes = new Set(tablasList.rows.map(r => r.tablename));
            const faltantes = tablasReq.filter(t => !existentes.has(t));
            await pgClient.end();
            checks.push(check('DB:connection', 'CRITICAL', nTablas > 0, `PostgreSQL OK — ${nTablas} tablas en schema public`, `PostgreSQL conecta pero la DB está vacía (faltan migraciones) — ${nTablas} tablas`, nTablas < 68 ? `Se esperan ≥68 tablas, se encontraron ${nTablas}` : undefined));
            checks.push(check('DB:tablas-criticas', 'CRITICAL', faltantes.length === 0, `Tablas críticas presentes: ${tablasReq.join(', ')}`, `Faltan tablas críticas: ${faltantes.join(', ')}`));
        }
        catch (err) {
            checks.push(check('DB:connection', 'CRITICAL', false, '', `Error al conectar a DATABASE_URL: ${err.message}`));
        }
    }
    // ── 5. Conexión Redis ────────────────────────────────────
    if (hasVar('REDIS_URL')) {
        try {
            const rc = (0, redis_1.createClient)({ url: process.env.REDIS_URL });
            await rc.connect();
            const pong = await rc.ping();
            await rc.disconnect();
            checks.push(check('Redis:ping', 'CRITICAL', pong === 'PONG', 'Redis OK — PING → PONG', `Redis responde pero PING devolvió: ${pong}`));
        }
        catch (err) {
            checks.push(check('Redis:ping', 'CRITICAL', false, '', `Error al conectar a REDIS_URL: ${err.message}`));
        }
    }
    ...
    if (bfaRpc) {
        try {
            const resp = await axios_1.default.post(bfaRpc, { jsonrpc: '2.0', method: 'eth_blockNumber', params: [], id: 1 }, { timeout: 5_000 });
            const blockHex = resp.data?.result;
            const block = blockHex ? parseInt(blockHex, 16) : null;
            checks.push(check('BFA:eth_blockNumber', 'WARN', resp.status === 200 && !!blockHex, `BFA nodo OK — bloque actual: ${block?.toLocaleString('es-AR')}`, 'BFA_RPC_URL respondió pero el formato JSON-RPC no es válido.', !blockHex ? `Respuesta: ${JSON.stringify(resp.data).slice(0, 80)}` : undefined));
        }
        catch (err) {
            // En producción con BFA: CRITICAL. Sin BFA configurado: solo INFO.
            const sev = isProd && hasVar('BFA_CONTRACT_ADDRESS') ? 'CRITICAL' : 'WARN';
            checks.push(check('BFA:eth_blockNumber', sev, false, '', `No se pudo conectar al nodo BFA (BFA_RPC_URL): ${err.message}`, 'Sin nodo BFA el modo es STUB — los NFTs no se acuñarán en mainnet.'));
        }
    }
    else {
        checks.push(check('BFA:configuracion', 'WARN', false, '', 'BFA_RPC_URL no configurada — modo STUB activo (NFTs simulados)', 'Para producción solicitar acceso al nodo BFA (ONTI).'));
    }
    // ── 7. MxM OAuth — reachability ─────────────────────────
    const mxmAuth = process.env.MXM_AUTH_URL;
    if (mxmAuth) {
        try {
            await axios_1.default.get(mxmAuth.replace('/oauth/authorize', '/.well-known/openid-configuration'), { timeout: 4_000 });
            checks.push(check('MxM:reachability', 'WARN', true, 'MxM endpoint alcanzable', ''));
        }
        catch (err) {
            const sev = isProd ? 'WARN' : 'INFO';
            checks.push(check('MxM:reachability', sev, false, '', `MxM no alcanzable desde este entorno: ${err.message}`, 'El servidor puede arrancar — MxM tiene circuit-breaker integrado.'));
        }
    }
    // ── 8. Anthropic API key — formato ──────────────────────
    const anthropicKey = process.env.ANTHROPIC_API_KEY ?? '';
    const anthropicOk = anthropicKey.startsWith('sk-ant-') && anthropicKey.length > 40;
    checks.push(check('Anthropic:api-key', isProd ? 'CRITICAL' : 'WARN', anthropicOk, 'ANTHROPIC_API_KEY con formato válido (sk-ant-...)', anthropicKey
    ...
    checks.push(check('MinSeg:credenciales', isProd ? 'CRITICAL' : 'WARN', minsegOk, 'Credenciales MinSeg configuradas (API_KEY + API_URL)', 'Faltan credenciales MinSeg — el cross-reference de denuncia operará en modo STUB'));
    // ── 10. S3 / Almacenamiento ──────────────────────────────
    const s3Ok = hasVar('S3_BUCKET_FOTOS') && hasVar('AWS_ACCESS_KEY_ID') && hasVar('AWS_SECRET_ACCESS_KEY');
    checks.push(check('S3:credenciales', 'WARN', s3Ok, 'S3 configurado — fotos de inspecciones operativas', 'S3 no configurado — las fotos de inspección se almacenarán localmente (solo dev)'));
    // ── 11. CDN URL — formato ─────────────────────────────────
    const cdnUrl = process.env.CDN_URL ?? '';
    checks.push(check('CDN:url', 'WARN', cdnUrl.startsWith('https://'), `CDN_URL configurada: ${cdnUrl}`, cdnUrl ? `CDN_URL no usa HTTPS: ${cdnUrl}` : 'CDN_URL no configurada — assets servidos desde /api/cdn/'));
    // ── 12. NODE_ENV explícito ────────────────────────────────
    checks.push(check('NODE_ENV', 'INFO', ['development', 'production', 'test'].includes(entorno), `NODE_ENV = ${entorno}`, `NODE_ENV inválido: "${entorno}"`));
    // ─────────────────────────────────────────────────────────
...
async function runEnvValidation() {
    ...
    logger_1.logger.info('─── RODAID · Auditoría de entorno ───────────────────');
    logger_1.logger.info(`Entorno: ${process.env.NODE_ENV ?? 'development (default)'}`);
    const result = await validateProductionEnvironment();
    // Imprimir todos los checks
    for (const c of result.checks) {
        if (c.ok) {
            if (c.severidad === 'CRITICAL')
                logger_1.logger.info(`  ✅ ${c.nombre}: ${c.mensaje}`);
            // WARN/INFO exitosos → silencio para no saturar el log
        }
        else {
            if (c.severidad === 'CRITICAL')
                logger_1.logger.error({ check: c.nombre, detalle: c.detalle }, `  ❌ CRÍTICO: ${c.mensaje}`);
            else if (c.severidad === 'WARN')
                logger_1.logger.warn({ check: c.nombre, detalle: c.detalle }, `  ⚠  WARN:   ${c.mensaje}`);
            else
                logger_1.logger.info({ check: c.nombre }, `  ℹ  INFO:   ${c.mensaje}`);
        }
    }
    logger_1.logger.info('─────────────────────────────────────────────────────');
    logger_1.logger.info(`Resultado: ${result.criticos.length} críticos · ${result.warnings.length} warnings · ${result.duracionMs}ms`);
    // Warnings: loggear resumen
    if (result.warnings.length > 0) {
        logger_1.logger.warn(`⚠  ${result.warnings.length} módulos en modo STUB/limitado:`);
        result.warnings.forEach(w => logger_1.logger.warn(`   · ${w.nombre.replace('VAR:', '')}`));
    }
    if (!result.success) {
        logger_1.logger.error('─────────────────────────────────────────────────────');
        logger_1.logger.error(`❌ ERROR CRÍTICO: ${result.criticos.length} checks fallaron`);
        result.criticos.forEach(c => logger_1.logger.error(`   · ${c.mensaje}`));
        logger_1.logger.error('─── Servidor detenido por seguridad ─────────────────');
        if (isProd) {
            // Solo crash en producción — en dev el desarrollador puede querer arrancar igual
            process.exit(1);
        }
        else {
            logger_1.logger.warn('(desarrollo) Continuando a pesar de los errores críticos…');
        }
    }
    else {
        logger_1.logger.info('✅ Entorno de producción validado con éxito.');
    }
}
