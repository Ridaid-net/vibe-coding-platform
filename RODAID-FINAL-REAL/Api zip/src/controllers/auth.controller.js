...
exports.login = (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    if (!usuario?.password_hash) {
        ...
        await (0, auth_lockout_service_1.logAuthEvento)({ evento: 'LOGIN_EMAIL_INEXISTENTE', ip: getSessionCtx(req).ipAddress ?? '0.0.0.0', userAgent: req.headers['user-agent'] ?? '', ok: false }).catch(() => { });
    ...
    const ipBloqueada = await (0, auth_lockout_service_1.verificarBloqueoIP)(ipAddr);
    if (ipBloqueada) {
        throw new errorHandler_1.AppError('Demasiados intentos desde tu IP. Intentá en 6 horas.', 429, 'IP_BLOCKED');
    }
    // Verificar bloqueo de cuenta
    const bloqueo = await (0, auth_lockout_service_1.verificarBloqueoCuenta)(data.email);
    if (bloqueo.bloqueado) {
        const mins = Math.ceil(bloqueo.segundosRestantes / 60);
        throw new errorHandler_1.AppError(`Cuenta bloqueada temporalmente por demasiados intentos fallidos. Intentá en ${mins} minuto${mins !== 1 ? 's' : ''}.`, 423, 'ACCOUNT_LOCKED');
    }
    const ok = await bcryptjs_1.default.compare(data.password, usuario.password_hash);
    if (!ok) {
        // Registrar fallo y potencialmente bloquear
        await (0, auth_lockout_service_1.registrarFalloLogin)({ email: data.email, ip: ipAddr, userAgent: req.headers['user-agent'] ?? '' });
        throw new errorHandler_1.AppError('Credenciales inválidas', 401, 'INVALID_CREDENTIALS');
    }
    ...
    await (0, auth_lockout_service_1.resetearContadorLogin)(usuario.id, ipAddr);
    // ── Audit log login exitoso ───────────────────────────────
    (0, auth_lockout_service_1.logAuthEvento)({ usuarioId: usuario.id, evento: 'LOGIN_EXITOSO', ip: ipAddr, userAgent: req.headers['user-agent'] ?? '', datos: { rol: usuario.rol } }).catch(() => { });
