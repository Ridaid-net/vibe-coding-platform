 ...
 */
export async function verificarFirmaCIT(opts: {
  citId?:    string
  numeroCIT?: string
  baseUrl?:  string
  ...
  if (!resp.ok) {
    if (resp.status === 404) return { valida: false, revocada: false, error: 'CIT sin firma registrada', fuente: 'servidor' }
    return { valida: false, revocada: false, error: `HTTP ${resp.status}`, fuente: 'servidor' }
  }
  const data = (await resp.json()) as { ok: boolean; data: any }
 ...
 */
export async function verificarFirmaOffline(opts: {
  firmaBase64url: string
  payloadJson:    string
  spkiBase64?:   string
  certPEM?:      string
}): Promise<{ valida: boolean; payload?: PayloadCIT; error?: string; fuente: 'offline' }> {
  try {
    ...
    let payload: PayloadCIT | undefined
    if (valida) {
      try { payload = JSON.parse(opts.payloadJson) } catch { /* ignorar */ }
    }

    return { valida, payload, fuente: 'offline' }

  } catch (err) {
    return { valida: false, fuente: 'offline', error: (err as Error).message }
  }
}
 ...
 */
export async function verificarFirmaConJWK(opts: {
  firmaBase64url: string
  payloadJson:    string
  jwk:            JsonWebKey
}): Promise<boolean> {
  ...
  return crypto.subtle.verify(
    ...
    pubKey,
    base64urlToArrayBuffer(opts.firmaBase64url),
    new TextEncoder().encode(opts.payloadJson)
  )
}
...
export async function verificarLoteCITs(citIds: string[], baseUrl?: string): Promise<
  Record<string, ResultadoVerificacion>
> {
  const resultados = await Promise.allSettled(
    citIds.map(citId => verificarFirmaCIT({ citId, baseUrl }))
  )
  const out: Record<string, ResultadoVerificacion> = {}
  citIds.forEach((id, i) => {
    const r = resultados[i]
    out[id] = r.status === 'fulfilled'
      ? r.value
      : { valida: false, revocada: false, error: (r.reason as Error).message, fuente: 'servidor' }
  })
  return out
}
...
let _clavePublicaCacheada: CryptoKey | null = null

/**
 * Obtiene la clave pública del servidor y la cachea en memoria.
 * Llamar una sola vez al iniciar la app.
 */
export async function cargarClavePublica(baseUrl = 'https://api.rodaid.com.ar/api/v1'): Promise<CryptoKey> {
  if (_clavePublicaCacheada) return _clavePublicaCacheada

  const resp = await fetch(`${baseUrl}/firma/clave-publica`)
  const data = (await resp.json()) as { ok: boolean; data: { spkiBase64: string } }

  const spkiBytes = base64ToArrayBuffer(data.data.spkiBase64)
  _clavePublicaCacheada = await crypto.subtle.importKey(
    'spki',
    spkiBytes,
    { name: 'RSA-PSS', hash: 'SHA-256' },
    false,
 ...
 */
export async function verificarConClavePreCargada(opts: {
  firmaBase64url: string
  payloadJson:    string
  pubKey:         CryptoKey
}): Promise<boolean> {
 ...
 */
function extractPublicKeyFromCertPEM(certPEM: string): string {
  ...
  if (!spki) throw new Error('No se pudo extraer la clave pública del certificado')
...
function extractSPKIFromCertDER(der: Uint8Array): Uint8Array | null {
  // ASN.1 DER parser para extraer SubjectPublicKeyInfo
  // Certificate SEQUENCE
  let pos = 0
  if (der[pos++] !== 0x30) return null  // SEQUENCE
  ...
  if (der[pos] === 0xa0) { pos++; const l = skipLength(der, pos); pos = l.pos + l.len }
...
function skipLength(der: Uint8Array, pos: number): { pos: number; len: number } {
  if (der[pos] < 0x80) return { pos: pos + 1, len: der[pos] }
  const numBytes = der[pos] & 0x7f; pos++
  let len = 0
  for (let i = 0; i < numBytes; i++) len = (len << 8) | der[pos++]
  return { pos, len }
}

function skipTag(der: Uint8Array, pos: number, expectedTag: number): number {
  if (der[pos] !== expectedTag) throw new Error(`Expected tag 0x${expectedTag.toString(16)} at pos ${pos}, got 0x${der[pos].toString(16)}`)
  pos++
