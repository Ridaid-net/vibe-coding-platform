...
exports.getHTMLParaPreview = getHTMLParaPreview;
exports.generarPDFPuppeteer = generarPDFPuppeteer;
exports.cargarCITParaPDF = cargarCITParaPDF;
...
class BrowserPool {
    ...
    async iniciar() {
        ...
        logger_1.log.pdf.info({ execPath: this.execPath }, '🚀 Iniciando Chrome headless');
        ...
        this.browser.on('disconnected', () => {
            logger_1.log.pdf.warn('Chrome desconectado — se reconectará en el próximo request');
            this.browser = null;
        });
        logger_1.log.pdf.info({ pid: this.browser.process()?.pid }, '✓ Chrome headless listo');
    }
    /** Obtener una página nueva (maneja concurrencia máxima) */
    async obtenerPagina() {
        if (this.concurrentes >= this.maxConcurrentes) {
            throw new Error(`Límite de ${this.maxConcurrentes} PDFs concurrentes alcanzado. ` +
    ...
    async cerrar() {
        if (this.browser) {
            ...
            this.browser = null;
            logger_1.log.pdf.info('Chrome headless cerrado');
        }
    }
...
async function buildHTML(d, qrDataURI, fontFaceCSS) {
...
async function getHTMLParaPreview(d) {
    let qrDataURI;
    try {
...
async function generarConPuppeteer(d) {
    const t0 = Date.now();
    const page = await exports.browserPool.obtenerPagina();
    try {
        ...
        try {
            ...
            logger_1.log.pdf.warn({ err: err.message }, 'QR generation failed — PDF sin QR');
        }
        ...
        try {
            ...
            logger_1.log.pdf.warn({ err: err.message }, 'Font CSS load failed — usando fuente del sistema');
        }
        ...
        logger_1.log.pdf.info({ numeroCIT: d.numeroCIT, bytes: pdf.length, ms }, '✓ PDF generado con Puppeteer');
        return Buffer.from(pdf);
    }
    finally {
        await exports.browserPool.liberarPagina(page);
    }
}
// ══════════════════════════════════════════════════════════
// FUNCIÓN PRINCIPAL — con fallback a PDFKit
// ══════════════════════════════════════════════════════════
async function generarPDFPuppeteer(d) {
    const t0 = Date.now();
    // Intentar Puppeteer primero
    if (exports.browserPool.disponible) {
        try {
            ...
            return {
                ...
                hash: crypto_1.default.createHash('sha256').update(buffer).digest('hex'),
            };
        }
        catch (err) {
            logger_1.log.pdf.warn({ err: err.message }, 'Puppeteer falló — usando PDFKit como fallback');
        }
    }
    else {
        logger_1.log.pdf.warn({
            hint: 'apt-get install chromium-browser',
            ejecutable: exports.browserPool.rutaChrome ?? 'no encontrado',
...
// ══════════════════════════════════════════════════════════
async function cargarCITParaPDF(citId) {
