...
export const PROTOCOLO_VERSION = 'v1.0'
export const PROTOCOLO_DESCRIPCION = {
...
const MODO_LIVE     = !!(env.MINSEG_API_URL && env.MINSEG_API_KEY)
const BASE_URL      = env.MINSEG_API_URL ?? 'https://api.seguridadmendoza.gob.ar'
 ...
 */
export function generarHeadersAuth(opts: {
  metodo: string
  path:   string
  body:   string
}): Record<string, string> {
  const timestamp = Math.floor(Date.now() / 1000).toString()
  const keyId     = env.MINSEG_KEY_ID ?? 'rodaid-prod-001'
  const secret    = env.MINSEG_API_KEY ?? 'STUB_SECRET'
  ...
  return {
    ...
    'X-RODAID-VERSION':   PROTOCOLO_VERSION,
    'Content-Type':       'application/json',
    'Accept':             'application/json',
    'User-Agent':         `RODAID/${PROTOCOLO_VERSION}`,
  }
}
...
export function verificarFirmaWebhook(opts: {
  signature:  string   // del header X-MINSEG-SIGNATURE
  timestamp:  string   // del header X-MINSEG-TIMESTAMP
  body:       string
  ipOrigen?:  string
  ...
  // 3. Verificar HMAC
  const sharedSecret = env.MINSEG_WEBHOOK_SECRET ?? env.MINSEG_API_KEY ?? 'STUB_WEBHOOK_SECRET'
  const bodyHash     = crypto.createHash('sha256').update(opts.body).digest('hex')
  const canonical    = `${opts.timestamp}.${bodyHash}`
  const esperado     = crypto.createHmac('sha256', sharedSecret).update(canonical).digest('hex')
  ...
  try {
    ...
    if (sigBuf.length === expBuf.length) {
      valida = crypto.timingSafeEqual(sigBuf, expBuf)
    }
  } catch { valida = false }
 ...
 */
export async function notificarCITMinSeg(opts: {
  numeroCIT:        string
  citId:            string
  serial:           string
  marca:            string
 ...
 */
export async function notificarDenunciaMinSeg(opts: {
 ...
 */
export async function consultarSerialMinSeg(serial: string): Promise<{
  encontrado:    boolean
  tipo?:         'ROBO' | 'RECUPERADO' | 'INVALIDO'
  numeroDenuncia?: string
  fechaRegistro?:  string
  accion:        'BLOQUEAR' | 'CONTINUAR' | 'ALERTA_SIN_RESPUESTA'
  fuente:        'MINSEG' | 'CACHE' | 'STUB'
}> {
  ...
  let resultado: Awaited<ReturnType<typeof consultarSerialMinSeg>>

  if (!MODO_LIVE) {
    log.minseg.warn({ serial }, '⚠ MinSeg STUB — consulta serial simulada')
    ...
    try {
      ...
      const body = await res.json() as { encontrado: boolean; tipo?: string; numeroDenuncia?: string; fechaRegistro?: string }
      ...
      log.minseg.warn({ serial, err: (err as Error).message }, '⚠ MinSeg no responde — continuando sin bloqueo')
 ...
 */
export async function notificarRecuperacionMinSeg(opts: {
  numeroDenuncia: string
  serial:         string
...
export async function procesarWebhookMinSeg(opts: {
  body:       string         // raw string del request body
  signature:  string
  timestamp:  string
  eventId:    string         // header X-MINSEG-EVENT-ID (idempotency)
  ipOrigen?:  string
}): Promise<{ procesado: boolean; accion?: string; mensaje: string }> {
  // 1. Verificar firma
  ...
  // 3. Parsear payload
  let evento: {
    tipo: string; serial?: string; numeroDenuncia?: string
    numeroCIT?: string; descripcion?: string; provincia?: string; datos?: unknown
  }
  try {
  ...
  try {
    ...
    log.minseg.error({ evento: evento.tipo, err: (err as Error).message }, 'Error procesando webhook MinSeg')
    accion = `ERROR: ${(err as Error).message}`
  }

  log.minseg.info({ tipo: evento.tipo, serial: evento.serial, accion }, '📥 Webhook MinSeg procesado')
  return { procesado: true, accion, mensaje: 'OK' }
}

async function ejecutarAccionWebhook(
  evento: { tipo: string; serial?: string; numeroDenuncia?: string; numeroCIT?: string; descripcion?: string; provincia?: string; datos?: unknown },
  intercambioId: string
): Promise<string> {
  switch (evento.tipo) {

    case 'ROBO_REGISTRADO': {
      if (!evento.serial) return 'Sin serial — ignorado'
      // Buscar CIT activo para este serial → bloquear
      const cit = await queryOne<{ id: string; numero_cit: string; propietario_id: string }>(
        `SELECT c.id, c.numero_cit, c.propietario_id
         FROM cits c JOIN bicicletas b ON b.id=c.bicicleta_id
         WHERE b.numero_serie=$1 AND c.estado='ACTIVO' LIMIT 1`,
        [evento.serial]
      )
      if (cit) {
        await query(`UPDATE cits SET estado='BLOQUEADO', actualizado_en=NOW() WHERE id=$1`, [cit.id])
        // Invalidar caché del serial
        await getRedis().del(`minseg:serial:${evento.serial.toUpperCase()}`).catch(() => {})
        // Notificar propietario (fire-and-forget)
        import('./device_token.service').then(dt =>
          dt.enviarPush(cit.propietario_id, {
    ...
    case 'RECUPERACION': {
      if (!evento.serial) return 'Sin serial'
      await query(
        `UPDATE cits c SET estado='ACTIVO', actualizado_en=NOW()
         FROM bicicletas b WHERE b.id=c.bicicleta_id AND b.numero_serie=$1 AND c.estado='BLOQUEADO'`,
        [evento.serial]
      )
      await getRedis().del(`minseg:serial:${evento.serial.toUpperCase()}`).catch(() => {})
    ...
    case 'CIT_INVALIDO': {
      if (!evento.numeroCIT) return 'Sin numeroCIT'
      const cit = await queryOne<{ id: string; firma_payload_id: string | null }>(
        `SELECT id, firma_payload_id FROM cits WHERE numero_cit=$1`, [evento.numeroCIT]
      )
      if (cit) {
        await query(`UPDATE cits SET estado='RECHAZADO', actualizado_en=NOW() WHERE id=$1`, [cit.id])
        if (cit.firma_payload_id) {
          await query(
            `UPDATE firmas_payload_cit SET revocada=TRUE, motivo_revocacion='MinSeg CIT_INVALIDO' WHERE id=$1`,
            [cit.firma_payload_id]
          )
        }
      }
    ...
    case 'ALERTA_ZONA': {
      ...
      import('./fcm.service').then(fcm =>
        fcm.enviarPushTopico(topico, {
 ...
 */
export async function sincronizarDiario(): Promise<SyncDiarioResult> {
  const inicio  = Date.now()
  const desde   = new Date(Date.now() - 24 * 3_600_000)
  const hasta   = new Date()

  const syncLog = await queryOne<{ id: string }>(
    `INSERT INTO minseg_sync_log (tipo_sync, periodo_desde, periodo_hasta)
     VALUES ('DIARIO',$1,$2) RETURNING id`,
    [desde, hasta]
  )
  const syncId = syncLog!.id

  let citsEnviados = 0; let errores = 0; const detalles: unknown[] = []

  try {
    // 1. CITs emitidos en las últimas 24h
    const cits = await query<any>(
      `SELECT c.id, c.numero_cit, b.numero_serie, b.marca, b.modelo,
    ...
    await query(
      `UPDATE minseg_sync_log SET estado='COMPLETADO', cits_enviados=$2,
       errores=$3, duracion_ms=$4, completado_en=NOW(), detalle=$5::jsonb WHERE id=$1`,
      [syncId, citsEnviados, errores, Date.now() - inicio, JSON.stringify(detalles)]
    )

    log.minseg.info({ syncId, citsEnviados, errores, ms: Date.now() - inicio },
      '✓ Sync diario MinSeg completado')

  } catch (err) {
    await query(
      `UPDATE minseg_sync_log SET estado='FALLIDO', errores=errores+1, completado_en=NOW() WHERE id=$1`,
      [syncId]
    )
    log.minseg.error({ syncId, err: (err as Error).message }, '✗ Sync diario MinSeg FALLIDO')
    throw err
  }
...
async function enviarLoteSync(items: any[], tipo: string): Promise<{ exitosos: number; errores: number; detalle: unknown }> {
  if (!MODO_LIVE) {
    return { exitosos: items.length, errores: 0, detalle: { modo: 'STUB', count: items.length } }
  }
  const payload = JSON.stringify({ protocolo: PROTOCOLO_VERSION, tipo: 'SYNC_LOTE', items, timestamp: new Date().toISOString() })
  ...
  try {
    ...
    const body = await res.json() as { exitosos?: number; errores?: number }
    return { exitosos: body.exitosos ?? items.length, errores: body.errores ?? 0, detalle: body }
  } catch (err) {
    return { exitosos: 0, errores: items.length, detalle: { error: (err as Error).message } }
  }
}
...
export async function procesarColaPendiente(): Promise<{ procesados: number; exitosos: number; fallidos: number }> {
  const pendientes = await query<any>(
    `SELECT id, tipo, http_metodo, http_url, payload_hash, reintentos
  ...
  for (const item of pendientes) {
    try {
      ...
      await query(
        `UPDATE minseg_intercambios SET estado='EXITOSO', resuelto_en=NOW() WHERE id=$1`, [item.id]
      )
      exitosos++
    } catch {
      fallidos++
...
export async function getEstadisticasIntercambio(dias = 7) {
  const [resumen, porTipo, pendientes] = await Promise.all([
    ...
    queryOne<{ count: string }>(
      `SELECT COUNT(*)::text AS count FROM minseg_intercambios
       WHERE estado IN ('PENDIENTE','REINTENTANDO')`, []
    ),
  ])
  return {
    ...
    protocolo:       PROTOCOLO_DESCRIPCION,
  }
}

export async function getHistorialIntercambios(opts?: {
  tipo?: string; serial?: string; pagina?: number; porPagina?: number
}) {
  ...
  let idx = 1
  if (opts?.tipo)   { conds.push(`tipo=$${idx++}`);    params.push(opts.tipo)   }
  if (opts?.serial) { conds.push(`serial=$${idx++}`);  params.push(opts.serial) }
  const where = conds.length > 0 ? `WHERE ${conds.join(' AND ')}` : ''
  return query<any>(
    `SELECT id, tipo, direccion, serial, http_status, estado, latencia_ms,
...
export interface IntercambioResult {
  intercambioId: string
  estado:        string
  httpStatus?:   number
  payload?:      unknown
  reintentosPendientes?: boolean
}

export interface SyncDiarioResult {
  syncId:        string
  desde:         Date
  hasta:         Date
  citsEnviados:  number
...
async function ejecutarIntercambio(opts: {
  ...
  if (!MODO_LIVE) {
    log.minseg.warn({ tipo: opts.tipo, serial: opts.serial }, '⚠ MinSeg STUB — intercambio simulado')
  ...
  try {
    ...
    if (!ok && opts.urgente) {
      ...
      await query(
        `UPDATE minseg_intercambios SET estado='REINTENTANDO', proximo_reintento=NOW()+INTERVAL'10s' WHERE id=$1`,
        [id]
      )
    }
    ...
    const id = await registrarIntercambio({
      ...
      mensajeResp: (err as Error).message,
    })
...
async function registrarIntercambio(opts: {
  tipo: string; metodo: string; path: string; serial?: string; citId?: string
  payloadHash?: string; httpStatus?: number; estado: string; latencia?: number
  mensajeResp?: string; ip?: string
}): Promise<string> {
  const row = await queryOne<{ id: string }>(
    `INSERT INTO minseg_intercambios
...
async function registrarAlerta(intercambioId: string, evento: { tipo: string; serial?: string; numeroDenuncia?: string; numeroCIT?: string; descripcion?: string; datos?: unknown }, accion: string): Promise<void> {
  await query(
    `INSERT INTO minseg_alertas (intercambio_id, tipo_alerta, serial, numero_denuncia, descripcion, datos_extra, accion_tomada, procesado_en)
     VALUES ($1,$2,$3,$4,$5,$6::jsonb,$7,NOW())`,
    [intercambioId, evento.tipo, evento.serial ?? null, evento.numeroDenuncia ?? null,
