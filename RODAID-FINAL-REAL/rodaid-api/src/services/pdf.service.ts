...
export async function generarPDFCIT(data: CITParaPDF): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    ...
    doc.fill(aprobado ? '#166534' : '#991B1B').font('Helvetica-Bold').fontSize(10)
       .text(
         `${aprobado ? '✓ APROBADO' : '✗ RECHAZADO'} — ${data.totalPuntos}/20 puntos`,
         L + 8, doc.y + 6
       )
    doc.moveDown(1.5)
    ...
    doc.fill('white').font('Helvetica').fontSize(7)
       .text(
         `Este certificado fue emitido conforme a la Ley Provincial de Mendoza N° 9556. ` +
         `El hash SHA-256 está anclado en la Blockchain Federal Argentina (BFA, ONTI) como prueba de integridad. ` +
         `Hash: ${data.hashSHA256.slice(0, 32)}...`,
         L, footerY, { width: R - L, align: 'center' }
       )
...
function sectionTitle(doc: PDFKit.PDFDocument, title: string, L: number, R: number) {
...
function infoGrid(doc: PDFKit.PDFDocument, rows: [string, string][], L: number, colW: number) {
