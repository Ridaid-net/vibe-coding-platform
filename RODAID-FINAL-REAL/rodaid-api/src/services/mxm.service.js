...
exports.mxmService = void 0;
exports.processMxMCallback = processMxMCallback;
exports.getMxMAccessToken = getMxMAccessToken;
exports.getMxMAuthUrl = getMxMAuthUrl;
exports.purgeExpiredStates = purgeExpiredStates;
exports.getMxMAuditLog = getMxMAuditLog;
...
const MXM_BASE = env_1.env.MXM_AUTH_URL ?? 'https://auth.mendoza.gob.ar';
const MXM_TOKEN = env_1.env.MXM_TOKEN_URL ?? `${MXM_BASE}/oauth/token`;
const MXM_USERINFO = `${MXM_BASE}/oauth/userinfo`;
const MXM_PAGOS = env_1.env.MXM_PAGOS_URL ?? `${MXM_BASE}/api/pagos`;
const MXM_TRAMITES = env_1.env.MXM_TRAMITES_URL ?? `${MXM_BASE}/api/tramites`;
const MXM_NOTIF = env_1.env.MXM_NOTIF_URL ?? `${MXM_BASE}/api/notificaciones`;
...
// DB HELPERS
// ══════════════════════════════════════════════════════════
async function storeOAuthState(state, codeVerifier, ctx) {
    await (0, database_1.query)(`INSERT INTO mxm_oauth_state (state, code_verifier, redirect_to, ip_address, user_agent, expires_at)
     VALUES ($1, $2, $3, $4::inet, $5, NOW() + INTERVAL '${STATE_TTL_MINUTES} minutes')`, [state, codeVerifier, ctx.redirectTo ?? null, ctx.ipAddress ?? null, ctx.userAgent ?? null]);
}
async function consumeOAuthState(state) {
    const row = await (0, database_1.queryOne)(`UPDATE mxm_oauth_state
     SET usado = TRUE
     WHERE state = $1 AND expires_at > NOW() AND NOT usado
     RETURNING code_verifier, redirect_to, usado`, [state]);
    if (!row)
        throw new errorHandler_1.AppError('State OAuth inválido o expirado. Iniciá el proceso nuevamente.', 400, 'INVALID_OAUTH_STATE');
    return { codeVerifier: row.code_verifier, redirectTo: row.redirect_to };
}
async function saveMxMTokens(userId, tokens, cuil, nivel) {
    const expiresAt = new Date(Date.now() + tokens.expires_in * 1000);
    await (0, database_1.query)(`INSERT INTO mxm_tokens (usuario_id, access_token, refresh_token, token_type, expires_at, scope, cuil, nivel)
     ...
     DO UPDATE SET
       ...
       actualizado_en = NOW()`, [userId, tokens.access_token, tokens.refresh_token ?? null, tokens.token_type, expiresAt, SCOPES, cuil, nivel]);
}
async function auditLog(evento, data) {
    await (0, database_1.query)(`INSERT INTO mxm_audit_log (usuario_id, evento, cuil, nivel, ip_address, error, metadata)
     VALUES ($1, $2, $3, $4, $5::inet, $6, $7)`, [data.usuarioId ?? null, evento, data.cuil ?? null, data.nivel ?? null,
        data.ipAddress ?? null, data.error ?? null,
        data.metadata ? JSON.stringify(data.metadata) : null]).catch(e => logger_1.log.mxm.warn({ e }, 'Audit log error (non-critical)'));
}
// ══════════════════════════════════════════════════════════
...
class MxMServiceReal {
    ...
    async initOAuth(ctx = {}) {
        ...
        const params = new URLSearchParams({
            response_type: 'code',
            client_id: env_1.env.MXM_CLIENT_ID,
            redirect_uri: env_1.env.MXM_REDIRECT_URI,
            scope: SCOPES,
            state,
        ...
        logger_1.log.mxm.info({ state: state.slice(0, 8) + '...', ip: ctx.ipAddress }, 'MxM OAuth iniciado');
        return { authUrl, state, codeChallenge };
    }
    // ── Paso 2: Intercambiar código por tokens ──────────────
    async exchangeCode(code, state, ctx = {}) {
        ...
        const body = new URLSearchParams({
            ...
            redirect_uri: env_1.env.MXM_REDIRECT_URI,
            client_id: env_1.env.MXM_CLIENT_ID,
            client_secret: env_1.env.MXM_CLIENT_SECRET,
        ...
        if (!res.ok) {
            ...
            logger_1.log.mxm.error({ status: res.status, body: errBody }, 'MxM token exchange falló');
            await auditLog('token_exchange_fail', { ipAddress: ctx.ipAddress, error: errBody });
            throw new errorHandler_1.AppError('Error al obtener tokens de MxM', 502, 'MXM_TOKEN_ERROR');
        }
        const tokens = await res.json();
        const identidad = await this.getIdentidad(tokens.access_token);
        logger_1.log.mxm.info({ cuil: identidad.cuil, nivel: identidad.nivel }, 'MxM token exchange exitoso');
        return { tokens, identidad, stateData };
    }
    // ── Obtener identidad del usuario ───────────────────────
    async getIdentidad(accessToken) {
        const res = await fetch(MXM_USERINFO, {
            headers: { Authorization: `Bearer ${accessToken}` },
        });
        if (!res.ok)
            throw new errorHandler_1.AppError('Error al obtener identidad MxM', 502, 'MXM_IDENTIDAD_ERROR');
    ...
    async refreshToken(userId) {
        const stored = await (0, database_1.queryOne)(`SELECT refresh_token, expires_at FROM mxm_tokens WHERE usuario_id = $1`, [userId]);
        if (!stored?.refresh_token)
            return null;
        // Si todavía no expiró, devolver null (no es necesario refrescar)
        if (new Date(stored.expires_at) > new Date(Date.now() + 60_000))
            return null;
        const body = new URLSearchParams({
        ...
        try {
            ...
            logger_1.log.mxm.info({ userId }, 'MxM access token refrescado');
            return tokens;
        }
        catch (err) {
            logger_1.log.mxm.warn({ err, userId }, 'MxM refresh token falló');
            return null;
        }
    }
    // ── Pago de tasa CIT via gateway oficial ────────────────
    async iniciarPago(accessToken, payload) {
        ...
        if (!res.ok)
            throw new errorHandler_1.AppError('Error al iniciar pago MxM', 502, 'MXM_PAGO_ERROR');
        return (await res.json()).pagoId;
    }
    // ── Expediente en sistema provincial ───────────────────
    async crearExpediente(accessToken, citId) {
        ...
        if (!res.ok)
            throw new errorHandler_1.AppError('Error al crear expediente', 502, 'MXM_TRAMITE_ERROR');
        return (await res.json()).expedienteId;
    }
    // ── Notificación oficial — endpoint gubernamental ────────
    async enviarNotificacion(accessToken, titulo, cuerpo, opciones) {
        const payload = {
            titulo,
            cuerpo,
        ...
        if (httpStatus === 429) {
            logger_1.log.mxm.warn({ titulo, httpStatus }, 'MxM rate limit — reintentar más tarde');
        }
        else if (httpStatus >= 500) {
            logger_1.log.mxm.error({ titulo, httpStatus, errBody: errBody.slice(0, 100) }, 'MxM error 5xx');
        }
        else {
            logger_1.log.mxm.warn({ titulo, httpStatus }, `MxM notificación rechazada`);
        }
...
class MxMServiceStub {
    ...
    async initOAuth(ctx = {}) {
        ...
        logger_1.log.mxm.warn({ authUrl }, '⚠️  MxM STUB — redirigir a URL simulada');
        return { authUrl, state, codeChallenge };
    }
    async exchangeCode(code, state, _ctx = {}) {
        const stateData = await consumeOAuthState(state);
        const stubUser = this.STUB_USERS[code] ?? this.STUB_USERS['dev_code'];
        logger_1.log.mxm.warn({ code, nivel: stubUser.nivel }, '⚠️  MxM STUB — identidad simulada');
        return {
            tokens: {
    ...
    async getIdentidad(_token) {
        return { sub: 'stub-sub', cuil: '20-30123456-7', dni: '30123456', nombre: 'Demo', apellido: 'MxM', nivel: 2 };
    }
    async refreshToken(_userId) { return null; }
    async iniciarPago(_token, _p) {
        logger_1.log.mxm.warn('MxM STUB: pago simulado');
        return 'pago_stub_' + Date.now();
    }
    async crearExpediente(_token, _citId) {
        return 'EXP-STUB-' + Date.now();
    }
    async enviarNotificacion(_token, titulo, _cuerpo, opciones) {
        logger_1.log.mxm.debug({ titulo, tipo: opciones?.tipoMxM ?? 'INFORMATIVA' }, '⚠ MxM STUB: notificación simulada');
...
// ══════════════════════════════════════════════════════════
async function processMxMCallback(code, state, ctx = {}) {
    const { tokens, identidad, stateData } = await exports.mxmService.exchangeCode(code, state, ctx);
    const id = identidad;
    ...
    if (identidad.nivel < 1) {
        ...
        throw new errorHandler_1.AppError('Identidad MxM insuficiente. Se requiere nivel 1 o superior.', 403, 'MXM_NIVEL_INSUFICIENTE');
    }
    let isNewUser = false;
    let usuarioId;
    // Buscar usuario existente por: sub MxM > DNI > email MxM
    const existente = await (0, database_1.queryOne)(`SELECT u.id, u.email, u.nombre, u.apellido, u.rol, u.mxm_nivel, u.email_verificado
     FROM usuarios u
     WHERE u.mxm_sub = $1
        OR u.dni = $2
        OR (u.email = $3 AND u.email_verificado = TRUE)
     LIMIT 1`, [id.sub ?? null, identidad.dni, id.email ?? null]);
    if (existente) {
        usuarioId = existente.id;
        // Actualizar datos MxM del usuario existente
        await (0, database_1.query)(`UPDATE usuarios SET
         mxm_sub              = $2,
         mxm_verificado       = TRUE,
         mxm_nivel            = $3,
         ...
         actualizado_en       = NOW()
       WHERE id = $1`, [usuarioId, id.sub ?? identidad.cuil, identidad.nivel, id.email ?? null]);
        ...
        const rows = await (0, database_1.transaction)(async (client) => {
            const plan = await client.query(`SELECT id FROM planes WHERE nombre = 'libre' LIMIT 1`);
            const planId = plan.rows[0]?.id ?? null;
            return client.query(`INSERT INTO usuarios
         ...
         RETURNING id`, [emailMxM, identidad.nombre, identidad.apellido,
    ...
    const usuario = await (0, database_1.queryOne)(`SELECT id, email, nombre, apellido, rol, email_verificado FROM usuarios WHERE id = $1`, [usuarioId]);
    // Emitir JWT RODAID
    const { buildTokenPair } = await import('./jwt.service');
    const jwtTokens = await buildTokenPair(usuarioId, usuario.email, usuario.rol, { ipAddress: ctx.ipAddress, userAgent: ctx.userAgent });
    return {
        usuario: {
            id: usuario.id,
            email: usuario.email,
...
async function getMxMAccessToken(userId) {
...
function getMxMAuthUrl(state) {
    if (env_1.isDev || !env_1.env.MXM_CLIENT_ID) {
        const base = process.env.FRONTEND_URL ?? 'http://localhost:5173';
        return `${base}/dev/mxm-callback?state=${state}&code=dev_code`;
    }
    const params = new URLSearchParams({
        response_type: 'code', client_id: env_1.env.MXM_CLIENT_ID, redirect_uri: env_1.env.MXM_REDIRECT_URI,
        scope: SCOPES, state,
    });
...
async function purgeExpiredStates() {
    const result = await (0, database_1.query)(`WITH deleted AS (
       DELETE FROM mxm_oauth_state WHERE expires_at < NOW() RETURNING state
     ) SELECT COUNT(*)::text AS count FROM deleted`);
    return parseInt(result[0]?.count ?? '0');
}
// ── Obtener historial de logins MxM ──────────────────────
async function getMxMAuditLog(userId, limit = 20) {
    return (0, database_1.query)(`SELECT evento, nivel, ip_address::text, creado_en, error
     FROM mxm_audit_log WHERE usuario_id = $1
     ORDER BY creado_en DESC LIMIT $2`, [userId, limit]);
}
// ── Instancia del servicio ────────────────────────────────
const hasMxMConfig = !!(env_1.env.MXM_CLIENT_ID && env_1.env.MXM_CLIENT_SECRET);
exports.mxmService = hasMxMConfig ? new MxMServiceReal() : new MxMServiceStub();
