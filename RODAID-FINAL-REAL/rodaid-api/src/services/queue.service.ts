...
export async function initQueue(): Promise<void> {
  ...
  qValidar   = new Bull<ValidarCITPayload>  ('rodaid:cit:validar',   REDIS_OPTS)
  qFinalizar = new Bull<FinalizarCITPayload>('rodaid:cit:finalizar', REDIS_OPTS)
  qNotif     = new Bull<NotifPayload>       ('rodaid:notif',         REDIS_OPTS)
  qExpire    = new Bull                     ('rodaid:cit:expirar',   REDIS_OPTS)
...
export async function encolarValidacion(citId: string, venceEn: Date): Promise<string | undefined> {
  if (!qValidar) throw new Error('Queue no inicializada — llamar initQueue() primero')
...
export async function encolarFinalizar(citId: string, propietarioWallet?: string): Promise<string | undefined> {
  if (!qFinalizar) throw new Error('Queue no inicializada')
...
export async function cancelarValidacion(citId: string): Promise<{ cancelado: boolean; jobId?: string }> {
  if (!qValidar) return { cancelado: false }
...
export async function encolarNotificacion(payload: NotifPayload): Promise<void> {
  if (!qNotif) { log.queue.warn('Queue notif no disponible'); return }
...
export async function getQueueStats() {
  if (!qValidar || !qFinalizar) return { error: 'Queues no inicializadas' }
...
export async function reintentarJob(jobId: string): Promise<{ ok: boolean; message: string }> {
  if (!qValidar || !qFinalizar) return { ok: false, message: 'Queues no inicializadas' }
