...
export interface Conversacion {
  ...
  noLeidosVendedor:    number
  noLeidosInteresado:  number
  bloqueada:           boolean
  creadaEn:            Date
  // Relaciones expandidas
  publicacion?: { slug: string; titulo: string; precioARS: number; fotosUrls: string[] }
  vendedor?:    { id: string; nombre: string }
  interesado?:  { id: string; nombre: string }
  ultimoMensaje?: { cuerpo: string; remitenteId: string; tipo: TipoMensaje }
}
...
export interface Mensaje {
  ...
  creadoEn:        Date
  esPropio?:       boolean   // relativo al usuario que consulta
}
...
export interface EnviarMensajeInput {
  ...
  datos?:         Record<string, unknown>
  adjuntos?:      string[]
  conversacionId?: string   // para que el vendedor especifique qué conv está respondiendo
}
...
function mapConversacion(r: any): Conversacion {
  return {
    id:                  r.id,
...
function mapMensaje(r: any, miId?: string): Mensaje {
  return {
    id:             r.id,
...
async function checkRateLimitMensajes(usuarioId: string, conversacionId: string): Promise<void> {
  try {
    const redis = getRedis()
    const key   = `msg:rl:${usuarioId}:${conversacionId}`
    const count = await redis.incr(key)
    if (count === 1) await redis.expire(key, 3600)  // TTL 1h
    if (count > 30) throw Object.assign(
      new Error('Límite de mensajes alcanzado. Máximo 30 mensajes por hora en esta conversación.'),
      { code: 'RATE_LIMIT_MENSAJES', status: 429 }
    )
  } catch (err: any) {
    if (err.code === 'RATE_LIMIT_MENSAJES') throw err
...
export async function enviarMensaje(input: EnviarMensajeInput): Promise<{
  mensaje:       Mensaje
  conversacionId: string
  nueva:         boolean
}> {
  ...
  const pub = await queryOne<{
    id: string; estado: string; vendedor_id: string; titulo: string
  }>(
    `SELECT id, estado::text AS estado, vendedor_id, titulo
  ...
  if (pub.estado === 'CANCELADA') throw Object.assign(
    new Error('Esta publicación ya no está disponible para mensajes'),
    { code: 'PUB_UNAVAILABLE', status: 422 }
  )
  ...
  if (esVendedor && input.remitenteId === pub.vendedor_id &&
    ...
    throw Object.assign(
      new Error('El vendedor debe especificar conversacionId para responder'),
      { code: 'VENDOR_NEEDS_CONV_ID', status: 422 }
    )
  }

  // 2. Obtener o crear conversación
  let conv = await queryOne<{ id: string; bloqueada: boolean; vendedor_id: string; interesado_id: string }>(
    input.conversacionId
      ? `SELECT id, bloqueada, vendedor_id, interesado_id FROM mensajes_conversaciones WHERE id=$1 AND publicacion_id=$2`
      : `SELECT id, bloqueada, vendedor_id, interesado_id FROM mensajes_conversaciones WHERE publicacion_id=$1 AND interesado_id=$2`,
    input.conversacionId
  ...
  if (!conv) {
    if (esVendedor) throw Object.assign(
      new Error('Conversación no encontrada para esta publicación'),
    ...
    try {
      ...
      if (countNew === 1) await redis.expire(keyNew, 3600)
      if (countNew > 5) throw Object.assign(
        new Error('Máximo 5 conversaciones nuevas por hora. Reintentá más tarde.'),
        { code: 'RATE_LIMIT_CONV', status: 429 }
      )
    } catch (err: any) {
      if (err.code === 'RATE_LIMIT_CONV') throw err
    }

    // Crear nueva conversación
    const created = await queryOne<{ id: string }>(
      `INSERT INTO mensajes_conversaciones
         (publicacion_id, vendedor_id, interesado_id)
       VALUES ($1, $2, $3)
       RETURNING id`,
      [input.publicacionId, pub.vendedor_id, input.remitenteId]
    )
    conv = { id: created!.id, bloqueada: false, vendedor_id: pub.vendedor_id, interesado_id: input.remitenteId }
    nueva = true
  }

  if (conv.bloqueada) throw Object.assign(
    new Error('Esta conversación ha sido bloqueada por moderación'),
    { code: 'CONV_BLOQUEADA', status: 403 }
  )
  ...
  getRedis().del(`msg:conv:${conv.id}`).catch(() => {})

  log.mensajeria.debug({
    convId: conv.id,
...
export async function getMensajes(opts: {
  conversacionId: string
  usuarioId:      string
  cursor?:        string   // UUID del último mensaje visto (paginación por cursor)
  limite?:        number
}): Promise<{ mensajes: Mensaje[]; hayMas: boolean; totalNoLeidos: number }> {
  const limite = Math.min(opts.limite ?? 30, 100)

  // Verificar acceso
  const conv = await queryOne<{
    id: string; vendedor_id: string; interesado_id: string; bloqueada: boolean
    no_leidos_vendedor: number; no_leidos_interesado: number
  }>(
    `SELECT id, vendedor_id, interesado_id, bloqueada,
            no_leidos_vendedor, no_leidos_interesado
     FROM mensajes_conversaciones WHERE id = $1`,
    [opts.conversacionId]
  )

  if (!conv) throw Object.assign(new Error('Conversación no encontrada'), { code: 'NOT_FOUND', status: 404 })

  const esParticipante = conv.vendedor_id === opts.usuarioId || conv.interesado_id === opts.usuarioId
  if (!esParticipante) throw Object.assign(
    new Error('No tenés acceso a esta conversación'),
  ...
  let cursorCond = ''
  const params: unknown[] = [opts.conversacionId]

  if (opts.cursor) {
    const cursorMsg = await queryOne<{ creado_en: Date }>(
      `SELECT creado_en FROM mensajes WHERE id = $1`, [opts.cursor]
    )
    if (cursorMsg) {
  ...
  const filas = await query<any>(
    `SELECT m.id, m.conversacion_id, m.remitente_id, m.cuerpo, m.adjuntos,
  ...
  const mensajes = filas.slice(0, limite).map((r: any) => mapMensaje(r, opts.usuarioId))
...
export async function marcarLeidos(conversacionId: string, usuarioId: string): Promise<void> {
  ...
  const conv = await queryOne<{ vendedor_id: string }>(
    `SELECT vendedor_id FROM mensajes_conversaciones WHERE id=$1`, [conversacionId]
  )

  if (conv?.vendedor_id === usuarioId) {
    await query(`UPDATE mensajes_conversaciones SET no_leidos_vendedor=0 WHERE id=$1`, [conversacionId])
  } else {
    await query(`UPDATE mensajes_conversaciones SET no_leidos_interesado=0 WHERE id=$1`, [conversacionId])
  }
}
...
export async function misConversaciones(usuarioId: string, opts: {
  ...
  const [rows, total] = await Promise.all([
    ...
    queryOne<{ n: string }>(
      `SELECT COUNT(*)::text AS n FROM mensajes_conversaciones c
       WHERE (c.vendedor_id=$1 OR c.interesado_id=$1) ${filtroNL}`,
      [usuarioId]
    ),
  ])
...
export async function getConversacion(opts: {
  conversacionId?: string
  publicacionId?:  string
  usuarioId:       string
  ...
  const row = await queryOne<any>(
    `SELECT ${SELECT_CONV} FROM mensajes_conversaciones c ${JOIN_CONV} WHERE ${cond}`,
    params
  )
...
export async function eliminarMensaje(mensajeId: string, usuarioId: string, esAdmin = false): Promise<void> {
  const msg = await queryOne<{ remitente_id: string; eliminado: boolean }>(
    `SELECT remitente_id, eliminado FROM mensajes WHERE id=$1`, [mensajeId]
  )

  if (!msg) throw Object.assign(new Error('Mensaje no encontrado'), { code: 'NOT_FOUND', status: 404 })
  if (msg.eliminado) throw Object.assign(new Error('El mensaje ya fue eliminado'), { code: 'ALREADY_DELETED', status: 422 })
  if (!esAdmin && msg.remitente_id !== usuarioId) throw Object.assign(
    new Error('Solo podés eliminar tus propios mensajes'),
    { code: 'FORBIDDEN', status: 403 }
  )

  await query(
    `UPDATE mensajes SET eliminado=TRUE, eliminado_por=$2 WHERE id=$1`,
    [mensajeId, usuarioId]
  )
}
...
export async function mensajeSistema(opts: {
  publicacionId: string
  interesadoId:  string
  cuerpo:        string
  datos?:        Record<string, unknown>
  ...
  const pub = await queryOne<{ vendedor_id: string }>(
    `SELECT vendedor_id FROM marketplace_publicaciones WHERE id=$1`,
    [opts.publicacionId]
  )
  if (!pub) return
  ...
  await enviarMensaje({
    ...
    tipo:           'SISTEMA',
    datos:          opts.datos,
  }).catch(err => log.mensajeria.warn({ err: err.message }, 'Error enviando mensaje de sistema'))
}
...
export async function bloquearConversacion(conversacionId: string, motivo: string): Promise<void> {
  await query(
    `UPDATE mensajes_conversaciones SET bloqueada=TRUE, motivo_bloqueo=$2 WHERE id=$1`,
    [conversacionId, motivo]
  )
  log.mensajeria.warn({ conversacionId, motivo }, '🚫 Conversación bloqueada')
}
...
export async function totalNoLeidos(usuarioId: string): Promise<number> {
  const r = await queryOne<{ n: string }>(
    `SELECT COALESCE(SUM(
     ...
     WHERE c.vendedor_id=$1 OR c.interesado_id=$1`,
    [usuarioId]
  )
