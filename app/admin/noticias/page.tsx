...
import { Plus, Edit3, Trash2, Eye, EyeOff, Save, X, Play } from 'lucide-react'
...
const VACIA = {
  titulo: '', resumen: '', url: '', fuente: 'RODAID',
  tipo: 'noticia' as const, orden: 0, video_url: '', es_comunicado_prensa: false,
}
...
export default function AdminNoticiasPage() {
  ...
  const cambiarVideoUrl = (valor: string) => {
    setEditando(prev => ({ ...prev, video_url: valor }))
    setVideoError(valor && !extraerEmbedSeguro(valor) ? 'Tiene que ser un link de YouTube o Instagram válido.' : null)
  }

  const abrirEdicion = (n: Noticia | typeof VACIA, nueva: boolean) => {
    setEditando(n)
    setEsNueva(nueva)
    elegirImagen(null)
    setVideoError(null)
  ...
  const guardar = async () => {
    ...
    if (editando.video_url && !extraerEmbedSeguro(editando.video_url)) {
      setVideoError('Tiene que ser un link de YouTube o Instagram válido.')
      return
    }
    setGuardando(true)
    try {
      const form = new FormData()
      form.append('titulo', editando.titulo ?? '')
      form.append('resumen', editando.resumen ?? '')
      form.append('url', editando.url ?? '')
      form.append('fuente', editando.fuente ?? 'RODAID')
      form.append('tipo', editando.tipo ?? 'noticia')
      form.append('orden', String(editando.orden ?? 0))
      form.append('video_url', editando.video_url ?? '')
      form.append('es_comunicado_prensa', String(editando.es_comunicado_prensa ?? false))
      if (imagenFile) form.append('imagen', imagenFile)

      if (esNueva) {
        await authedFetch('/api/v1/admin/noticias', { method: 'POST', body: form })
      } else {
        await authedFetch(`/api/v1/admin/noticias/${editando.id}`, { method: 'PATCH', body: form })
      }
      cerrarEdicion()
      cargar()
    ...
    <div className="min-h-screen bg-paper">
      ...
      <main className="mx-auto max-w-4xl px-5 py-10 sm:px-8">
        <div className="flex items-center justify-between mb-8 flex-wrap gap-4">
          ...
          <button type="button" onClick={() => abrirEdicion(VACIA, true)}
        ...
        {editando && (
          <div className="rounded-2xl border border-[#2BBCB8]/30 bg-white p-6 mb-6">
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="text-xs font-semibold text-slate-warm block mb-1">Imagen de portada</label>
                ...
                {(imagenPreview ?? editando.imagen_url) && (
                  <div className="mt-2 rounded-xl overflow-hidden border border-slate-100 h-32">
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img src={imagenPreview ?? editando.imagen_url ?? ''} alt="Preview" className="w-full h-full object-cover" />
                  </div>
                )}
              </div>

              <div>
                <label className="text-xs font-semibold text-slate-warm block mb-1">Video (YouTube o Instagram)</label>
                <input type="url" value={editando.video_url ?? ''} onChange={e => cambiarVideoUrl(e.target.value)}
                  className="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm outline-none focus:border-[#2BBCB8]"
                  placeholder="https://youtu.be/... o https://instagram.com/reel/..." />
              ...
              <label className="flex items-center gap-2 text-sm text-slate-700">
                <input type="checkbox" checked={editando.es_comunicado_prensa ?? false}
                  onChange={e => setEditando({ ...editando, es_comunicado_prensa: e.target.checked })}
                  className="rounded border-slate-300" />
                Marcar como Comunicado de Prensa
              </label>
              ...
              <div key={n.id} className={`rounded-2xl border bg-white p-5 flex gap-4 ${n.activa ? 'border-ink/10' : 'border-slate-100 opacity-60'}`}>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1 flex-wrap">
                    ...
                    {n.es_comunicado_prensa && (
                      <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-[#F47B20]/10 text-[#F47B20]">Prensa</span>
                    )}
                  </div>
                ...
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  <button type="button" onClick={() => abrirEdicion(n, false)}
