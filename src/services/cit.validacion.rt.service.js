...
exports.getValidacionSnapshot = getValidacionSnapshot;
exports.sseValidacion = sseValidacion;
exports.publicarEventoValidacion = publicarEventoValidacion;
exports.crearValidacionRT = crearValidacionRT;
exports.getValidacionesActivas = getValidacionesActivas;
const crypto_1 = __importDefault(require("crypto"));
const database_1 = require("../config/database");
const redis_1 = require("../config/redis");
const logger_1 = require("../middleware/logger");
const CANAL_CIT = (citId) => `cit:validacion:${citId}`;
const CANAL_GLOBAL = 'cit:validacion:global';
// ══════════════════════════════════════════════════════════
// SNAPSHOT — estado completo en un instante
// ══════════════════════════════════════════════════════════
async function getValidacionSnapshot(citIdOrNumero) {
...
// ══════════════════════════════════════════════════════════
function sseValidacion(citId, res) {
    // Headers SSE
    res.set({
    ...
    res.flushHeaders();
    const clientId = crypto_1.default.randomUUID().slice(0, 8);
    let closed = false;
    const emit = (evento, data) => {
        if (closed)
            return;
        try {
    ...
    getValidacionSnapshot(citId).then(snap => {
        if (snap)
            emit('cit_validacion', snap);
        else
            emit('error', { mensaje: 'CIT no encontrado', citId });
    }).catch(() => { });
    ...
    sub.on('message', (ch, msg) => {
        ...
        try {
            ...
            if (ch === CANAL_GLOBAL && data.citId !== citId)
                return;
            emit('cit_validacion', data);
        }
        catch { /* JSON inválido */ }
    });
    logger_1.log.escrow.info({ citId: citId.slice(0, 8), clientId }, '📡 SSE cliente conectado');
    ...
    const cleanup = () => {
        ...
        sub.quit().catch(() => { });
        logger_1.log.escrow.info({ citId: citId.slice(0, 8), clientId }, '📡 SSE cliente desconectado');
    };
...
// ══════════════════════════════════════════════════════════
async function publicarEventoValidacion(citId, fase, opts) {
    const redis = (0, redis_1.getRedis)();
    // Actualizar DB
    const sets = ['fase=$2', 'actualizado_en=NOW()'];
    const params = [citId, fase];
    if (opts?.progresoPct !== undefined) {
        params.push(opts.progresoPct);
        sets.push(`progreso_pct=$${params.length}`);
    }
    const tsMap = {
    ...
    if (tsMap[fase])
        sets.push(`${tsMap[fase]}=NOW()`);
    if (fase === 'COMPLETADA')
        sets.push(`activo=FALSE`);
    await (0, database_1.query)(`UPDATE cit_validaciones_rt SET ${sets.join(',')}
     WHERE cit_id=$1::uuid AND activo=TRUE`, params).catch(() => { });
    ...
    logger_1.log.escrow.info({ citId: citId.slice(0, 8), fase, progreso: snap.progresoPct }, `📡 Evento SSE publicado: ${fase}`);
}
// ══════════════════════════════════════════════════════════
// CREAR VALIDACIÓN (llamar desde iniciarCIT)
// ══════════════════════════════════════════════════════════
async function crearValidacionRT(opts) {
    const deadline72h = new Date(Date.now() + 72 * 3_600_000);
    const row = await (0, database_1.queryOne)(`INSERT INTO cit_validaciones_rt
       (cit_id, propietario_id, inspector_id, fase, progreso_pct, deadline_72h)
     VALUES ($1::uuid, $2::uuid, $3::uuid, 'INICIADA', 0, $4)
     ON CONFLICT DO NOTHING
     RETURNING id`, [opts.citId, opts.propietarioId, opts.inspectorId ?? null, deadline72h]);
...
// ══════════════════════════════════════════════════════════
async function getValidacionesActivas(limite = 20) {
...
// HELPERS
// ══════════════════════════════════════════════════════════
function calcularTiempoRestante(deadlineISO) {
...
function inferirFaseDesdeCIT(cit) {
    if (cit.estado === 'RECHAZADO')
        return { fase: 'RECHAZADA', progresoPct: 0 };
    if (cit.nft_token_id)
        return { fase: 'COMPLETADA', progresoPct: 100 };
    if (cit.estado === 'ACTIVO' && cit.tasa_pagada)
        return { fase: 'BFA_PENDING', progresoPct: 80 };
    if (cit.estado === 'ACTIVO')
        return { fase: 'TASA_PENDIENTE', progresoPct: 60 };
    if (cit.estado === 'PAGO_PENDIENTE')
        return { fase: 'TASA_PENDIENTE', progresoPct: 60 };
    if (cit.puntos_total >= 16)
        return { fase: 'PUNTUACION_OK', progresoPct: 40 };
    if (cit.fotos_count >= 1)
        return { fase: 'FOTOS_OK', progresoPct: 20 };
