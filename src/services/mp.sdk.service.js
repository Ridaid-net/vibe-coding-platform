...
exports.COMISION_PCT = exports.MODO_SANDBOX = exports.MODO_STUB = void 0;
exports.crearPreferenciaPago = crearPreferenciaPago;
exports.procesarRetornoMP = procesarRetornoMP;
exports.verificarFirmaWebhookMP = verificarFirmaWebhookMP;
exports.procesarWebhookSDK = procesarWebhookSDK;
exports.getPago = getPago;
exports.getPagoPorPreferencia = getPagoPorPreferencia;
exports.getPagoPorTransaccion = getPagoPorTransaccion;
exports.getResumenPagos = getResumenPagos;
...
// CONFIG SDK
// ══════════════════════════════════════════════════════════
const MODO_STUB = !env_1.env.RODAID_MP_ACCESS_TOKEN;
exports.MODO_STUB = MODO_STUB;
const MODO_SANDBOX = env_1.env.RODAID_MP_ACCESS_TOKEN?.startsWith('TEST-') ?? false;
exports.MODO_SANDBOX = MODO_SANDBOX;
const COMISION_PCT = parseFloat(process.env.RODAID_MP_COMISION_PCT ?? '2.50');
exports.COMISION_PCT = COMISION_PCT;
const BASE_URL = env_1.env.RODAID_BASE_URL ?? 'https://rodaid.com.ar';
const API_URL = process.env.RODAID_API_URL ?? 'https://api.rodaid.com.ar';
const WH_SECRET = process.env.MP_WEBHOOK_SECRET ?? '';
function getMPConfig(accessToken) {
    const token = accessToken ?? env_1.env.RODAID_MP_ACCESS_TOKEN ?? 'TEST-stub';
...
// ══════════════════════════════════════════════════════════
async function crearPreferenciaPago(input) {
    ...
    const pagoRow = await (0, database_1.queryOne)(`INSERT INTO pagos_mp
    ...
    if (MODO_STUB) {
        ...
        await (0, database_1.query)(`UPDATE pagos_mp SET preference_id=$2 WHERE id=$1`, [pagoId, preferenceId]);
        logger_1.log.escrow.warn({ pagoId: pagoId.slice(0, 8), monto: input.monto }, '⚠ MP STUB — configurar RODAID_MP_ACCESS_TOKEN para pagos reales');
        return {
    ...
    const prefClient = new mercadopago_1.Preference(config);
    ...
    let prefResult = null;
    try {
        prefResult = await prefClient.create({ body: prefData });
    }
    catch (err) {
        // Fallback a STUB si falla MP (no bloquear al usuario)
        logger_1.log.escrow.error({ err: err.message, transaccionId: input.transaccionId }, '✗ MP SDK preference fallida — STUB fallback');
        const stubPref = `STUB_FALLBACK_${Date.now()}`;
        await (0, database_1.query)(`UPDATE pagos_mp SET preference_id=$2, estado='CREADO' WHERE id=$1`, [pagoId, stubPref]);
        return {
    ...
    await (0, database_1.query)(`UPDATE pagos_mp SET preference_id=$2, estado='PENDIENTE' WHERE id=$1`, [pagoId, preferenceId]);
 ...
 */
async function procesarRetornoMP(params) {
    const transaccionId = params.externalRef;
    const pago = transaccionId ? await (0, database_1.queryOne)(`SELECT id, estado FROM pagos_mp WHERE transaccion_id=$1 ORDER BY creado_en DESC LIMIT 1`, [transaccionId]) : null;
    // Actualizar mp_payment_id si viene en el retorno
    if (pago && params.paymentId) {
        await (0, database_1.query)(`UPDATE pagos_mp SET mp_payment_id=$2, webhook_count=webhook_count+1 WHERE id=$1`, [pago.id, parseInt(params.paymentId)]).catch(() => { });
    }
    const mensajes = {
 ...
 */
function verificarFirmaWebhookMP(opts) {
    if (!WH_SECRET || MODO_STUB)
        return { valida: true }; // STUB: siempre válido
    const parts = {};
    ...
    const v1 = parts['v1'] ?? '';
    if (!ts || !v1)
        return { valida: false, ts, v1 };
    // Template: "id:{data.id};request-id:{X-Request-Id};ts:{ts};"
    // MP firma el id del recurso + request-id + timestamp
    const dataId = (() => {
        try {
            return JSON.parse(opts.rawBody)?.data?.id ?? '';
        }
        catch {
            return '';
        }
    ...
    const esperado = crypto_1.default.createHmac('sha256', WH_SECRET).update(plantilla).digest('hex');
    const valida = crypto_1.default.timingSafeEqual(Buffer.from(v1.padEnd(64, '0').slice(0, 64), 'hex'), Buffer.from(esperado.slice(0, 64), 'hex'));
    return { valida, ts, v1 };
}
/**
 * Procesar webhook de MP usando el SDK para consultar el estado real.
 */
async function procesarWebhookSDK(opts) {
    // 1. Verificar firma
    const firma = verificarFirmaWebhookMP(opts);
    if (!firma.valida) {
        logger_1.log.escrow.warn({ sig: opts.xSignature.slice(0, 30) }, '⚠ Webhook MP: firma inválida');
        return { procesado: false, accionesEjecutadas: [], mensaje: 'Firma inválida' };
    }
    // 2. Parsear evento
    let evento;
    try {
        evento = JSON.parse(opts.rawBody);
    }
    catch {
    ...
    if (!mpId)
        return { procesado: false, accionesEjecutadas: [], mensaje: 'Sin data.id' };
    ...
    if (tipo === 'payment') {
        ...
        const payClient = new mercadopago_1.Payment(config);
        let mpPago = null;
        if (!MODO_STUB) {
        ...
        const mpFee = mpPago.fee_details?.find((f) => f.type === 'mercadopago_fee')?.amount ?? 0;
        // Buscar nuestro pago (por transaccion_id o mp_payment_id)
        const mpIdNum = parseInt(mpId);
        const pago = await (0, database_1.queryOne)(`SELECT id, estado, monto_ars FROM pagos_mp
       WHERE mp_payment_id=$1 OR (transaccion_id::text=$2 AND $2 IS NOT NULL)
       ORDER BY creado_en DESC LIMIT 1`, [isNaN(mpIdNum) ? null : mpIdNum, transRef ?? null]);
        if (pago) {
            estadoAnterior = pago.estado;
            pagoId = pago.id;
            await (0, database_1.query)(`UPDATE pagos_mp SET
         ...
         WHERE id=$1`, [
            ...
            if (transRef) {
                await (0, database_1.query)(`INSERT INTO pagos_mp
...
async function ejecutarConsecuencias(pagoId, mpPaymentId, acciones) {
    // a. Confirmar transacción del marketplace si aplica
    const pago = await (0, database_1.queryOne)(`SELECT transaccion_id FROM pagos_mp WHERE id=$1`, [pagoId]);
    ...
    const comprador = await (0, database_1.queryOne)(`SELECT comprador_id FROM pagos_mp WHERE id=$1`, [pagoId]);
    if (comprador?.comprador_id) {
        import('./device_token.service').then(dt => dt.enviarPush(comprador.comprador_id, {
...
async function getPago(pagoId) {
    return (0, database_1.queryOne)(`SELECT * FROM pagos_mp WHERE id=$1`, [pagoId]);
}
async function getPagoPorPreferencia(preferenceId) {
    return (0, database_1.queryOne)(`SELECT * FROM pagos_mp WHERE preference_id=$1`, [preferenceId]);
}
async function getPagoPorTransaccion(transaccionId) {
    return (0, database_1.queryOne)(`SELECT * FROM pagos_mp WHERE transaccion_id=$1 ORDER BY creado_en DESC LIMIT 1`, [transaccionId]);
}
async function getResumenPagos(dias = 30) {
