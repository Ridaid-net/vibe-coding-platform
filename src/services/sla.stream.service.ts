...
import { calcularSLA72h, ENDPOINT_XREF, SLA_OBJETIVO_MS } from './sla.service'
...
export interface SLASnapshotRT {
  ts:                 number
  ventana:            string
  endpoint:           string
  objetivo:           number
  ...
  proximoRecalculo: number    // unix ms
...
async function calcularSnapshotRT(endpoint: string = ENDPOINT_XREF): Promise<SLASnapshotRT> {
  ...
  if (cached) {
    const parsed = JSON.parse(cached) as SLASnapshotRT
    return { ...parsed, fuente: 'cache' }
  }

  // Cálculo real desde DB
  const metricas = await query<{
    latencia_ms: string; creado_en: string
  }>(
    `SELECT latencia_ms, creado_en::text
  ...
  const sobreObj    = lats.filter(l => l > SLA_OBJETIVO_MS).length
  ...
  const anterior = await queryOne<{ cumplimiento: string }>(
    `SELECT cumplimiento FROM sla_snapshots WHERE endpoint=$1
  ...
  const estado: SLASnapshotRT['estado'] =
    cumplimiento >= 99    ? 'OK'
    : cumplimiento >= 95  ? 'WARNING'
    : 'CRITICAL'

  const snapshot: SLASnapshotRT = {
    ts:                 Date.now(),
    ventana:            '72h',
    endpoint,
...
async function calcularTendenciaRT(endpoint: string) {
...
function snapshotVacio(endpoint: string): SLASnapshotRT {
  return {
    ts: Date.now(), ventana: '72h', endpoint,
    objetivo: SLA_OBJETIVO_MS, total: 0,
...
export function sseHandler(res: Response, endpoint: string = ENDPOINT_XREF): string {
  ...
  log.escrow.info({ clientId, endpoint, totalClients: clients.size }, '→ SSE cliente conectado')
  ...
  res.on('close', () => {
    clearInterval(heartbeatInterval)
    clients.delete(clientId)
    log.escrow.info({ clientId, totalClients: clients.size }, '← SSE cliente desconectado')
  })
...
let anteriorEstado: string | null = null

export async function broadcastSLASnapshot(): Promise<void> {
  ...
  if (anteriorEstado && anteriorEstado !== snap.estado) {
    ...
    log.escrow.warn({ alerta }, '⚠ SLA estado cambió')
  }
...
export async function getSLASnapshotRT(
  endpoint: string = ENDPOINT_XREF
): Promise<SLASnapshotRT & {
  pollingInterval: number   // ms recomendados entre polls
...
export function iniciarBroadcastCron(intervalMs = 15_000): void {
  if (broadcastTimer) return   // ya iniciado
  broadcastTimer = setInterval(async () => {
    await broadcastSLASnapshot().catch(() => {})
  }, intervalMs)
  log.escrow.info({ intervalMs }, '✅ SLA broadcast cron iniciado')
}

export function detenerBroadcastCron(): void {
  if (broadcastTimer) {
...
export function getActiveSSEClients(): number { return clients.size }
