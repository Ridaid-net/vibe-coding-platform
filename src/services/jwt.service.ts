...
export interface TokenPair {
  accessToken:   string
  refreshToken:  string
  expiresIn:     number   // segundos hasta expirar el access token
  tokenType:     'Bearer'
}
...
const ACCESS_EXPIRY_SEC  = parseExpiry(env.JWT_ACCESS_EXPIRES  ?? '15m')
const REFRESH_EXPIRY_SEC = parseExpiry(env.JWT_REFRESH_EXPIRES ?? '7d')
...
export async function verifyAndExtractToken(token: string): Promise<JWTPayload & { jti: string }> {
  ...
  if (!decoded.jti) throw new AppError('Token inválido (sin jti)', 401, 'TOKEN_INVALID')

  // Verificar blacklist (best-effort: si la DB falla, no bloqueamos)
  try {
    const blacklisted = await queryOne<{ jti: string }>(
      `SELECT jti FROM token_blacklist WHERE jti = $1::uuid AND expires_at > NOW()`,
      [decoded.jti]
    )
    if (blacklisted) {
    ...
    log.auth.warn({ err }, 'Blacklist check failed — allowing token')
  }
...
export async function revokeAccessToken(
  ...
  await query(
    `INSERT INTO token_blacklist (jti, usuario_id, expires_at, reason)
     VALUES ($1::uuid, $2, $3, $4)
...
export async function issueRefreshToken(
  ...
  const familyId   = crypto.randomUUID()   // nueva familia para esta sesión
...
export async function saveRefreshToken(userId: string, token: string): Promise<void> {
  ...
  const exists = await queryOne<{ id: string }>(
    `SELECT id FROM refresh_tokens WHERE token = $1`, [token]
  )
  if (!exists) {
    const expiresAt = new Date(Date.now() + REFRESH_EXPIRY_SEC * 1000)
    await query(
...
export async function rotateRefreshToken(
  ...
  let familyToRevoke: { familyId: string; userId: string } | null = null

  try {
    const result = await transaction(async (client) => {

    // 1. Buscar el token (activo O revocado — necesitamos ver ambos para detectar reúso)
    const found = await client.query<RefreshTokenRow>(
      `SELECT id, usuario_id, family_id, expires_at, revoked, revoke_reason
       FROM refresh_tokens
       WHERE token = $1`,
      [oldToken]
    )
    ...
    if (rt.revoked) {
      ...
      throw Object.assign(
        new AppError('Sesión comprometida por seguridad. Iniciá sesión nuevamente.', 401, 'TOKEN_REUSE_DETECTED'),
        { _familyId: compromisedFamily, _userId: compromisedUserId }
      )
    }
    ...
    if (err instanceof AppError && err.code === 'TOKEN_REUSE_DETECTED') {
      ...
      if (anyErr._familyId) {
        ...
        await query(
          ...
          [anyErr._familyId]
        ).catch(e => log.auth.error({ e }, 'Error revocando familia'))
      }
    }
    throw err
  }
...
export async function revokeAllUserTokens(
  ...
  if (count > 0) {
    log.auth.info({ userId, count, reason }, 'Sesiones revocadas')
  }
  return count
}

// ══════════════════════════════════════════════════════════
// SESIONES — consultas y gestión
// ══════════════════════════════════════════════════════════
...
export async function purgeExpiredTokens(): Promise<{ refreshTokens: number; blacklistEntries: number }> {
  ...
  if (refreshTokens > 0 || blacklistEntries > 0) {
    log.auth.info({ refreshTokens, blacklistEntries }, 'Tokens expirados purgados')
  }
 ...
 */
export async function buildTokenPairInspector(
  userId:   string,
  email:    string,
  ctx:      SessionContext = {}
): Promise<TokenPair> {
  const { query: dbQuery, queryOne: dbQueryOne } = await import('./jwt.service').then(() =>
    import('../config/database')
  )

  // Leer perfil del inspector para incluir en el token
  const insp = await dbQueryOne<{
    id: string; taller_aliado_id: string; taller_nombre: string; activo: boolean; habilitado: boolean
  }>(
  ...
  if (!insp) throw Object.assign(
    new Error('Sin perfil de inspector'), { code: 'NO_INSPECTOR_PROFILE', status: 403 }
  )
  if (!insp.activo) throw Object.assign(
    new Error('Inspector inactivo'), { code: 'INSPECTOR_INACTIVE', status: 403 }
  )
 ...
 */
export async function buildTokenPairAliado(
  userId:   string,
  email:    string,
  ctx:      SessionContext = {}
): Promise<TokenPair> {
  ...
  const taller = await dbQueryOne<{
    id: string; nombre: string; habilitado: boolean; activo: boolean
  }>(
    `SELECT id, nombre, habilitado, activo FROM talleres_aliados
     WHERE propietario_id=$1 AND activo=TRUE LIMIT 1`,
    [userId]
  )

  if (!taller) throw Object.assign(
    new Error('Sin taller aliado vinculado'), { code: 'NO_TALLER_ALIADO', status: 403 }
  )
