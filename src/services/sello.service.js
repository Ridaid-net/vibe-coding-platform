...
exports.generarCodigoCorto = generarCodigoCorto;
exports.sellarDocumento = sellarDocumento;
exports.verificarSello = verificarSello;
exports.buscarPorCodigo = buscarPorCodigo;
exports.getSelloCIT = getSelloCIT;
 ...
 */
function generarCodigoVerificacion(numeroCIT, documentoHash, citId, timestamp) {
    const hashA = documentoHash.slice(0, 6).toUpperCase();
    const hashB = crypto_1.default.createHash('sha256').update(citId).digest('hex').slice(0, 6).toUpperCase();
 ...
 */
function generarCodigoCorto(documentoHash, citId, timestamp) {
...
// ══════════════════════════════════════════════════════════
function buildTSPRequest(documentoHash) {
    // Crear nonce aleatorio (64 bits)
    const nonce = crypto_1.default.randomBytes(8);
    ...
    const request = node_forge_1.default.asn1.create(node_forge_1.default.asn1.Class.UNIVERSAL, node_forge_1.default.asn1.Type.SEQUENCE, true, [
        // version: INTEGER v1(1)
        node_forge_1.default.asn1.create(node_forge_1.default.asn1.Class.UNIVERSAL, node_forge_1.default.asn1.Type.INTEGER, false, node_forge_1.default.asn1.integerToDer(1).getBytes()),
        // messageImprint: SEQUENCE { hashAlgorithm, hashedMessage }
        node_forge_1.default.asn1.create(node_forge_1.default.asn1.Class.UNIVERSAL, node_forge_1.default.asn1.Type.SEQUENCE, true, [
            // hashAlgorithm: AlgorithmIdentifier { OID, NULL }
            node_forge_1.default.asn1.create(node_forge_1.default.asn1.Class.UNIVERSAL, node_forge_1.default.asn1.Type.SEQUENCE, true, [
                node_forge_1.default.asn1.create(node_forge_1.default.asn1.Class.UNIVERSAL, node_forge_1.default.asn1.Type.OID, false, node_forge_1.default.asn1.oidToDer(OID_SHA256).getBytes()),
                node_forge_1.default.asn1.create(node_forge_1.default.asn1.Class.UNIVERSAL, node_forge_1.default.asn1.Type.NULL, false, ''),
            ]),
            // hashedMessage: OCTET STRING (SHA-256 del documento)
            node_forge_1.default.asn1.create(node_forge_1.default.asn1.Class.UNIVERSAL, node_forge_1.default.asn1.Type.OCTETSTRING, false, hashBytes.toString('binary')),
        ]),
        // nonce: INTEGER (8 bytes aleatorio)
        node_forge_1.default.asn1.create(node_forge_1.default.asn1.Class.UNIVERSAL, node_forge_1.default.asn1.Type.INTEGER, false, nonce.toString('binary')),
        // certReq: BOOLEAN TRUE (queremos el certificado de la TSA)
        node_forge_1.default.asn1.create(node_forge_1.default.asn1.Class.UNIVERSAL, node_forge_1.default.asn1.Type.BOOLEAN, false, String.fromCharCode(0xff)),
    ]);
    return Buffer.from(node_forge_1.default.asn1.toDer(request).getBytes(), 'binary');
}
function parseTSTInfo(tstDER) {
    try {
        ...
        const asn1 = node_forge_1.default.asn1.fromDer(tstDER.toString('binary'));
        const respSeq = asn1;
        ...
        //   ...
        // }
        const tstInfoASN1 = node_forge_1.default.asn1.fromDer(eContent);
        const tstInfoValue = tstInfoASN1.value;
        const policy = node_forge_1.default.asn1.derToOid(tstInfoValue[1]?.value ?? '');
        const hashAlg = node_forge_1.default.asn1.derToOid(tstInfoValue[2]?.value[0]?.value[0]?.value ?? '');
        ...
        logger_1.log.sello.warn({ err: err.message }, 'Error parseando TSTInfo');
        return null;
    }
}
// ══════════════════════════════════════════════════════════
...
// ══════════════════════════════════════════════════════════
async function generarSelloStub(documentoHash, timestamp) {
    ...
    const hashBytes = Buffer.from(documentoHash, 'hex');
    const serialNum = crypto_1.default.randomBytes(8).toString('hex').toUpperCase();
    const genTimeStr = timestamp.toISOString()
        .replace(/[-T:]/g, '').replace(/\.\d+Z/, 'Z'); // → "20260526201234Z"
    // TSTInfo ASN.1
    const tstInfo = node_forge_1.default.asn1.create(node_forge_1.default.asn1.Class.UNIVERSAL, node_forge_1.default.asn1.Type.SEQUENCE, true, [
        // version v1(1)
        node_forge_1.default.asn1.create(node_forge_1.default.asn1.Class.UNIVERSAL, node_forge_1.default.asn1.Type.INTEGER, false, node_forge_1.default.asn1.integerToDer(1).getBytes()),
        // policy: RODAID stub OID (1.3.6.1.4.1.99999.1.1 — private arc placeholder)
        node_forge_1.default.asn1.create(node_forge_1.default.asn1.Class.UNIVERSAL, node_forge_1.default.asn1.Type.OID, false, node_forge_1.default.asn1.oidToDer('1.3.6.1.4.1.99999.1.1').getBytes()),
        ...
        // serialNumber
        node_forge_1.default.asn1.create(node_forge_1.default.asn1.Class.UNIVERSAL, node_forge_1.default.asn1.Type.INTEGER, false, Buffer.from(serialNum, 'hex').toString('binary')),
        // genTime: GeneralizedTime
        node_forge_1.default.asn1.create(node_forge_1.default.asn1.Class.UNIVERSAL, 24 /* GeneralizedTime */, false, genTimeStr),
    ]);
    const tstInfoDER = Buffer.from(node_forge_1.default.asn1.toDer(tstInfo).getBytes(), 'binary');
    // Firmar el TSTInfo con la clave RODAID (RSA-SHA256)
    const sig = crypto_1.default.createSign('RSA-SHA256');
    sig.update(tstInfoDER);
    const signature = sig.sign(node_forge_1.default.pki.privateKeyToPem(privateKey));
    // Empaquetar como ContentInfo → SignedData (simplificado)
    const stubTST = node_forge_1.default.asn1.create(node_forge_1.default.asn1.Class.UNIVERSAL, node_forge_1.default.asn1.Type.SEQUENCE, true, [
        // status: SEQUENCE { status: 0 (GRANTED) }
        node_forge_1.default.asn1.create(node_forge_1.default.asn1.Class.UNIVERSAL, node_forge_1.default.asn1.Type.SEQUENCE, true, [
            node_forge_1.default.asn1.create(node_forge_1.default.asn1.Class.UNIVERSAL, node_forge_1.default.asn1.Type.INTEGER, false, node_forge_1.default.asn1.integerToDer(0).getBytes()),
        ]),
        // timeStampToken: CONTEXT[0] { TSTInfo, signature, certSerial }
        node_forge_1.default.asn1.create(node_forge_1.default.asn1.Class.CONTEXT_SPECIFIC, 0, true, [
            tstInfo,
            // signature raw
            node_forge_1.default.asn1.create(node_forge_1.default.asn1.Class.UNIVERSAL, node_forge_1.default.asn1.Type.OCTETSTRING, false, signature.toString('binary')),
            // cert serial
            node_forge_1.default.asn1.create(node_forge_1.default.asn1.Class.UNIVERSAL, node_forge_1.default.asn1.Type.UTF8, false, certificate.serialNumber),
        ]),
    ]);
    return Buffer.from(node_forge_1.default.asn1.toDer(stubTST).getBytes(), 'binary');
}
// ══════════════════════════════════════════════════════════
// FUNCIÓN PRINCIPAL — solicitar sello temporal
// ══════════════════════════════════════════════════════════
async function sellarDocumento(input) {
    const t0 = Date.now();
    // Idempotencia
    const existente = await (0, database_1.queryOne)(`SELECT id, codigo_verif, sellado_en, modo, tst_hex, tsa_url
     FROM sellos_temporales
     WHERE cit_id=$1 AND documento_hash=$2`, [input.citId, input.documentoHash]);
    if (existente) {
        logger_1.log.sello.info({ citId: input.citId, id: existente.id }, '✓ Sello reutilizado (idempotente)');
        return {
    ...
    const tsaUrl = env_1.env.RODAID_TSA_URL ?? null;
    let modo = 'STUB';
    let tstDER = null;
    let selladoEn = new Date();
    let tsaSerial = null;
    let tsaPolicy = null;
    // ── Intentar TSA real ──────────────────────────────────────
    if (tsaUrl) {
        try {
            ...
            logger_1.log.sello.warn({ tsaUrl, err: err.message }, 'TSA falló — usando sello STUB local');
            tstDER = null;
        }
    }
    ...
    const codigoVerif = generarCodigoVerificacion(input.numeroCIT, input.documentoHash, input.citId, selladoEn);
    const codigoCorto = generarCodigoCorto(input.documentoHash, input.citId, selladoEn);
    const tstHex = tstDER.toString('hex');
    const ms = Date.now() - t0;
    // ── Persistir en DB ───────────────────────────────────────
    const row = await (0, database_1.queryOne)(`INSERT INTO sellos_temporales
    ...
    await (0, database_1.query)(`UPDATE cits
     ...
     WHERE id=$1`, [input.citId, row?.id, codigoVerif, selladoEn]);
    logger_1.log.sello.info({
...
// ══════════════════════════════════════════════════════════
async function verificarSello(pdfBuffer, codigoVerif, citId) {
    const pdfHash = crypto_1.default.createHash('sha256').update(pdfBuffer).digest('hex');
    // Buscar el sello en DB
    let sello = null;
    if (citId) {
        sello = await (0, database_1.queryOne)(`SELECT codigo_verif, sellado_en, modo, tst_hex, tsa_url, documento_hash
       FROM sellos_temporales WHERE cit_id=$1 ORDER BY creado_en DESC LIMIT 1`, [citId]);
    }
    else if (codigoVerif) {
        sello = await (0, database_1.queryOne)(`SELECT codigo_verif, sellado_en, modo, tst_hex, tsa_url, documento_hash
       FROM sellos_temporales WHERE codigo_verif=$1`, [codigoVerif]);
    }
    if (!sello) {
        return {
...
// ══════════════════════════════════════════════════════════
async function buscarPorCodigo(codigoVerif) {
    return (0, database_1.queryOne)(`SELECT id, cit_id, codigo_verif, sellado_en, modo, tsa_url, documento_hash
     FROM sellos_temporales WHERE codigo_verif=$1`, [codigoVerif]);
}
async function getSelloCIT(citId) {
    return (0, database_1.queryOne)(`SELECT id, codigo_verif, sellado_en, modo, tsa_url, tst_hex, documento_hash
     FROM sellos_temporales WHERE cit_id=$1 ORDER BY creado_en DESC LIMIT 1`, [citId]);
}
