...
export type EstadoEfectivo =
  | 'VIGENTE'               // activo, pagado, con NFT, no vencido
  | 'VIGENTE_SIN_NFT'       // activo y pagado pero NFT pendiente de mint
  | 'VIGENTE_SIN_TASA'      // activo pero tasa no acreditada
  | 'EXPIRADO'              // fecha_vencimiento < hoy
  | 'VENCE_PRONTO'          // vence en < 60 días
  | 'LISTO_PARA_PAGO'       // borrador, puntaje ≥ 16, puede pagar tasa
  | 'INSPECCION_INCOMPLETA' // borrador, puntaje < 16 o fotos insuficientes
  | 'PAGO_PENDIENTE'        // tasa MxM iniciada pero no confirmada
  | 'RECHAZADO'             // inspector rechazó
  | 'BLOQUEADO'             // alerta activa (denuncia de robo)

export interface CITEstadoResponse {
  // Identificación
  id:               string
  numeroCIT:        string

  // Estado calculado
  estadoBase:       string           // campo 'estado' de DB
  estadoEfectivo:   EstadoEfectivo
  estadoLabel:      string           // texto legible para UI
  estadoColor:      'verde' | 'amarillo' | 'rojo' | 'gris' | 'azul'
  vigente:          boolean
  ...
  inspeccion: {
    puntosTotal:  number
    puntosMax:    number
    puntosMin:    number          // 16/20 para aprobar
    aprobada:     boolean
    fotosCount:   number
    geoLat:       number | null
    geoLng:       number | null
  ...
  tasa: {
    pagada:           boolean
    mxmPagoId:        string | null
    mxmEstado:        string | null
    montoARS:         number
  ...
  fechas: {
    ...
    diasVigencia:     number | null   // días hasta vencimiento (negativo si expirado)
    creado:           string
    actualizado:      string
  }
  ...
  alertas: Array<{
    tipo:   string
    mensaje:string
    accion: string            // endpoint sugerido para resolver
  }>
...
export async function getCITEstado(
  idOrNumero: string   // UUID o numero_cit como RCIT-2026-00041
): Promise<CITEstadoResponse | null> {
  ...
  let estadoColor:    CITEstadoResponse['estadoColor']
  let vigente = false

  if (denuncia) {
    estadoEfectivo = 'BLOQUEADO'; estadoLabel = 'Bloqueado — alerta activa'; estadoColor = 'rojo'
  } else if (row.estado === 'RECHAZADO') {
    estadoEfectivo = 'RECHAZADO'; estadoLabel = 'Rechazado por inspector'; estadoColor = 'rojo'
  } else if (row.estado === 'PAGO_PENDIENTE') {
    estadoEfectivo = 'PAGO_PENDIENTE'; estadoLabel = 'Pago de tasa en proceso'; estadoColor = 'amarillo'
    ...
    if (expirado) {
      estadoEfectivo = 'EXPIRADO'; estadoLabel = 'Vencido'; estadoColor = 'rojo'
    } else if (!row.tasa_pagada) {
      estadoEfectivo = 'VIGENTE_SIN_TASA'; estadoLabel = 'Activo — tasa pendiente'; estadoColor = 'amarillo'; vigente = true
    } else if (!row.nft_token_id) {
      estadoEfectivo = 'VIGENTE_SIN_NFT'; estadoLabel = 'Activo — NFT pendiente'; estadoColor = 'azul'; vigente = true
    } else if (vencePronto) {
      estadoEfectivo = 'VENCE_PRONTO'; estadoLabel = `Vigente — vence en ${diasVigencia} días`; estadoColor = 'amarillo'; vigente = true
    } else {
      estadoEfectivo = 'VIGENTE'; estadoLabel = 'Vigente'; estadoColor = 'verde'; vigente = true
    }
  } else {
    estadoEfectivo = 'INSPECCION_INCOMPLETA'; estadoLabel = row.estado; estadoColor = 'gris'
  }

  // ── Alertas ─────────────────────────────────────────────
  const alertas: CITEstadoResponse['alertas'] = []

  if (denuncia) alertas.push({
    tipo:    'ROBO',
    mensaje: `Denuncia activa: ${row.denuncia_motivo}`,
    accion:  `GET /cit/${row.id}/alertas`,
  })
  if (expirado) alertas.push({
    tipo:    'VENCIMIENTO',
    mensaje: `CIT vencido hace ${Math.abs(diasVigencia!)} días. Requiere nueva inspección.`,
    accion:  `POST /inspector/cit/nuevo`,
  })
  if (vencePronto && !expirado) alertas.push({
    tipo:    'VENCIMIENTO_PROXIMO',
    mensaje: `El CIT vence en ${diasVigencia} días (${row.fecha_vencimiento?.slice(0,10)}).`,
    accion:  `POST /inspector/cit/renovar`,
  })
  if (row.estado === 'ACTIVO' && !row.tasa_pagada) alertas.push({
    tipo:    'TASA_PENDIENTE',
...
export async function getCITEstadoPorNumero(numeroCIT: string) {
