...
exports.liberarFondosTx = liberarFondosTx;
exports.reembolsarCompradorTx = reembolsarCompradorTx;
exports.reprocesarLiberacionesPendientes = reprocesarLiberacionesPendientes;
exports.getLiberacion = getLiberacion;
exports.getComprobanteCompleto = getComprobanteCompleto;
exports.getResumenLiberaciones = getResumenLiberaciones;
...
// FUNCIÓN PRINCIPAL
// ══════════════════════════════════════════════════════════
async function liberarFondosTx(input) {
    // 1. Idempotencia — una sola liberación por transacción
    const yaLiberado = await (0, database_1.queryOne)(`SELECT id, comprobante_id, estado, monto_liberado
     FROM liberaciones_fondos WHERE transaccion_id=$1 AND NOT es_reembolso
     LIMIT 1`, [input.transaccionId]);
    if (yaLiberado) {
        logger_1.log.escrow.warn({ txId: input.transaccionId.slice(0, 8), comp: yaLiberado.comprobante_id }, '⚠ Liberación ya existente — retornando idempotente');
        return construirResultadoExistente(yaLiberado, input);
    }
    // 2. Generar comprobante_id correlativo
    const seq = await (0, database_1.queryOne)(`SELECT COUNT(*)::text AS count FROM liberaciones_fondos WHERE NOT es_reembolso`, []);
    ...
    const [vendedor, comprador] = await Promise.all([
        (0, database_1.queryOne)(`SELECT email, nombre FROM usuarios WHERE id=$1`, [input.vendedorId]),
        (0, database_1.queryOne)(`SELECT email, nombre FROM usuarios WHERE id=$1`, [input.compradorId]),
    ]);
    ...
    try {
        await (0, database_1.transaction)(async (client) => {
            // Insertar liberación
            const lib = await client.query(`INSERT INTO liberaciones_fondos
         ...
         RETURNING id`, [
           ...
           actualizado_en=NOW()
         WHERE id=$1 AND estado_pago IN ('EN_CAMINO','FONDOS_RETENIDOS')`, [input.transaccionId]);
        });
    }
    catch (err) {
        throw new errorHandler_1.AppError(`Error registrando liberación: ${err.message}`, 500, 'LIBERACION_ERROR');
    }
    ...
    if (input.gateway === 'MERCADOPAGO' && input.mpPaymentId) {
        try {
            ...
            if (result?.ok) {
                ...
                await (0, database_1.query)(`UPDATE liberaciones_fondos SET
             estado='ACREDITADO', mp_release_id=$2, acreditado_en=NOW()
           WHERE id=$1`, [liberacionId, mpReleaseId ?? null]);
            }
        }
        ...
        await (0, database_1.query)(`UPDATE liberaciones_fondos SET estado='ACREDITADO', acreditado_en=NOW() WHERE id=$1`, [liberacionId]);
    }
...
// ══════════════════════════════════════════════════════════
async function reembolsarCompradorTx(opts) {
    // Idempotencia
    const yaReembolsado = await (0, database_1.queryOne)(`SELECT id, comprobante_id FROM liberaciones_fondos
     WHERE transaccion_id=$1 AND es_reembolso=TRUE LIMIT 1`, [opts.transaccionId]);
    if (yaReembolsado) {
        return { reembolsoId: yaReembolsado.id, comprobanteId: yaReembolsado.comprobante_id, ok: true };
    }
    const seq = await (0, database_1.queryOne)(`SELECT COUNT(*)::text AS count FROM liberaciones_fondos WHERE es_reembolso=TRUE`, []);
    const comprobanteId = `REM-${new Date().getFullYear()}-${String(parseInt(seq?.count ?? '0') + 1).padStart(5, '0')}`;
    const lib = await (0, database_1.queryOne)(`INSERT INTO liberaciones_fondos
    ...
    await (0, database_1.query)(`UPDATE liberaciones_fondos SET
     ...
     WHERE id=$1`, [lib.id, ok ? 'ACREDITADO' : 'PENDIENTE']);
    // Notificar al comprador
    import('./device_token.service').then(dt => dt.enviarPush(opts.compradorId, {
...
async function notificarVendedor(opts) {
    ...
    // Push FCM
    import('./device_token.service').then(dt => dt.enviarPush(opts.vendedorId, {
...
// ══════════════════════════════════════════════════════════
async function reprocesarLiberacionesPendientes() {
    const pendientes = await (0, database_1.query)(`SELECT id, transaccion_id, vendedor_id, monto_liberado, gateway, mp_payment_id
...
// CONSULTAS
// ══════════════════════════════════════════════════════════
async function getLiberacion(transaccionId) {
    return (0, database_1.queryOne)(`SELECT lf.*, u_v.email AS vendedor_email, u_v.nombre AS vendedor_nombre,
     ...
     ORDER BY lf.creado_en DESC LIMIT 1`, [transaccionId]);
}
async function getComprobanteCompleto(comprobanteId) {
    const lib = await (0, database_1.queryOne)(`SELECT lf.*,
     ...
     WHERE lf.comprobante_id=$1`, [comprobanteId]);
    if (!lib)
        return null;
    return {
...
async function getResumenLiberaciones(usuarioId, rol, dias = 90) {
    ...
    return (0, database_1.queryOne)(`SELECT COUNT(*)::int                                            AS total,
     ...
     FROM liberaciones_fondos
     WHERE ${campo}=$1 AND creado_en > NOW()-($2||' days')::interval`, [usuarioId, dias]);
}
// ══════════════════════════════════════════════════════════
// HELPER
// ══════════════════════════════════════════════════════════
function construirResultadoExistente(lib, input) {
    return {
        liberacionId: lib.id,
        comprobanteId: lib.comprobante_id,
        estado: lib.estado,
