...
exports.getAccessTokenConRenovacion = getAccessTokenConRenovacion;
exports.renovarTokensProximos = renovarTokensProximos;
exports.invalidarToken = invalidarToken;
exports.getEstadoTokens = getEstadoTokens;
exports.getHistorialRenovaciones = getHistorialRenovaciones;
exports.getEstadisticasRenovaciones = getEstadisticasRenovaciones;
...
const MODO_STUB = !env_1.env.MXM_CLIENT_ID || !env_1.env.MXM_CLIENT_SECRET;
const lockKey = (userId) => `mxm:refresh_lock:${userId}`;
const cacheKey = (userId) => `mxm:access_token:${userId}`;
// ══════════════════════════════════════════════════════════
// FUNCIÓN PRINCIPAL: getAccessToken con renovación proactiva
// ══════════════════════════════════════════════════════════
async function getAccessTokenConRenovacion(userId) {
    ...
    // 2. Leer token de DB
    const row = await (0, database_1.queryOne)(`SELECT access_token, refresh_token, expires_at, renovando
     FROM mxm_tokens WHERE usuario_id=$1`, [userId]);
    if (!row)
        return { token: null, origen: 'sin_token', expiraEn: null, renovado: false };
    ...
    if (!row.refresh_token) {
        logger_1.log.mxm.warn({ userId: userId.slice(0, 8), minHastaExp: minHastaExp.toFixed(1) }, 'Token MxM por vencer sin refresh_token — usuario debe re-autenticar');
...
// ══════════════════════════════════════════════════════════
async function renovarConLock(userId, refreshToken, minHastaExp) {
    const redis = (0, redis_1.getRedis)();
    const lk = lockKey(userId);
    // Intentar adquirir lock (SET NX EX)
    const lockAcquired = await redis.set(lk, '1', 'EX', LOCK_TTL_SEC, 'NX');
    if (!lockAcquired) {
        logger_1.log.mxm.debug({ userId: userId.slice(0, 8) }, 'Renovación en curso por otro proceso — esperando');
...
async function waitForLockRelease(lk, redis, maxMs = 5000) {
...
// ══════════════════════════════════════════════════════════
async function renovarConReintentos(userId, _refreshToken) {
    for (let intento = 0; intento < MAX_REINTENTOS; intento++) {
        if (intento > 0)
            await new Promise(r => setTimeout(r, BACKOFF_MS[intento] ?? 6000));
        try {
            ...
            if (expMs / 60_000 < MIN_TTL_MIN) {
                logger_1.log.mxm.warn({ userId: userId.slice(0, 8), expMs }, 'Token MxM con TTL sospechosamente corto');
            }
            ...
            await (0, database_1.query)(`INSERT INTO mxm_token_renovaciones
           (usuario_id, resultado, access_expira_en, intentos)
         VALUES ($1,'OK',$2,$3)`, [userId, expDate, intento + 1]).catch(() => { });
            ...
            logger_1.log.mxm.warn({ userId: userId.slice(0, 8), intento, err: msg }, `Renovación fallida (intento ${intento + 1})`);
            if (intento === MAX_REINTENTOS - 1) {
                await (0, database_1.query)(`INSERT INTO mxm_token_renovaciones (usuario_id, resultado, error_msg, intentos) VALUES ($1,'FALLIDO',$2,$3)`, [userId, msg.slice(0, 500), intento + 1]).catch(() => { });
                // Marcar el usuario para re-autenticación
                await (0, database_1.query)(`UPDATE usuarios SET mxm_verificado=FALSE WHERE id=$1`, [userId]).catch(() => { });
                logger_1.log.mxm.error({ userId: userId.slice(0, 8) }, '🚨 Refresh token MxM agotado — usuario requiere re-autenticación');
            }
        }
    }
    return false;
...
// ══════════════════════════════════════════════════════════
async function ejecutarRefresh(userId) {
    if (MODO_STUB) {
        // STUB: genera token rotante sin llamar al IdP
        const stubToken = `stub_access_${crypto_1.default.randomBytes(16).toString('hex')}`;
        const expiresIn = 3600; // 1 hora
        await (0, database_1.query)(`UPDATE mxm_tokens SET
         access_token   = $2,
         expires_at     = NOW() + INTERVAL '1 hour',
         actualizado_en = NOW()
       WHERE usuario_id = $1`, [userId, stubToken]);
        logger_1.log.mxm.warn({ userId: userId.slice(0, 8) }, '⚠ STUB: token MxM renovado sintéticamente');
        return { access_token: stubToken, expires_in: expiresIn };
    }
    // REAL: usar mxmService.refreshToken()
    const tokens = await mxm_service_1.mxmService.refreshToken(userId);
    if (!tokens)
        return null;
...
// ══════════════════════════════════════════════════════════
async function renovarTokensProximos(opts) {
    ...
    const usuarios = await (0, database_1.query)(`SELECT usuario_id, expires_at,
            EXTRACT(EPOCH FROM (expires_at - NOW()))/60 AS minutos_restantes
     FROM mxm_tokens
     WHERE refresh_token IS NOT NULL
       AND expires_at < NOW() + ($1 || ' minutes')::interval
       AND renovando = FALSE
     ORDER BY expires_at`, [buffer]);
    if (usuarios.length === 0)
        return { procesados: 0, renovados: 0, fallidos: 0, omitidos: 0 };
    logger_1.log.mxm.info({ count: usuarios.length, buffer }, `🔄 Iniciando renovación proactiva de ${usuarios.length} tokens MxM`);
    let renovados = 0;
    let fallidos = 0;
    let omitidos = 0;
    for (const u of usuarios) {
        // Marcar como "renovando" para evitar dobles procesados
        const marcado = await (0, database_1.query)(`UPDATE mxm_tokens SET renovando=TRUE WHERE usuario_id=$1 AND NOT renovando RETURNING usuario_id`, [u.usuario_id]);
        if (marcado.length === 0) {
            omitidos++;
            continue;
        }
        try {
            ...
            if (result.renovado)
                ...
                omitidos++; // ya fue renovado por otro proceso
            else
                fallidos++;
        }
        catch (err) {
            logger_1.log.mxm.error({ userId: u.usuario_id.slice(0, 8), err: err.message }, 'Error renovando token');
            fallidos++;
        }
        finally {
            await (0, database_1.query)(`UPDATE mxm_tokens SET renovando=FALSE WHERE usuario_id=$1`, [u.usuario_id]).catch(() => { });
        }
    }
    logger_1.log.mxm.info({ procesados: usuarios.length, renovados, fallidos, omitidos }, `✓ Renovación proactiva: ${renovados} renovados, ${fallidos} fallidos, ${omitidos} omitidos`);
    return { procesados: usuarios.length, renovados, fallidos, omitidos };
}
// ══════════════════════════════════════════════════════════
// INVALIDAR TOKEN (logout, desconexión, sospecha)
// ══════════════════════════════════════════════════════════
async function invalidarToken(userId, motivo) {
    const redis = (0, redis_1.getRedis)();
    await Promise.all([
        redis.del(cacheKey(userId)),
        redis.del(lockKey(userId)),
        (0, database_1.query)(`UPDATE mxm_tokens SET
         access_token   = '',
         refresh_token  = NULL,
    ...
    await (0, database_1.query)(`INSERT INTO mxm_token_renovaciones (usuario_id, resultado, error_msg)
     VALUES ($1,'OMITIDO',$2)`, [userId, motivo ? `Invalidado: ${motivo}` : 'Invalidado manualmente']).catch(() => { });
    logger_1.log.mxm.info({ userId: userId.slice(0, 8), motivo }, '🗑 Token MxM invalidado');
}
// ══════════════════════════════════════════════════════════
// CONSULTAS
// ══════════════════════════════════════════════════════════
async function getEstadoTokens() {
...
async function getHistorialRenovaciones(userId, limit = 20) {
    return (0, database_1.query)(`SELECT id, resultado, error_msg, access_expira_en, intentos, registrado_en
     FROM mxm_token_renovaciones WHERE usuario_id=$1 ORDER BY registrado_en DESC LIMIT $2`, [userId, limit]);
}
async function getEstadisticasRenovaciones(horas = 24) {
    const row = await (0, database_1.queryOne)(`SELECT
