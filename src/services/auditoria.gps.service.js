...
exports.auditarInspeccionGPS = auditarInspeccionGPS;
exports.getAnomaliasPendientes = getAnomaliasPendientes;
exports.resolverAnomalia = resolverAnomalia;
exports.getEstadisticasAuditoria = getEstadisticasAuditoria;
exports.getHistorialAuditoriaInspector = getHistorialAuditoriaInspector;
...
// AUDITORÍA PRINCIPAL
// ══════════════════════════════════════════════════════════
async function auditarInspeccionGPS(input) {
    const inicio = Date.now();
    const anomalias = [];
    let scoreRiesgo = 0;
    // ── Cargar datos necesarios ──────────────────────────
    const [taller, ultimaInsp, citsHora, historialIPs, historialDevices] = await Promise.all([
        (0, database_1.queryOne)(`SELECT lat, lng, nombre FROM talleres_aliados WHERE id=$1`, [input.tallerId]),
        (0, database_1.queryOne)(`SELECT insp_lat, insp_lng, auditado_en, cit_id
       FROM auditoria_cit WHERE inspector_id=$1 AND insp_lat IS NOT NULL
       ORDER BY auditado_en DESC LIMIT 1`, [input.inspectorId]),
        (0, database_1.queryOne)(`SELECT COUNT(*)::text AS count FROM auditoria_cit
       WHERE inspector_id=$1 AND auditado_en > NOW() - INTERVAL '1 hour'`, [input.inspectorId]),
        (0, database_1.query)(`SELECT ip_address::text AS ip FROM cits
       WHERE inspector_id=(SELECT usuario_id FROM inspectores WHERE id=$1)
         AND insp_ip IS NOT NULL
       GROUP BY ip_address ORDER BY MAX(creado_en) DESC LIMIT 10`, [input.inspectorId]),
        (0, database_1.query)(`SELECT insp_device_id AS device_id FROM cits
       WHERE inspector_id=(SELECT usuario_id FROM inspectores WHERE id=$1)
         AND insp_device_id IS NOT NULL
       GROUP BY insp_device_id ORDER BY MAX(creado_en) DESC LIMIT 10`, [input.inspectorId]),
    ]);
    ...
    if (ultimaInsp?.insp_lat != null && ultimaInsp?.insp_lng != null &&
        ...
        const distDesdeUltima = haversineKm(ultimaInsp.insp_lat, ultimaInsp.insp_lng, input.inspLat, input.inspLng);
    ...
    const accion = riesgo === 'CRITICO' ? 'SUSPENDER'
    ...
    // ── Persistir en DB ─────────────────────────────────
    const row = await (0, database_1.queryOne)(`INSERT INTO auditoria_cit
    ...
    if (riesgo === 'CRITICO') {
        await (0, database_1.query)(`UPDATE cits SET estado='BLOQUEADO', actualizado_en=NOW() WHERE id=$1 AND estado='PENDIENTE'`, [input.citId]).catch(() => { });
        logger_1.log.gps.error({
...
// ══════════════════════════════════════════════════════════
async function getAnomaliasPendientes(opts) {
    const pagina = Math.max(1, opts?.pagina ?? 1);
    const porPagina = Math.min(100, opts?.porPagina ?? 25);
    const offset = (pagina - 1) * porPagina;
    const conds = ['a.requiere_revision=TRUE', 'a.revisado_en IS NULL'];
    const params = [];
    let idx = 1;
    if (opts?.riesgo) {
        conds.push(`a.riesgo=$${idx++}`);
        params.push(opts.riesgo);
    }
    if (opts?.tallerId) {
        conds.push(`a.taller_id=$${idx++}`);
        params.push(opts.tallerId);
    }
    const where = conds.join(' AND ');
    const [items, count] = await Promise.all([
        (0, database_1.query)(`SELECT a.id, a.cit_id, a.riesgo, a.score_riesgo, a.anomalias,
       ...
       LIMIT $${idx} OFFSET $${idx + 1}`, [...params, porPagina, offset]),
        (0, database_1.queryOne)(`SELECT COUNT(*)::text AS count FROM auditoria_cit a WHERE ${where}`, params),
    ]);
    return { items, total: parseInt(count?.count ?? '0') };
}
async function resolverAnomalia(auditoriaId, opts) {
    await (0, database_1.query)(`UPDATE auditoria_cit SET
       resolucion=$2, revisado_por=$3::uuid, revisado_en=NOW(), requiere_revision=FALSE
     WHERE id=$1`, [auditoriaId, opts.resolucion, opts.revisadoPor]);
    // Si es FRAUDE → bloquear CIT
    if (opts.resolucion === 'FRAUDE') {
        const citId = await (0, database_1.queryOne)(`SELECT cit_id FROM auditoria_cit WHERE id=$1`, [auditoriaId]);
        if (citId) {
            await (0, database_1.query)(`UPDATE cits SET estado='BLOQUEADO', actualizado_en=NOW() WHERE id=$1`, [citId.cit_id]);
        }
    }
}
async function getEstadisticasAuditoria(dias = 30) {
    const [resumen, porRiesgo, topAnomalias] = await Promise.all([
       ...
       FROM auditoria_cit WHERE auditado_en > NOW()-($1||' days')::interval`, [dias]),
        (0, database_1.query)(`SELECT riesgo, COUNT(*)::int AS count
       FROM auditoria_cit WHERE auditado_en > NOW()-($1||' days')::interval
       GROUP BY riesgo ORDER BY count DESC`, [dias]),
        (0, database_1.query)(`SELECT UNNEST(anomalias) AS codigo, COUNT(*)::int AS count
       FROM auditoria_cit WHERE auditado_en > NOW()-($1||' days')::interval
       GROUP BY codigo ORDER BY count DESC LIMIT 8`, [dias]),
    ]);
    return { resumen, porRiesgo, topAnomalias };
}
async function getHistorialAuditoriaInspector(inspectorId, limite = 20) {
    return (0, database_1.query)(`SELECT id, cit_id, riesgo, score_riesgo, anomalias,
     ...
     ORDER BY auditado_en DESC LIMIT $2`, [inspectorId, limite]);
}
