...
exports.ENDPOINT_XREF = exports.UMBRAL_WARNING = exports.UMBRAL_OK = exports.VENTANA_H = exports.SLA_OBJETIVO_MS = void 0;
exports.slaMiddleware = slaMiddleware;
exports.registrarMetrica = registrarMetrica;
exports.calcularSLA72h = calcularSLA72h;
exports.getHistorialSLA = getHistorialSLA;
exports.getLatenciasRecientes = getLatenciasRecientes;
exports.getSLAStatus = getSLAStatus;
exports.resetearMetricas = resetearMetricas;
...
exports.SLA_OBJETIVO_MS = SLA_OBJETIVO_MS;
const VENTANA_H = 72; // ventana rolling
exports.VENTANA_H = VENTANA_H;
...
exports.ENDPOINT_XREF = ENDPOINT_XREF;
// Umbrales de estado
const UMBRAL_OK = 99.0;
exports.UMBRAL_OK = UMBRAL_OK;
const UMBRAL_WARNING = 95.0;
exports.UMBRAL_WARNING = UMBRAL_WARNING;
// ══════════════════════════════════════════════════════════
 ...
 */
function slaMiddleware(endpoint = ENDPOINT_XREF) {
    return function sla(req, res, next) {
        ...
        res.on('finish', () => {
            ...
            registrarMetrica({
                ...
                certSubject: req.mtlsClient?.cn,
                serial: req.body?.serial,
            }).catch(err => logger_1.log.minseg.warn({ err: err.message }, 'SLA metric error'));
        });
        next();
    };
}
...
// REGISTRAR MÉTRICA
// ══════════════════════════════════════════════════════════
async function registrarMetrica(opts) {
    const ahora = Date.now();
    const redis = (0, redis_1.getRedis)();
    // 1. Redis time-series (para cálculo rápido de percentiles sin DB)
    const key = `${REDIS_KEY_BASE}:${opts.endpoint.replace(/\s+/g, '_').replace(/\//g, '-')}`;
    const metrica = {
        ms: opts.latenciaMs,
    ...
    await (0, database_1.query)(`INSERT INTO endpoint_metrics
 ...
 */
async function calcularSLA72h(endpoint = ENDPOINT_XREF, forzarDB = false) {
    const redis = (0, redis_1.getRedis)();
    ...
    const rawItems = await redis.lrange(key, 0, -1).catch(() => []);
    ...
    if (rawItems.length > 0) {
        metricas = rawItems
            ...
            .filter((m) => m !== null && m.ts >= cutoff && !m.error);
    }
    else {
        // Fallback: leer de DB
        const rows = await (0, database_1.query)(`SELECT latencia_ms, creado_en FROM endpoint_metrics
       WHERE endpoint=$1 AND NOT error AND creado_en > NOW() - ($2||' hours')::interval
       ORDER BY creado_en DESC LIMIT 50000`, [endpoint, VENTANA_H]);
        metricas = rows.map(r => ({
    ...
    // Persistir en DB
    await (0, database_1.query)(`INSERT INTO sla_snapshots
...
// CÁLCULOS INTERNOS
// ══════════════════════════════════════════════════════════
function calcularSnapshot(endpoint, metricas) {
    ...
    const pct = (p) => ms[Math.min(Math.ceil(total * p / 100) - 1, total - 1)] ?? 0;
    const estado = cumplimiento >= UMBRAL_OK ? 'OK'
...
async function calcularTendencia(endpoint, metricas) {
    const bloques = [];
    const ahora = Date.now();
...
async function getHistorialSLA(endpoint = ENDPOINT_XREF, horas = 72) {
    return (0, database_1.query)(`SELECT cumplimiento, p99, p95, total, estado, calculado_en, latencia_avg
     FROM sla_snapshots WHERE endpoint=$1
       AND calculado_en > NOW()-($2||' hours')::interval
     ORDER BY calculado_en DESC LIMIT 200`, [endpoint, horas]);
}
/** Percentiles de latencia en tiempo real desde DB */
async function getLatenciasRecientes(endpoint = ENDPOINT_XREF, minutos = 60) {
    return (0, database_1.queryOne)(`SELECT COUNT(*)::int                                                          AS total,
     ...
     WHERE endpoint=$1 AND NOT error
       AND creado_en > NOW()-($3||' minutes')::interval`, [endpoint, SLA_OBJETIVO_MS, minutos]);
}
/** Estado SLA resumido para health check */
async function getSLAStatus(endpoint = ENDPOINT_XREF) {
...
async function resetearMetricas(endpoint = ENDPOINT_XREF) {
    const redis = (0, redis_1.getRedis)();
    const key = `${REDIS_KEY_BASE}:${endpoint.replace(/\s+/g, '_').replace(/\//g, '-')}`;
    ...
    await Promise.all([
        ...
        (0, database_1.query)(`DELETE FROM endpoint_metrics WHERE endpoint=$1`, [endpoint]).catch(() => { }),
        (0, database_1.query)(`DELETE FROM sla_snapshots WHERE endpoint=$1`, [endpoint]).catch(() => { }),
    ]);
    logger_1.log.minseg.info({ endpoint }, '🗑 Métricas SLA reseteadas');
}
