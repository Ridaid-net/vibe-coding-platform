...
export type SeveridadAnomalia = 'INFO' | 'ADVERTENCIA' | 'CRITICA'
...
export interface GPSCheckResult {
  coordsValidas:    boolean
  distanciaTaller:  number | null   // metros
  anomalias:        Anomalia[]
  hayAnomaliasCrit: boolean
  resumen:          string
}
...
async function checkFueraRango(
  input: CoordsInput,
  tallerLat: number,
  tallerLng: number
...
async function checkVelocidadImposible(input: CoordsInput): Promise<Anomalia | null> {
  ...
  const anterior = await queryOne<{
    lat: string; lng: string; registrado_en: Date
  }>(
    `SELECT lat::text, lng::text, registrado_en
  ...
  if (diffMin <= 0) return null   // mismo instante
...
async function checkClusterEstatico(input: CoordsInput): Promise<Anomalia | null> {
  ...
  const count = await queryOne<{ n: string }>(
    `SELECT COUNT(*)::text AS n
     ...
     WHERE inspector_id = $1
       ...
       AND ABS(lng - $3) < 0.0005`,
    [input.inspectorId, input.lat, input.lng]
  )
...
async function checkZonaNoHabitual(input: CoordsInput): Promise<Anomalia | null> {
  // Calcular centroide del historial del inspector (últimas 30 inspecciones)
  const historial = await queryOne<{
    lat_prom: string; lng_prom: string; n: string
  }>(
    `SELECT AVG(lat)::text AS lat_prom, AVG(lng)::text AS lng_prom, COUNT(*)::text AS n
     FROM inspector_gps_historia
     WHERE inspector_id = $1
  ...
  if (n < 5) return null  // insuficiente historial para análisis
...
async function checkPatronSospechoso(input: CoordsInput): Promise<Anomalia | null> {
  const resCrit = await queryOne<{ n: string }>(
    `SELECT COUNT(*)::text AS n
     FROM gps_anomalias
     WHERE inspector_id = $1
       AND severidad = 'CRITICA'
       AND creado_en > NOW() - INTERVAL '${PATRON_DIAS} days'`,
    [input.inspectorId]
  )
...
export async function checkGPSAnomalia(input: CoordsInput): Promise<GPSCheckResult> {
  const ts = input.timestamp ?? new Date()

  // Cargar datos del taller aliado
  const taller = await queryOne<{
    lat: string | null; lng: string | null; nombre: string; localidad: string
  }>(
    `SELECT lat::text, lng::text, nombre, localidad
     FROM talleres_aliados WHERE id = $1`,
  ...
  let distanciaTaller: number | null = null
  ...
  if (tallerLat !== null && tallerLng !== null) {
    ...
    if (rangoCheck) anomalias.push(rangoCheck)
  } else {
    log.gps.warn({ tallerAliadoId: input.tallerAliadoId }, 'Taller sin coordenadas GPS — check FUERA_RANGO omitido')
  }

  if (velCheck)     anomalias.push(velCheck)
  if (clusterCheck) anomalias.push(clusterCheck)
  if (zonaCheck)    anomalias.push(zonaCheck)
  if (patronCheck)  anomalias.push(patronCheck)
  ...
  if (anomalias.length > 0) {
    for (const a of anomalias) {
      ...
      ).catch(err => log.gps.warn({ err: err.message }, 'gps_anomalias insert error'))
    }
  }

  // ── Guardar en historial GPS ─────────────────────────────
  await query(
    `INSERT INTO inspector_gps_historia (inspector_id, cit_id, lat, lng, registrado_en)
     VALUES ($1, $2, $3, $4, $5)`,
    [input.inspectorId, input.citId, input.lat, input.lng, ts]
  ).catch(() => {})
...
export async function getAnomaliasPendientes(limit = 50) {
  return query<{
    id: string; tipo: string; severidad: string; descripcion: string
    inspector_id: string; taller_aliado_id: string
    distancia_m: number; creado_en: Date; cit_id: string
  }>(
    `SELECT a.id, a.tipo, a.severidad,
...
export async function getResumenAnomalias() {
  const [totales, porTipo, topInspectores, ultimasSemana] = await Promise.all([
    query<{ severidad: string; count: string }>(
      `SELECT severidad, COUNT(*)::text AS count FROM gps_anomalias GROUP BY severidad`, []
    ),
    query<{ tipo: string; count: string }>(
      `SELECT tipo, COUNT(*)::text AS count FROM gps_anomalias GROUP BY tipo ORDER BY count DESC`, []
    ),
    query<{ inspector_id: string; count: string; ultima: Date }>(
      `SELECT inspector_id, COUNT(*)::text AS count, MAX(creado_en) AS ultima
       FROM gps_anomalias WHERE severidad='CRITICA' AND revisada=FALSE
       GROUP BY inspector_id ORDER BY count DESC LIMIT 10`, []
    ),
    queryOne<{ count: string }>(
      `SELECT COUNT(*)::text AS count FROM gps_anomalias WHERE creado_en > NOW()-INTERVAL '7 days'`, []
    ),
  ])
...
export async function marcarAnomaliaRevisada(
  ...
  const row = await queryOne<{ id: string }>(
    `UPDATE gps_anomalias
     SET revisada=TRUE, revisada_por=$2, revisada_en=NOW()
     WHERE id=$1 AND revisada=FALSE RETURNING id`,
    [anomaliaId, revisadaPor]
  )
  return !!row
}

export async function getHistorialGPSInspector(inspectorId: string, limit = 50) {
  return query<{
    lat: string; lng: string; registrado_en: Date; cit_id: string
  }>(
    `SELECT lat::text, lng::text, registrado_en, cit_id
     FROM inspector_gps_historia
     WHERE inspector_id = $1
