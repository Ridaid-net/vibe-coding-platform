...
import { consultarSerial }       from './minseg.service'
import { log }                   from '../middleware/logger'
import { verificarPorSerial }    from './bfa.indexer'
...
export type CheckResultado = 'OK' | 'ALERTA' | 'BLOQUEANTE'
...
export interface ValidacionSerialResult {
  serial:          string
  bicicletaId:     string | null
  aprobado:        boolean    // false → bloquea iniciarCIT
  tieneAlertas:    boolean    // true → advertencia, no bloquea
  checks:          CheckResult[]
  resumen:         string
}
 ...
 */
function checkFormatoSerial(serial: string): CheckResult {
  const min = 5
  const max = 60
 ...
 */
async function checkExistenciaDB(serial: string): Promise<{
  check: CheckResult
  bicicletaId: string | null
  ...
  const bici = await queryOne<{
    id: string; propietario_id: string; marca: string; modelo: string; anio: number
  }>(
    `SELECT id, propietario_id, marca, modelo, anio
     FROM bicicletas WHERE numero_serie = $1`,
    [serial]
  )
...
async function checkPropiedad(
  ...
  const usuario = await queryOne<{ dni: string | null; nombre: string; apellido: string }>(
    `SELECT dni, nombre, apellido FROM usuarios WHERE id = $1`,
    [propietarioId]
  )
 ...
 */
async function checkDenunciasLocales(serial: string): Promise<CheckResult> {
  const denuncias = await query<{
    id: string; estado: string; creado_en: Date
    min_seg_expediente: string | null
  }>(
    `SELECT d.id, d.estado, d.creado_en, d.min_seg_expediente
 ...
 */
async function checkEstadoBFA(serial: string): Promise<CheckResult> {
  try {
 ...
 */
async function checkMinisterioSeguridad(serial: string): Promise<CheckResult> {
  const result = await consultarSerial(serial)
 ...
 */
async function checkCITDuplicado(bicicletaId: string | null): Promise<CheckResult> {
  if (!bicicletaId) {
    return { nombre: 'cit_duplicado', resultado: 'OK', mensaje: 'Sin bicicleta registrada — sin duplicado' }
  }

  const activo = await queryOne<{
    id: string; numero_cit: string; estado: string; fecha_vencimiento: Date | null
  }>(
    `SELECT id, numero_cit, estado, fecha_vencimiento
     FROM cits
...
export async function validarSerial(input: {
  serial:            string
  propietarioDNI:    string
  propietarioNombre: string
}): Promise<ValidacionSerialResult> {
  ...
  await persistirValidacion(
    serial, bicicletaId, null,
    result.aprobado ? (result.tieneAlertas ? 'ALERTA' : 'APROBADO') : 'RECHAZADO',
    todos
  )
...
async function persistirValidacion(
  serial:      string,
  bicicletaId: string | null,
  citId:       string | null,
  resultado:   'APROBADO' | 'RECHAZADO' | 'ALERTA',
  checks:      CheckResult[]
): Promise<void> {
  try {
    await query(
      `INSERT INTO serial_validaciones (numero_serie, bicicleta_id, cit_id, resultado, checks)
       VALUES ($1, $2, $3, $4, $5)`,
      [serial, bicicletaId, citId, resultado, JSON.stringify(checks)]
    )
  } catch (err) {
    log.bfa.warn({ err: (err as Error).message }, 'serial_validaciones insert error')
  }
}

// ── Actualizar validación con citId después del CIT creado ─

export async function vincularValidacionACIT(serial: string, citId: string): Promise<void> {
  await query(
    `UPDATE serial_validaciones
     SET cit_id = $2
