...
import { verificarFirmaWebhook }         from './minseg.protocol.service'
...
export interface RecuperacionPayload {
  tipo:                 string   // 'RECUPERACION_NOTIFICADA'
  serial:               string
  numero_expediente?:   string
  numero_denuncia?:     string
  fecha_recuperacion:   string   // ISO 8601
  lugar_recuperacion?:  string
  autoridad_actuante?:  string
  descripcion?:         string
  datos_extra?:         unknown
}
...
export interface RecuperacionResult {
  ...
  citId?:               string
  denunciaId?:          string
  citReactivado:        boolean
  denunciaActualizada:  boolean
  propietarioNotificado:boolean
  bfa:                  BFAUnlockInfo
  mensaje:              string
  estado:               'PROCESADO' | 'NO_ENCONTRADO' | 'DUPLICADO' | 'ERROR'
}

interface BFAUnlockInfo {
  intentado:  boolean
  ok:         boolean
  txHash?:    string
  error?:     string
...
export async function procesarRecuperacionMinSeg(opts: {
  rawBody:    string
  signature:  string
  timestamp:  string
  eventId:    string
  ...
  if (!firma.valida && process.env.MTLS_ALLOW_STUB !== 'true') {
    log.minseg.warn({ eventId: opts.eventId, motivo: firma.motivo }, '⚠ Firma webhook inválida')
    return _resultado('ERROR', opts.eventId, serial_placeholder, false, false, false,
      { intentado: false, ok: false }, `Firma inválida: ${firma.motivo}`)
  }
  ...
  if (yaVisto) {
    return _resultado('DUPLICADO', opts.eventId, serial_placeholder, false, false, false,
      { intentado: false, ok: false }, `Evento ${opts.eventId} ya procesado`)
  }
  // Marcar como visto (con TTL de 7 días para ventana de replay)
  await redis.set(idempKey, '1', 'EX', 7 * 86_400).catch(() => {})

  // ── 3. Parsear payload ────────────────────────────────
  let payload: RecuperacionPayload
  try {
    payload = JSON.parse(opts.rawBody)
  } catch {
    return _resultado('ERROR', opts.eventId, serial_placeholder, false, false, false,
      { intentado: false, ok: false }, 'Payload JSON inválido')
  }

  const serial = payload.serial?.toUpperCase().trim().replace(/\s+/g, '-') ?? ''
  if (!serial) {
    return _resultado('ERROR', opts.eventId, serial_placeholder, false, false, false,
      { intentado: false, ok: false }, 'Campo serial requerido')
  }

  // Registrar el evento en DB (antes del procesamiento)
  const recRow = await queryOne<{ id: string }>(
    `INSERT INTO minseg_recuperaciones
  ...
  if (!recRow) {
    ...
    return _resultado('DUPLICADO', opts.eventId, serial, false, false, false,
      { intentado: false, ok: false }, 'Evento ya registrado en DB')
  }
  const recuperacionId = recRow.id

  try {
    // ── 4. Buscar CIT + denuncia activos ─────────────────
    const citRow = await queryOne<{
      id: string; numero_cit: string; estado: string
      propietario_id: string; nft_token_id: number | null; hash_sha256: string | null
    }>(
      `SELECT c.id, c.numero_cit, c.estado, c.propietario_id,
    ...
    if (!citRow) {
      await _actualizarEstado(recuperacionId, 'NO_ENCONTRADO', null, null,
        false, { intentado: false, ok: false },
        `Serial ${serial} no registrado en RODAID`)
      return _resultado('NO_ENCONTRADO', opts.eventId, serial, false, false, false,
        { intentado: false, ok: false }, `Serial ${serial} no encontrado en RODAID`)
    }

    const denunciaRow = await queryOne<{
      id: string; estado: string; numero_expediente: string | null
    }>(
      `SELECT id, estado, numero_expediente FROM denuncias
    ...
    await transaction(async (client) => {
      ...
      if (denunciaRow) {
        await client.query(
          `UPDATE denuncias SET
           ...
           WHERE id=$1`,
          [denunciaRow.id, payload.numero_expediente ?? null]
        )
      ...
      await client.query(
        `UPDATE minseg_recuperaciones SET cit_id=$2, denuncia_id=$3 WHERE id=$1`,
        [recuperacionId, citRow.id, denunciaRow?.id ?? null]
      )
    })
    ...
    // ── 6. BFA unlock ─────────────────────────────────────
    let bfaInfo: BFAUnlockInfo = { intentado: false, ok: false }
    if (citRow.nft_token_id && citRow.estado === 'BLOQUEADO') {
      try {
        ...
        let result = { ok: false, txHash: null as string | null, error: 'BFA stub' as string | undefined }
        try {
          const segSvc = await import('./seguridad.service')
          if (denunciaRow?.id) {
            const r = await (segSvc as any).reintentarBFALock(denunciaRow.id)
        ...
        if (bfaInfo.txHash) {
          await query(
            `UPDATE minseg_recuperaciones SET bfa_unlock_tx=$2, bfa_unlock_ok=$3 WHERE id=$1`,
            [recuperacionId, bfaInfo.txHash, bfaInfo.ok]
          )
        }
      } catch (err) {
        bfaInfo = { intentado: true, ok: false, error: (err as Error).message }
      }
    }
    ...
    try {
      ...
      await query(
        `UPDATE minseg_recuperaciones SET notificado_propietario=$2, procesado_en=NOW(), estado='PROCESADO' WHERE id=$1`,
        [recuperacionId, propietarioNotificado]
      )
    } catch (err) {
      log.minseg.warn({ err: (err as Error).message, serial }, 'Error notificando propietario')
    }
    ...
    await _actualizarEstado(recuperacionId, 'ERROR', null, null,
      false, { intentado: false, ok: false }, errMsg)
    log.minseg.error({ serial, eventId: opts.eventId, err: errMsg }, '✗ Error procesando recuperación MinSeg')
    throw err
  }
}
...
async function notificarPropietario(opts: {
  ...
  numeroCIT:         string
  citId:             string
  lugarRecuperacion?: string
  autoridadActuante?: string
  ...
  import('./email.sender').then(async mail => {
    const usuario = await queryOne<{ email: string; nombre: string | null }>(
      `SELECT email, nombre FROM usuarios WHERE id=$1`, [opts.propietarioId]
    )
    if (usuario?.email) {
...
export async function reprocesarPendientes(): Promise<{
  procesados: number; exitosos: number; fallidos: number
}> {
  const pendientes = await query<{
    id: string; event_id: string; serial: string; datos_extra: unknown
  }>(
    `SELECT id, event_id, serial FROM minseg_recuperaciones
  ...
  for (const item of pendientes) {
    try {
      ...
      const citRow = await queryOne<{ id: string; estado: string; propietario_id: string }>(
        `SELECT c.id, c.estado, c.propietario_id
         FROM cits c JOIN bicicletas b ON b.id=c.bicicleta_id
         WHERE UPPER(b.numero_serie)=$1 ORDER BY c.creado_en DESC LIMIT 1`,
        [item.serial]
      )
      if (citRow && citRow.estado === 'BLOQUEADO') {
        await query(`UPDATE cits SET estado='ACTIVO', actualizado_en=NOW() WHERE id=$1`, [citRow.id])
        await query(`UPDATE denuncias SET estado='RECUPERADA' WHERE cit_id=$1 AND estado NOT IN ('RECUPERADA','ARCHIVADA')`, [citRow.id])
        await query(`UPDATE minseg_recuperaciones SET estado='PROCESADO', cit_id=$2, procesado_en=NOW() WHERE id=$1`, [item.id, citRow.id])
        exitosos++
      } else {
        await query(`UPDATE minseg_recuperaciones SET estado='NO_ENCONTRADO' WHERE id=$1`, [item.id])
        fallidos++
      }
    } catch {
      fallidos++
...
export async function getRecuperacionesMinSeg(opts?: {
  estado?: string; serial?: string; pagina?: number; porPagina?: number
}) {
  const pagina    = Math.max(1, opts?.pagina ?? 1)
  const porPagina = Math.min(100, opts?.porPagina ?? 25)
  const conds: string[] = []; const params: unknown[] = []
  let idx = 1
  if (opts?.estado) { conds.push(`estado=$${idx++}`); params.push(opts.estado) }
  if (opts?.serial) { conds.push(`serial ILIKE '%'||$${idx++}||'%'`); params.push(opts.serial) }
  const where = conds.length ? `WHERE ${conds.join(' AND ')}` : ''
  return query<any>(
    `SELECT id, event_id, serial, numero_expediente, autoridad_actuante,
...
export async function getEstadisticasRecuperaciones(dias = 30) {
  return queryOne<any>(
...
function _resultado(
  estado:   RecuperacionResult['estado'],
  eventId:  string,
  serial:   string,
  citReactivado: boolean,
  denunciaActualizada: boolean,
...
async function _actualizarEstado(
  recuperacionId: string,
  estado:         string,
  citId:          string | null,
  ...
  await query(
    `UPDATE minseg_recuperaciones SET
       estado=$2, cit_id=$3, denuncia_id=$4,
       bfa_unlock_ok=$5, error_detalle=$6, procesado_en=NOW()
     WHERE id=$1`,
    [recuperacionId, estado, citId, denunciaId, bfaOk, error ?? null]
  ).catch(() => {})
}
