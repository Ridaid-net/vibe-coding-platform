...
exports.SUGERENCIAS_PREDEFINIDAS = void 0;
exports.buscarEnCache = buscarEnCache;
exports.guardarEnCache = guardarEnCache;
exports.invalidarCacheUsuario = invalidarCacheUsuario;
exports.getCacheMetrics = getCacheMetrics;
exports.normalizar = normalizar;
exports.hashPrompt = hashPrompt;
exports.hashContexto = hashContexto;
...
exports.SUGERENCIAS_PREDEFINIDAS = SUGERENCIAS_PREDEFINIDAS;
// ══════════════════════════════════════════════════════════
...
function hashPrompt(texto) {
    return crypto_1.default.createHash('sha256').update(normalizar(texto), 'utf8').digest('hex');
}
function hashContexto(ctx) {
    // NO incluye nombre ni datos PII — solo estado funcional
    const huella = `${ctx.plan}:${ctx.bikeCount}:${ctx.citStates.sort().join('|')}`;
    return crypto_1.default.createHash('sha256').update(huella).digest('hex').slice(0, 16);
}
// ══════════════════════════════════════════════════════════
// BUSCAR EN CACHÉ
// ══════════════════════════════════════════════════════════
async function buscarEnCache(prompt, contexto, tipoConsulta) {
    ...
    for (const sug of SUGERENCIAS_PREDEFINIDAS) {
        ...
        if (match) {
            ...
            logger_1.log.mxm.info({ nivel: 0, tipo: sug.tipo, tokens: sug.tokensAhorrados }, '✅ Cache NIVEL-0 (predefinida)');
            return {
                hit: true,
    ...
    try {
        ...
        if (cached) {
            ...
            logger_1.log.mxm.info({ nivel: 1, cacheKey: cacheKey.slice(-12), ttl, tokens: data.tokens }, '✅ Cache NIVEL-1 (Redis)');
            return {
                hit: true,
    ...
    try {
        ...
        if (cached) {
            ...
            logger_1.log.mxm.info({ nivel: 2, semKey: semKey.slice(-12) }, '✅ Cache NIVEL-2 (semántico)');
            return {
                hit: true,
                respuesta: data.respuesta,
                nivel: 2,
...
// GUARDAR EN CACHÉ
// ══════════════════════════════════════════════════════════
async function guardarEnCache(prompt, respuesta, contexto, tipoConsulta, tokensEntrada, tokensSalida) {
    const redis = (0, redis_1.getRedis)();
    const promptHash = hashPrompt(prompt);
    const ctxHash = hashContexto(contexto);
    ...
    // Guardar stat en DB
    await (0, database_1.query)(`INSERT INTO gpt_cache_stats
     ...
     SET ultimo_hit=NOW()`, [promptHash, ctxHash, tipoConsulta, cacheKey, respuesta.length, tokens, ttl]).catch(() => { });
    logger_1.log.mxm.debug({ cacheKey: cacheKey.slice(-12), ttl, tokens }, '💾 Respuesta guardada en caché');
}
// ══════════════════════════════════════════════════════════
// INVALIDAR CACHÉ DEL USUARIO
// ══════════════════════════════════════════════════════════
async function invalidarCacheUsuario(contexto) {
    const redis = (0, redis_1.getRedis)();
    const ctxHash = hashContexto(contexto);
    ...
    try {
        ...
        if (keys.length > 0) {
            ...
            logger_1.log.mxm.info({ ctxHash, borradas }, '🗑 Caché de usuario invalidada');
        }
...
// MÉTRICAS DE CACHÉ
// ══════════════════════════════════════════════════════════
async function getCacheMetrics(dias = 30) {
    const redis = (0, redis_1.getRedis)();
    ...
    const totalHits = stats.reduce((s, r) => s + (r.total_hits || 0), 0);
    const tokensTotal = stats.reduce((s, r) => s + (r.tokens_total || 0), 0);
...
// HELPERS PRIVADOS
// ══════════════════════════════════════════════════════════
async function registrarHitStats(promptHash, ctxHash, nivel, tipo, respuestaLen, tokensAhorrados) {
    await (0, database_1.query)(`INSERT INTO gpt_cache_stats
       (prompt_hash, context_hash, tipo_consulta, cache_key,
        respuesta_len, tokens_ahorrados, hits, ttl_segundos)
     VALUES ($1,$2,$3,$4,$5,$6,1,$7)
     ON CONFLICT (prompt_hash, context_hash) DO UPDATE
     SET hits=gpt_cache_stats.hits+1, ultimo_hit=NOW()`, [promptHash, ctxHash, tipo,
