...
const MXM_PAGOS_URL = env.MXM_PAGOS_URL
  ?? (env.MXM_AUTH_URL
    ? `${env.MXM_AUTH_URL}/api/pagos`
    : 'https://auth.mendoza.gob.ar/api/pagos')

const MXM_TRAMITES_URL = env.MXM_TRAMITES_URL
  ?? (env.MXM_AUTH_URL
    ? `${env.MXM_AUTH_URL}/api/tramites`
    : 'https://auth.mendoza.gob.ar/api/tramites')

const TASA_CIT_ARS  = parseFloat(process.env.RODAID_TASA_CIT_ARS ?? '3000')
const MODO_STUB     = !env.MXM_CLIENT_ID && !env.MXM_PAGOS_URL
...
export interface IniciarPagoCITInput {
  citId:         string   // UUID del CIT en estado BORRADOR / PAGO_PENDIENTE
  propietarioId: string   // usuario que paga la tasa
  inspectorId?:  string   // inspector que emitió el CIT
  montoARS?:     number   // override de la tasa (default: TASA_CIT_ARS)
}

export interface PagoCITResult {
  pagoId:       string    // ID interno (cit_pagos_mxm.id)
  mxmPagoId:    string    // ID en el sistema MxM
  redirectUrl:  string    // URL para completar pago en portal MxM
  montoARS:     number
  concepto:     string
  venceEn:      Date
  estado:       string
...
export interface WebhookPagoMxMPayload {
  pagoId:       string    // mxm_pago_id
  estado:       string    // APROBADO | RECHAZADO | VENCIDO
  montoARS?:    number
  concepto?:    string
  timestamp:    string
  expedienteId?: string
...
export async function iniciarPagoCIT(input: IniciarPagoCITInput): Promise<PagoCITResult> {
  const montoARS = input.montoARS ?? TASA_CIT_ARS

  // Verificar que el CIT existe y puede pagar
  const cit = await queryOne<{
    id: string; estado: string; propietario_id: string
    propietario_dni: string | null; numero_cit: string | null
  }>(
    `SELECT id, estado, propietario_id, propietario_dni, numero_cit FROM cits WHERE id=$1`,
    [input.citId]
  )
  if (!cit) throw new AppError('CIT no encontrado', 404, 'CIT_NOT_FOUND')
  if (!['BORRADOR', 'PAGO_PENDIENTE', 'PENDIENTE'].includes(cit.estado)) {
    throw new AppError(
      `El CIT está en estado ${cit.estado}. Solo se puede pagar en estado BORRADOR.`,
      422, 'CIT_ESTADO_INVALIDO'
    )
  }

  // Obtener CUIL del propietario
  const propietario = await queryOne<{ cuil: string | null; nombre: string | null; email: string | null }>(
    `SELECT cuil, nombre, email FROM usuarios WHERE id=$1`, [input.propietarioId]
  )
  if (!propietario?.cuil) {
    throw new AppError(
      'El propietario no tiene CUIL verificado. Debe verificar su identidad con MxM Nivel 2.',
      422, 'CUIL_REQUERIDO'
    )
  }
  ...
  const yaExiste = await queryOne<{
    id: string; mxm_pago_id: string; mxm_estado: string; vence_en: Date
  }>(
    `SELECT id, mxm_pago_id, mxm_estado, vence_en FROM cit_pagos_mxm
     WHERE cit_id=$1 AND mxm_estado='PENDIENTE' AND vence_en > NOW()
     ORDER BY iniciado_en DESC LIMIT 1`,
    [input.citId]
  )
  if (yaExiste) {
    log.mxm.warn({ citId: input.citId.slice(0, 8), pagoId: yaExiste.mxm_pago_id },
      '⚠ Pago CIT ya pendiente — retornando idempotente')
    return {
      pagoId:      yaExiste.id,
  ...
  if (MODO_STUB) {
    ...
    log.mxm.warn({ citId: input.citId.slice(0, 8), mxmPagoId },
      '⚠ MxM STUB — pago de tasa CIT simulado')
  } else {
    ...
    if (!resp.ok) {
      ...
      throw new AppError(
        `Error MxM al iniciar pago: ${err.message ?? resp.status}`,
        502, 'MXM_PAGO_ERROR'
      )
    }
    const body = await resp.json() as { pagoId: string; redirectUrl: string }
  ...
  await query(
    `UPDATE cits SET estado='PAGO_PENDIENTE', actualizado_en=NOW() WHERE id=$1`,
    [input.citId]
  )
...
export async function procesarWebhookPagoMxM(opts: {
  rawBody:   string
  signature: string
  eventId:   string
  ipOrigen?: string
  ...
  if (WH_SECRET && !MODO_STUB) {
    ...
    const bodyHash  = crypto.createHash('sha256').update(opts.rawBody).digest('hex')
    const canonical = `${ts}.${bodyHash}`
    const esperado  = crypto.createHmac('sha256', WH_SECRET).update(canonical).digest('hex')
    ...
    const valida    = sigBuf.length === expBuf.length
      ? crypto.timingSafeEqual(sigBuf, expBuf) : false
    if (!valida) {
  ...
  // 3. Parsear payload
  let payload: WebhookPagoMxMPayload
  try { payload = JSON.parse(opts.rawBody) }
  catch { return { procesado: false, estado: 'ERROR', accion: 'ERROR_PARSE', mensaje: 'JSON inválido' } }

  const { pagoId: mxmPagoId, estado, expedienteId } = payload

  // 4. Buscar pago en DB
  const pago = await queryOne<{ id: string; cit_id: string | null; propietario_id: string; inspector_id: string | null }>(
    `SELECT id, cit_id, propietario_id, inspector_id
     FROM cit_pagos_mxm WHERE mxm_pago_id=$1`,
    [mxmPagoId]
  )
  if (!pago) {
  ...
  await query(
    `UPDATE cit_pagos_mxm SET
     ...
     WHERE id=$1::uuid`,
    [pago.id, nuevoEstado, opts.eventId, opts.rawBody, expedienteId ?? null]
  )
  ...
  if (nuevoEstado === 'APROBADO' && pago.cit_id) {
    ...
    await query(
      `UPDATE cits SET estado='BORRADOR', actualizado_en=NOW() WHERE id=$1 AND estado='PAGO_PENDIENTE'`,
      [pago.cit_id]
    )
    // Notificar al propietario del rechazo
    import('./device_token.service').then(dt =>
      dt.enviarPush(pago.propietario_id, {
...
async function procesarPagoAprobado(
  ...
  await query(
    `UPDATE cits SET
       estado='ACTIVO',
       actualizado_en=NOW()
     WHERE id=$1 AND estado='PAGO_PENDIENTE'`,
    [citId]
  )
  ...
  if (expedienteId) {
    await query(
      `UPDATE cits SET estado_pago_mxm=$2 WHERE id=$1`,
      [citId, expedienteId]
  ...
  await query(
    `INSERT INTO notificaciones (usuario_id, tipo, titulo, cuerpo, datos)
     VALUES ($1,'CIT_TASA_PAGADA','✅ Tasa CIT pagada — certificado activo','Tu bicicleta ya tiene su CIT activo en la Blockchain Federal Argentina.',$2::jsonb)`,
    [propietarioId, JSON.stringify({ citId, expedienteMxm: expedienteId ?? null })]
  ).catch(() => {})

  import('./device_token.service').then(dt =>
    dt.enviarPush(propietarioId, {
      titulo: '🎉 ¡CIT Activo!',
  ...
  import('./cit.triggers.service').then(async t => {
    ...
    const citData = await qo<any>(
      `SELECT c.*, b.numero_serie, b.marca, b.modelo, u.id AS usuario_id, i.taller_aliado_id
       FROM cits c
  ...
  if (inspectorId) {
    const inspector = await queryOne<{ usuario_id: string }>(
      `SELECT usuario_id FROM inspectores WHERE id=$1`, [inspectorId]
    )
    if (inspector?.usuario_id) {
      import('./device_token.service').then(dt =>
        dt.enviarPush(inspector.usuario_id, {
...
export async function getPagoCIT(citId: string) {
  return queryOne<any>(
    `SELECT p.id, p.mxm_pago_id, p.mxm_estado, p.mxm_expediente_id,
            p.monto_ars, p.concepto, p.canal, p.aprobado_en, p.vence_en, p.creado_en,
            p.webhook_recibido_en
     FROM cit_pagos_mxm p
     WHERE p.cit_id=$1 ORDER BY p.creado_en DESC LIMIT 1`,
    [citId]
  )
}

export async function getMisPagosMxM(propietarioId: string, pagina = 1, porPagina = 25) {
  const offset = (pagina - 1) * porPagina
  const [rows, total] = await Promise.all([
    query<any>(
      `SELECT p.id, p.cit_id, p.mxm_pago_id, p.mxm_estado, p.mxm_expediente_id,
       ...
       ORDER BY p.creado_en DESC LIMIT $2 OFFSET $3`,
      [propietarioId, porPagina, offset]
    ),
    queryOne<{ count: string }>(
      `SELECT COUNT(*)::text AS count FROM cit_pagos_mxm WHERE propietario_id=$1`,
      [propietarioId]
    ),
  ])
...
export async function getEstadisticasTasaCIT(dias = 30) {
  return queryOne<any>(
    `SELECT COUNT(*)::int                                              AS total,
     ...
     FROM cit_pagos_mxm WHERE creado_en > NOW()-($1||' days')::interval`,
    [dias]
  )
}

/** Simular confirmación de pago para testing (sin MxM real) */
export async function simularPagoAprobado(mxmPagoId: string): Promise<{
  procesado: boolean; estado: string; accion: string
}> {
...
async function getMxMToken(propietarioId: string): Promise<string | null> {
  const row = await queryOne<{ access_token: string }>(
    `SELECT access_token FROM mxm_tokens WHERE usuario_id=$1 AND (expires_at IS NULL OR expires_at > NOW())
...
function construirRedirectUrl(mxmPagoId: string): string {
...
export { TASA_CIT_ARS, MODO_STUB as MXM_STUB }
