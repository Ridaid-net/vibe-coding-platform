...
exports.buildPasswordChangedEmail = buildPasswordChangedEmail;
exports.sendPasswordChangedEmail = sendPasswordChangedEmail;
exports.emailCITEmitido = emailCITEmitido;
exports.emailTasaConfirmada = emailTasaConfirmada;
exports.emailDenunciaRobo = emailDenunciaRobo;
exports.emailVentaConfirmada = emailVentaConfirmada;
exports.emailCompraCompletada = emailCompraCompletada;
exports.emailDisputaAbierta = emailDisputaAbierta;
...
`;
function buildVerificationEmail(nombre, verificationUrl) {
    return {
...
function buildPasswordResetEmail(nombre, resetUrl) {
    return {
        to: '',
...
async function send(to, payload) {
    ...
    if (env_1.isDev || !env_1.env.RESEND_API_KEY) {
    ...
    try {
        ...
        const result = await resend.emails.send({
            from: env_1.env.EMAIL_FROM ?? 'noreply@rodaid.com.ar',
            to: [to],
            subject: email.subject,
            html: email.html,
            text: email.text,
        ...
        if (result.error) {
            logger_1.log.auth.error({ error: result.error, to }, 'Email send failed');
            return { ok: false, error: result.error.message };
        }
        logger_1.log.auth.info({ emailId: result.data?.id, to, subject: email.subject }, 'Email enviado');
        return { ok: true, emailId: result.data?.id };
    }
    catch (err) {
        logger_1.log.auth.error({ err, to }, 'Email service error');
...
function buildPasswordChangedEmail(nombre, ipAddress) {
...
async function sendPasswordChangedEmail(to, nombre, ipAddress) {
...
// ══════════════════════════════════════════════════════════
function emailBase(titulo, contenido, cta) {
    return `
<div style="${BASE_STYLE}">
  <div style="${CARD_STYLE}">
...
async function emailCITEmitido(opts) {
    const html = emailBase(`✅ Tu CIT fue emitido: ${opts.numeroCIT}`, `<p>Hola <strong>${opts.nombre}</strong>,</p>
     ...
     </table>`, { texto: 'Ver mi CIT', url: `${process.env.RODAID_FRONTEND_URL ?? 'https://rodaid.com.ar'}/cit/${opts.numeroCIT}` });
    return enviarEmail({
        to: opts.to,
...
async function emailTasaConfirmada(opts) {
    const html = emailBase(`💳 Pago de tasa CIT confirmado`, `<p>Hola <strong>${opts.nombre}</strong>,</p>
...
async function emailDenunciaRobo(opts) {
    const html = emailBase(`🚨 Denuncia registrada — ${opts.serial}`, `<p>Hola <strong>${opts.nombre}</strong>,</p>
     ...
     </p>`, { texto: 'Ver estado de denuncia', url: `${process.env.RODAID_FRONTEND_URL ?? 'https://rodaid.com.ar'}/denuncias/${opts.numeroDenuncia}` });
    return enviarEmail({
        to: opts.to,
...
async function emailVentaConfirmada(opts) {
    const neto = opts.montoARS - opts.comisionARS;
    const html = emailBase(`💰 Venta confirmada — $${neto.toLocaleString('es-AR')} ARS`, `<p>Hola <strong>${opts.nombre}</strong>,</p>
...
async function emailCompraCompletada(opts) {
    const html = emailBase(`✅ Compra completada — ${opts.marca} ${opts.modelo}`, `<p>Hola <strong>${opts.nombre}</strong>,</p>
...
async function emailDisputaAbierta(opts) {
    const html = emailBase(`⚠ Disputa abierta — RODAID PAY`, `<p>Hola <strong>${opts.nombre}</strong>,</p>
...
async function enviarEmail(payload) {
    if (env_1.isDev || !env_1.env.RESEND_API_KEY) {
        logger_1.log.auth.warn({ to: payload.to, subject: payload.subject }, `📧 EMAIL STUB — no enviado (configurar RESEND_API_KEY para prod)`);
    ...
    try {
        ...
        logger_1.log.auth.error({ err: err.message, to: payload.to }, 'Error enviando email');
        return { ok: false, error: err.message };
    }
}
