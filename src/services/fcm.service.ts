...
export interface FCMPayload {
  ...
  imagen?:    string
  clickUrl?:  string
  datos?:     Record<string, string>  // FCM datos solo acepta strings
  silencioso?: boolean               // true = data-only, sin barra de notificación
  // iOS-specific
  subtitulo?:     string             // iOS notification subtitle
  badge?:         number             // iOS badge count
  sound?:         string             // iOS sound file name
  categoria?:     string             // iOS interactive notification category
  threadId?:      string             // iOS notification grouping
  collapseId?:    string             // FCM/APNs collapse key
  mutableContent?: boolean           // iOS service extension (rich notifications)
}

export interface FCMResultado {
  enviados:   number
  fallidos:   number
  tokens_invalidos: string[]  // tokens que FCM reportó como inválidos
  message_id?: string
}
...
const MODO_LIVE = !!(env.FCM_PROJECT_ID && env.FCM_PRIVATE_KEY && env.FCM_CLIENT_EMAIL)
const PROJECT_ID = env.FCM_PROJECT_ID ?? ''
...
let _tokenExpira:  number        = 0

async function getOAuthToken(): Promise<string | null> {
  if (!MODO_LIVE) return 'STUB_TOKEN'

  // Usar token cacheado si aún es válido (con 60s de buffer)
  if (_accessToken && Date.now() < _tokenExpira - 60_000) return _accessToken
  ...
  try {
    ...
    log.mensajeria.error({ err: (err as Error).message }, '✗ Error obteniendo OAuth token para FCM')
    return null
  }
}
...
function buildMessage(token: string, plataforma: Plataforma, payload: FCMPayload): object {
  const datos = {
    ...payload.datos,
    source: 'RODAID',
  ...
  return {
    ...
    apns: {
      ...
      payload: {
        ...
        ...Object.fromEntries(
          Object.entries({ ...payload.datos, source: 'RODAID' }).map(([k, v]) => [k, String(v)])
        ),
      },
      fcm_options: {
...
export async function enviarPushToken(
  fcmToken:   string,
  plataforma: Plataforma,
  payload:    FCMPayload,
  opts?: { tokenId?: string; notifId?: string; usuarioId?: string }
): Promise<{ enviado: boolean; messageId?: string; error?: string; tokenInvalido?: boolean }> {
  ...
  if (!authToken) return { enviado: false, error: 'Sin token OAuth para FCM' }
  ...
  try {
    ...
    const body = await res.json() as { name?: string; error?: { message: string; code: number; status: string } }
    ...
    if (!res.ok) {
      ...
      if (tokenInvalido) {
        ...
        await query(`UPDATE fcm_tokens SET activo=FALSE WHERE token=$1`, [fcmToken]).catch(() => {})
        log.mensajeria.warn({ token: fcmToken.slice(0, 10) + '...', status: body.error?.status }, 'Token FCM inválido — desactivado')
      }
    ...
    log.mensajeria.info({ token: fcmToken.slice(0, 10) + '...', plataforma, titulo: payload.titulo, messageId }, '📱 Push FCM enviado')
...
export async function enviarPushUsuario(
  usuarioId: string,
  payload:   FCMPayload,
  opts?: { notifId?: string; plataformas?: Plataforma[] }
): Promise<FCMResultado> {

  const plataformaFiltro = opts?.plataformas?.length
    ? `AND plataforma = ANY(ARRAY[${opts.plataformas.map(p => `'${p}'`).join(',')}])`
    : ''

  const tokens = await query<{ id: string; token: string; plataforma: Plataforma }>(
    `SELECT id, token, plataforma FROM fcm_tokens
  ...
  const resultados = await Promise.allSettled(
    tokens.map(t => enviarPushToken(t.token, t.plataforma, payload, {
  ...
  resultados.forEach((r, i) => {
    if (r.status === 'fulfilled') {
      if (r.value.enviado) enviados++
        ...
        fallidos++
        if (r.value.tokenInvalido) invalidos.push(tokens[i].token)
      }
    } else {
      fallidos++
    }
  ...
  if (enviados > 0) {
    await query(
      `UPDATE fcm_tokens SET ultimo_uso=NOW() WHERE usuario_id=$1 AND activo=TRUE`,
      [usuarioId]
    ).catch(() => {})
  }
...
export async function enviarPushTopico(
  topico:  string,
  payload: FCMPayload,
): Promise<{ enviado: boolean; messageId?: string; error?: string }> {

  if (!MODO_LIVE) {
    log.mensajeria.warn({ topico, titulo: payload.titulo }, '⚠ FCM STUB — envío a tópico simulado')
    return { enviado: true, messageId: 'stub_topic_' + Date.now() }
  }

  const authToken = await getOAuthToken()
  if (!authToken) return { enviado: false, error: 'Sin token OAuth' }
  ...
  try {
    ...
    if (!res.ok) return { enviado: false, error: body.error?.message ?? `HTTP ${res.status}` }

    log.mensajeria.info({ topico, titulo: payload.titulo, messageId: body.name }, '📢 Push FCM tópico enviado')
    return { enviado: true, messageId: body.name }
  } catch (err) {
    return { enviado: false, error: (err as Error).message }
  }
}
...
export async function registrarToken(opts: {
  usuarioId:  string
  token:      string
  plataforma: Plataforma
  dispositivo?: string
  ...
  const row = await queryOne<{ id: string; created: boolean }>(
    `INSERT INTO fcm_tokens (usuario_id, token, plataforma, dispositivo, app_version, ultimo_uso)
    ...
    [opts.usuarioId, opts.token, opts.plataforma, opts.dispositivo ?? null, opts.appVersion ?? null]
  )
...
export async function desregistrarToken(token: string, usuarioId: string): Promise<boolean> {
  const result = await query(
    `UPDATE fcm_tokens SET activo=FALSE, actualizado_en=NOW()
...
export async function getTokensUsuario(usuarioId: string) {
  return query<{
    id: string; plataforma: string; dispositivo: string | null
    ultimo_uso: Date | null; activo: boolean; creado_en: Date
  }>(
    `SELECT id, plataforma, dispositivo, ultimo_uso, activo, creado_en
     FROM fcm_tokens WHERE usuario_id=$1 ORDER BY ultimo_uso DESC NULLS LAST`,
    [usuarioId]
  )
}
...
export async function suscribirTopico(tokens: string[], topico: string): Promise<void> {
  if (!MODO_LIVE) {
    log.mensajeria.warn({ topico, count: tokens.length }, '⚠ FCM STUB — suscripción a tópico simulada')
    return
  }
  const authToken = await getOAuthToken()
  if (!authToken) return
...
export async function desuscribirTopico(tokens: string[], topico: string): Promise<void> {
  if (!MODO_LIVE) return
  const authToken = await getOAuthToken()
  if (!authToken) return
...
export async function getEstadisticas(dias = 30): Promise<{
  totalTokens: number; web: number; android: number; ios: number
  mensajesEnviados: number; mensajesFallidos: number; tasaEntrega: number
}> {
  const [tokens, mensajes] = await Promise.all([
    queryOne<{ total: string; web: string; android: string; ios: string }>(
      `SELECT
         COUNT(*)::text AS total,
...
export function getModo(): 'LIVE' | 'STUB' { return MODO_LIVE ? 'LIVE' : 'STUB' }

// ── Helper privado ──────────────────────────────────────
async function registrarMensaje(opts: {
  usuarioId?: string; notifId?: string; tokenId?: string
  plataforma?: string; titulo?: string; cuerpo?: string
  estado: string; messageId?: string; error?: string
  datos?: Record<string, unknown>
}): Promise<void> {
  await query(
    `INSERT INTO fcm_mensajes
