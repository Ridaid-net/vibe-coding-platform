...
function generarNumeroExpediente(tipo: TipoTramite): string {
...
async function registrarHistorial(opts: {
  ...
  await query(
    `INSERT INTO mxm_tramites_historial
  ...
  ).catch(e => log.mxm.warn({ err: e.message }, 'Error en historial trámite'))
}
...
export async function crearTramite(opts: {
  usuarioId:    string
  citId:        string
  tipoTramite?: TipoTramite
  descripcion?: string
  ...
  leyRef?:      string
}): Promise<CrearTramiteResult> {

  const tipo = opts.tipoTramite ?? 'REGISTRO_CIT'

  // ── 1. Validar CIT ──────────────────────────────────────
  const cit = await queryOne<{
    id: string; numero_cit: string; estado: string; tasa_pagada: boolean | null
    propietario_id: string; mxm_expediente: string | null
    numero_serie: string; marca: string; modelo: string
  }>(
    `SELECT c.id, c.numero_cit, c.estado::text, c.tasa_pagada,
  ...
  if (tipo === 'REGISTRO_CIT') {
    if (!['ACTIVO', 'EN_VALIDACION'].includes(cit.estado)) throw new AppError(
      `El CIT debe estar ACTIVO para crear el expediente (estado actual: ${cit.estado})`,
      422, 'CIT_ESTADO_INVALIDO'
    )
    if (cit.mxm_expediente && tipo === 'REGISTRO_CIT') throw new AppError(
      `Ya existe un expediente para este CIT: ${cit.mxm_expediente}`,
      409, 'EXPEDIENTE_DUPLICADO'
    )
  }
  ...
  if (!identidad.conectado) throw new AppError(
    'Debés conectar tu cuenta MxM antes de crear el expediente provincial.',
    403, 'MXM_NOT_CONNECTED'
  )
  if (identidad.nivel < 2) throw new AppError(
    `Se requiere Nivel 2 MxM (RENAPER) para crear expedientes. Tu nivel: ${identidad.nivel}`,
  ...
  let respuestaRaw:     Record<string, unknown>
  let esStub:           boolean

  if (accessToken && !MODO_STUB) {
    // ── LIVE: llamada real al sistema provincial ──────────
    try {
      expedienteId = await mxmService.crearExpediente(accessToken, opts.citId)
      ...
      log.mxm.info({ expedienteId, citId: opts.citId, tipo }, '✓ Expediente MxM creado (LIVE)')

    } catch (err) {
      throw new AppError(
        `Error al crear expediente en el sistema provincial: ${(err as Error).message}`,
        502, 'MXM_TRAMITE_ERROR'
      )
    }
  } else {
    // ── STUB ──────────────────────────────────────────────
    expedienteId     = `EXP-STUB-${crypto.randomBytes(4).toString('hex').toUpperCase()}`
    ...
    log.mxm.warn({ expedienteId, citId: opts.citId, tipo },
      '⚠ MxM Trámite STUB — configurar MXM_TRAMITES_URL para expedientes reales')
  }

  // ── 5. Persistir el trámite ─────────────────────────────
  const row = await queryOne<{ id: string; creado_en: Date }>(
    `INSERT INTO mxm_tramites
  ...
  await query(
    `UPDATE cits SET
     ...
     WHERE id=$1`,
    [opts.citId, expedienteId, numeroExpediente]
  )
...
export async function procesarWebhookTramite(opts: {
  rawBody:    string
  xSignature: string | null
  payload:    Record<string, unknown>
}): Promise<{ ok: boolean; tramiteId?: string; estado?: EstadoMxM }> {
  ...
  if (!expedienteId) return { ok: true }

  // Buscar el trámite por expediente_id
  const tramite = await queryOne<{ id: string; estado_mxm: string; cit_id: string }>(
    `SELECT id, estado_mxm, cit_id FROM mxm_tramites WHERE expediente_id=$1`, [expedienteId]
  )
  if (!tramite) return { ok: true }

  const estadoFinal = mapEstadoMxM(estadoMxM)
  if (!estadoFinal || tramite.estado_mxm === estadoFinal) return { ok: true, tramiteId: tramite.id, estado: tramite.estado_mxm as EstadoMxM }

  const ahora = new Date()
  await query(
    `UPDATE mxm_tramites SET
       estado_mxm    = $2::text,
       observaciones = $3,
       actualizado_en = NOW()
     WHERE id=$1`,
    [tramite.id, estadoFinal, opts.payload.observaciones ?? null]
  )
  if (['APROBADO','RECHAZADO','ARCHIVADO'].includes(estadoFinal)) {
    await query(`UPDATE mxm_tramites SET resuelto_en=$2 WHERE id=$1`, [tramite.id, ahora])
  }

  // Sincronizar con cits.expediente_estado
  await query(
    `UPDATE cits SET expediente_estado=$2 WHERE id=$1`,
    [tramite.cit_id, estadoFinal]
  )
  ...
  log.mxm.info({ tramiteId: tramite.id, expedienteId, estadoFinal }, `Trámite → ${estadoFinal}`)
...
const ESTADOS_FLUJO: EstadoMxM[] = ['INICIADO','PRESENTADO','EN_REVISION','APROBADO']

export async function stubAvanzarEstado(tramiteId: string): Promise<{
  estadoAnterior: EstadoMxM; estadoNuevo: EstadoMxM
}> {
  const tramite = await queryOne<{ estado_mxm: string; es_stub: boolean; cit_id: string }>(
    `SELECT estado_mxm, es_stub, cit_id FROM mxm_tramites WHERE id=$1`, [tramiteId]
  )
  if (!tramite) throw new AppError('Trámite no encontrado', 404, 'NOT_FOUND')
  if (!tramite.es_stub && process.env.NODE_ENV === 'production')
    throw new AppError('Solo disponible en modo STUB', 400, 'NOT_STUB')

  const idx  = ESTADOS_FLUJO.indexOf(tramite.estado_mxm as EstadoMxM)
  if (idx < 0 || idx >= ESTADOS_FLUJO.length - 1)
    throw new AppError(`El trámite ya está en estado final: ${tramite.estado_mxm}`, 422, 'ESTADO_FINAL')

  const estadoNuevo = ESTADOS_FLUJO[idx + 1]

  await query(
    `UPDATE mxm_tramites SET estado_mxm=$2::text, actualizado_en=NOW() WHERE id=$1`,
    [tramiteId, estadoNuevo]
  )
  if (estadoNuevo === 'APROBADO') {
    await query(`UPDATE mxm_tramites SET resuelto_en=NOW() WHERE id=$1`, [tramiteId])
  }
  await query(`UPDATE cits SET expediente_estado=$2 WHERE id=$1`, [tramite.cit_id, estadoNuevo])
  await registrarHistorial({ tramiteId, estadoPrevio: tramite.estado_mxm, estadoNuevo, origen: 'SISTEMA', datos: { stub: true } })

  log.mxm.warn({ tramiteId, estadoNuevo }, `⚠ STUB: estado avanzado a ${estadoNuevo}`)
  return { estadoAnterior: tramite.estado_mxm as EstadoMxM, estadoNuevo }
}
...
export async function getTramite(id: string): Promise<Tramite | null> {
  const row = await queryOne<any>(
    `SELECT id, cit_id, usuario_id, expediente_id, numero_expediente,
     ...
     FROM mxm_tramites WHERE id=$1`,
    [id]
  )
  return row ? mapRow(row) : null
}

export async function getTramitePorCIT(citId: string): Promise<Tramite | null> {
  const row = await queryOne<any>(
    `SELECT id, cit_id, usuario_id, expediente_id, numero_expediente,
            numero_tramite, tipo_tramite, ley_referencia, descripcion,
            estado_mxm, url_consulta, observaciones, es_stub,
...
export async function getTramitesUsuario(usuarioId: string): Promise<Tramite[]> {
  const rows = await query<any>(
    `SELECT t.id, t.cit_id, t.usuario_id, t.expediente_id, t.numero_expediente,
...
export async function getHistorialTramite(tramiteId: string) {
  return query(
    `SELECT id, estado_previo, estado_nuevo, origen, datos, registrado_en
...
export async function getEstadisticasTramites(dias = 30): Promise<{
  total: number; aprobados: number; enRevision: number; rechazados: number
}> {
  const row = await queryOne<{ total: string; apro: string; rev: string; rech: string }>(
    `SELECT
       COUNT(*)::text                                            AS total,
     ...
     FROM mxm_tramites WHERE creado_en > NOW() - ($1||' days')::interval`,
    [dias]
  )
  return {
...
export async function tramiteCITEmitido(opts: {
  citId:     string
  usuarioId: string
}): Promise<void> {
  await crearTramite({
    ...
    log.mxm.error({ citId: opts.citId, err: e.message },
      '⚠ No se pudo crear el expediente MxM (no bloquea la emisión del CIT)')
  })
}

/** Crear expediente de transferencia al cerrar una venta */
export async function tramiteTransferenciaCIT(opts: {
  citId:      string
  vendedorId: string
  compradorId: string
}): Promise<void> {
  ...
  }).catch(e => log.mxm.error({ err: e.message }, 'Error expediente transferencia'))
}

// ── Helpers privados ─────────────────────────────────────

function descripcionDefault(tipo: TipoTramite, cit: { numero_cit: string; numero_serie: string; marca: string; modelo: string }): string {
  const base = `${cit.marca} ${cit.modelo} (S/N: ${cit.numero_serie}) — CIT ${cit.numero_cit}`
  const descs: Record<TipoTramite, string> = {
...
function mapEstadoMxM(raw: string): EstadoMxM | null {
  const mapa: Record<string, EstadoMxM> = {
...
function mapRow(row: any): Tramite {
  return {
    id:               row.id,
    citId:            row.cit_id,
