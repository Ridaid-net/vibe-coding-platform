...
exports.fmt = void 0;
exports.t = t;
exports.getProvinciaConfig = getProvinciaConfig;
exports.inferirProvinciaDesdeGPS = inferirProvinciaDesdeGPS;
exports.detectarLocale = detectarLocale;
exports.i18nMiddleware = i18nMiddleware;
exports.renderProvinciaParaGPT = renderProvinciaParaGPT;
exports.canonicalizarLocale = canonicalizarLocale;
...
// ── t() ─────────────────────────────────────────────────
function t(key, params, locale = 'es-AR') {
...
// ── fmt ─────────────────────────────────────────────────
exports.fmt = {
    moneda(centavos, locale = 'es-AR', monedaCod = 'ARS') {
    ...
    fecha(fecha, locale = 'es-AR', zona) {
        const d = typeof fecha === 'string' ? new Date(fecha) : fecha;
        try {
            return new Intl.DateTimeFormat(canonicalizarLocale(locale), { day: '2-digit', month: '2-digit', year: 'numeric', ...(zona ? { timeZone: zona } : {}) }).format(d);
        }
        catch {
            return d.toLocaleDateString('es-AR');
        }
    },
    fechaHora(fecha, locale = 'es-AR', zona = 'America/Argentina/Mendoza') {
        const d = typeof fecha === 'string' ? new Date(fecha) : fecha;
        try {
            return new Intl.DateTimeFormat(canonicalizarLocale(locale), { day: '2-digit', month: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit', timeZone: zona }).format(d);
        }
        catch {
            return d.toLocaleString('es-AR');
        }
    },
    relativo(fecha, locale = 'es-AR') {
        const d = typeof fecha === 'string' ? new Date(fecha) : fecha;
    ...
    numero(n, locale = 'es-AR', opts) {
        try {
            return new Intl.NumberFormat(canonicalizarLocale(locale), opts).format(n);
        }
        catch {
            return n.toLocaleString('es-AR');
        }
    },
    porcentaje(n, locale = 'es-AR') {
        try {
            return new Intl.NumberFormat(canonicalizarLocale(locale), { style: 'percent', minimumFractionDigits: 0, maximumFractionDigits: 1 }).format(n / 100);
        }
        catch {
            return `${n}%`;
        }
...
// ── getProvinciaConfig ───────────────────────────────────
async function getProvinciaConfig(codigo) {
    ...
    const row = await (0, database_1.queryOne)(`SELECT * FROM provincias_config WHERE codigo=$1`, [codigo.toUpperCase()]);
    if (!row)
        return null;
    const config = {
        ...
        tasaCITFormatted: exports.fmt.moneda(row.tasa_cit_centavos, row.locale, row.moneda),
    ...
    try {
        await redis.set(cacheKey, JSON.stringify(config), 'EX', '1800');
    }
    catch { }
    return config;
}
// ── inferirProvinciaDesdeGPS ─────────────────────────────
async function inferirProvinciaDesdeGPS(lat, lng) {
    const rows = await (0, database_1.query)('SELECT codigo,bbox_lat_min,bbox_lat_max,bbox_lng_min,bbox_lng_max FROM provincias_config WHERE activa=TRUE', []);
    for (const r of rows) {
...
// ── detectarLocale ───────────────────────────────────────
function detectarLocale(acceptLanguage) {
    if (!acceptLanguage)
        return 'es-AR';
...
function i18nMiddleware() {
    return async (req, _res, next) => {
        const locale = detectarLocale(req.headers['accept-language']);
        const codigoProv = req.query.provincia?.toUpperCase();
        let prov;
        if (codigoProv) {
        ...
        if (!prov)
            prov = await getProvinciaConfig('MZA').then(c => c ?? undefined);
        req.i18n = {
            locale, provincia: prov,
            t: (key, params) => t(key, params, prov?.locale ?? locale),
            fmt: exports.fmt,
        };
        next();
    };
...
async function renderProvinciaParaGPT(usuarioId) {
    const gpsRow = await (0, database_1.queryOne)(`SELECT AVG(insp_geo_lat)::float AS avg_lat, AVG(insp_geo_lng)::float AS avg_lng
     FROM cits c JOIN bicicletas b ON b.id=c.bicicleta_id
     WHERE b.propietario_id=$1::uuid AND c.insp_geo_lat IS NOT NULL`, [usuarioId]);
    let prov = gpsRow?.avg_lat ? await inferirProvinciaDesdeGPS(gpsRow.avg_lat, gpsRow.avg_lng) : null;
    if (!prov)
        prov = await getProvinciaConfig('MZA');
    if (!prov)
        return 'Provincia: Mendoza (default)';
    return [
...
// ── helpers ──────────────────────────────────────────────
function canonicalizarLocale(locale) {
    if (locale.startsWith('es-AR') || locale.startsWith('es'))
        return 'es-AR';
    if (locale.startsWith('pt-BR') || locale.startsWith('pt'))
        return 'pt-BR';
