...
exports.requireMtls = requireMtls;
exports.requireMtlsIpWhitelist = requireMtlsIpWhitelist;
...
function extraerDatosCert(req) {
    ...
    const headerCert = req.headers['x-ssl-client-cert']
        ?? req.headers['x-ssl-client-escaped-cert'];
    if (headerCert) {
        try {
            const pem = decodeURIComponent(headerCert);
            const cert = node_forge_1.default.pki.certificateFromPem(pem);
            const derBytes = node_forge_1.default.asn1.toDer(node_forge_1.default.pki.certificateToAsn1(cert)).getBytes();
            const thumbprint = crypto_1.default.createHash('sha256')
    ...
    const stubThumb = req.headers['x-stub-cert-thumbprint'];
    if (stubThumb && process.env.MTLS_ALLOW_STUB === 'true') {
        logger_1.log.minseg.warn({ thumb: stubThumb?.slice(0, 16) }, '⚠ mTLS STUB — sin TLS real');
        return {
            pem: null,
...
// ══════════════════════════════════════════════════════════
async function requireMtls(req, res, next) {
    const ip = req.ip ?? req.headers['x-forwarded-for']?.split(',')[0]?.trim();
    const cert = extraerDatosCert(req);
    // 1. Sin certificado o no autorizado por la CA
    if (!cert.authorized || !cert.thumbprint) {
        logger_1.log.minseg.warn({ ip, cn: cert.cn }, '⛔ mTLS: certificado ausente o no autorizado por CA');
        res.status(401).json({
            ok: false,
    ...
    const registro = await (0, database_1.queryOne)(`SELECT organizacion, cn, permisos, activo, valido_hasta, max_consultas_dia
     FROM mtls_certificados WHERE thumbprint=$1`, [cert.thumbprint]);
    if (!registro) {
        logger_1.log.minseg.warn({ ip, thumb: cert.thumbprint?.slice(0, 16), cn: cert.cn }, '⛔ mTLS: certificado no registrado');
        res.status(403).json({
            ok: false,
    ...
    if (count === 1)
        await redis.expire(rateKey, 86_400).catch(() => { });
    if (count > registro.max_consultas_dia) {
        logger_1.log.minseg.warn({ cn: cert.cn, count, max: registro.max_consultas_dia }, '⛔ mTLS: cuota diaria agotada');
        res.status(429).json({
            ok: false,
            error: 'CUOTA_AGOTADA',
...
async function requireMtlsIpWhitelist(req, res, next) {
    if (!req.mtlsClient) {
        next();
        return;
    } // requireMtls debe ir antes
    const ip = req.ip ?? req.headers['x-forwarded-for']?.split(',')[0]?.trim() ?? '';
