...
exports.startMtlsServer = startMtlsServer;
exports.getMtlsServerInfo = getMtlsServerInfo;
...
const CERT_DIR = path_1.default.join(process.cwd(), 'certs');
...
async function startMtlsServer(app) {
    ...
    if (ALLOW_STUB || !fs_1.default.existsSync(path_1.default.join(CERT_DIR, 'server-cert.pem'))) {
        logger_1.log.minseg.warn({ port: MTLS_PORT }, '⚠ mTLS STUB — usando HTTP sin TLS (solo para testing/desarrollo)');
        return new Promise(resolve => {
            const server = http_1.default.createServer(app);
            server.listen(MTLS_PORT, () => {
                logger_1.log.minseg.info({ port: MTLS_PORT }, `⚠ Servidor mTLS STUB escuchando en :${MTLS_PORT}`);
                resolve(server);
            });
        });
    }
    ...
    const tlsOptions = {
        key: fs_1.default.readFileSync(path_1.default.join(CERT_DIR, 'server-key.pem')),
        cert: fs_1.default.readFileSync(path_1.default.join(CERT_DIR, 'server-cert.pem')),
        ca: fs_1.default.readFileSync(path_1.default.join(CERT_DIR, 'ca-cert.pem')),
    ...
    return new Promise((resolve, reject) => {
        ...
        server.listen(MTLS_PORT, () => {
            logger_1.log.minseg.info({ port: MTLS_PORT, tls: 'TLS 1.2+', requestCert: true }, `🔒 Servidor mTLS escuchando en :${MTLS_PORT}`);
            resolve(server);
        });
    });
}
 ...
 */
function getMtlsServerInfo() {
    const certsExisten = fs_1.default.existsSync(path_1.default.join(CERT_DIR, 'server-cert.pem'));
    return {
        puerto: MTLS_PORT,
        modo: certsExisten && !ALLOW_STUB ? 'LIVE' : 'STUB',
        tlsVersion: 'TLS 1.2+',
        certPath: path_1.default.join(CERT_DIR, 'server-cert.pem'),
        caPath: path_1.default.join(CERT_DIR, 'ca-cert.pem'),
    };
}
