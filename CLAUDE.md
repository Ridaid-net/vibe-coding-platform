...
- **Item 8 — `app/sobre/page.tsx`'s "Que es el CIT" section describes one flat product at the old $18.000 ARS price; today there are two products (CIT Express $5.100, CIT Completo $28.500 — see `parametros_pricing_cit`) with genuinely different processes, not just different prices.** This needs a rewrite into two side-by-side blocks, not a number swap, and spans three separate spots in the same file:
  - Lines 48-83 (the main "Que es el CIT" card, heading "Cedula de Identidad Tecnica de la Bicicleta"): a single narrative paragraph plus a "Valor del tramite" stat block hardcoding `18.000` (line 64, `<p className="text-2xl font-semibold text-ink">18.000</p>`). The narrative itself (line 53) describes only the physical-inspection path ("Cada CIT es generado por un taller aliado RODAID luego de una inspeccion tecnica de 20 puntos") — that's CIT Completo's process specifically, with no mention of CIT Express at all.
  - Line 112 (the "Talleres aliados" benefits card, in the `Beneficios a usuarios` section): bullet text `'Nueva fuente de ingresos por CIT ($18.000 ARS)'` — same stale figure, and the adjacent vendedores bullet at line 111 (`'60% de la tarifa CIT va al taller aliado'`) should be double-checked against current split logic for both products before assuming it still applies identically to each.
  - Line 152 (the "Alcances del marketplace" stat grid): `{ val: '$18.000', lbl: 'ARS por CIT emitido' }` — same stale figure, third independent occurrence in one file.
  Related sighting worth checking in the same pass (not confirmed to be the same bug, just adjacent and suspicious): `components/rodaid/solicitar-verificacion-modal.tsx` (lines 98, 120) also hardcodes `$18.000 ARS` for CIT *renewal* specifically — may legitimately be a different number for a different flow (renewal vs. initial purchase), but should be verified against current pricing rather than assumed correct.
